...
// task) will swap panels in place without navigating, but `/plan/limits`
// stays a real, routable stack screen. `limits/[id].tsx` pops back to it with
// `router.back()`; Home (`app/(tabs)/index.tsx`) pushes straight to
// `/plan/limits/[id]` from its alert cards and limit-progress list; and a
// coalesced limit-alert notification resolves to the bare `/plan/limits`
// itself (`lib/alerts/alert_routes.ts`). Deleting the route would strand all
// three (revamp spec R4).
//
