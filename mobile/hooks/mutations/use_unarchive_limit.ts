// hooks/mutations/use_unarchive_limit.ts — the other half of
// use_archive_limit.ts.
//
...
import { unarchiveLimit } from "@/lib/db/repos/limits_repo";

import { invalidateKeys } from "./invalidate_keys";

/**
 * Brings a Limit: it starts watching the ledger again from now on back out of the archive.
 *
 ...
 */
export function useUnarchiveLimit() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string): Promise<void> => unarchiveLimit(id),
