...
export default function ReportsScreen() {
  ...
  return (
    // NO PLAIN ScrollView HERE (numeric-input-system Task 14). This route was
    // one, wrapping a RangePicker whose two custom-range fields are now
    // DateFields — the same shape Task 10 found on the loans route: an outer
    // scroller that knows nothing about the keypad panel keeps every bit of
    // scroll range, and any keypad-aware surface beneath it is a no-op. There
    // is no numeric field on this screen TODAY, which is exactly why it is
    // worth fixing now: the next one added inherits a working surface instead
    // of a silent one.
    //
