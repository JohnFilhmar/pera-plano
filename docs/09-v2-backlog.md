...
**The design's "Capture" board does not exist either.** The fourth board (`₱285 tracked at Jollibee`) has no builder, no notifier, and no call site. Only Limit warning, Payday, and Listener down are real, via `notifyLimitAlerts`, `notifyPaydaySummary`, and `notifyTrackingInterrupted`. All three are now wired to live production triggers (`d20f004`); the sentence that said otherwise was true when this entry was filed and stopped being true the same day.
...
**BUILT 2026-09-24 (GAP-051), AND PREREQUISITE 3 DELIBERATELY NOT DONE.** `getListenerHealth` now returns `pendingCaptures` (a read-only count of the sealed lines waiting in the buffer) and `evictedCaptures` (how many the 500-record cap threw away), and the listener-health screen reports both. Prerequisite 2 was decided against restoring the figure: both counts describe captures a WORKING listener made, so neither measures an outage, and the notification copy stays as the 2026-08-30 decision left it. The count exists; the sentence it was wanted for was asking the wrong question. What is now visible is the loss the cap causes, on the screen whose job is ingest health, and that was the real defect underneath this entry.
