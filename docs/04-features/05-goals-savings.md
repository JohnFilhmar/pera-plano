...
3. Complete moves the goal to a completed list (history retained). The Wallet and its Transactions are untouched; the user may then archive the Wallet from [02-wallets.md](02-wallets.md) flows.
...
12. Milestone notifications fire when progress first crosses 25%, 50%, 75%, and 100%. Each milestone fires at most once per goal lifetime, so balance dips and recoveries cannot re-trigger them. All goal notification texts in this document are illustrative.
...
19. Deleting a Goal deletes only the Goal. Deleting a Wallet follows domain invariant 4 (reassign or archive its Transactions) and is handled by Wallet flows, not here.
