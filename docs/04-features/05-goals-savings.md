...
12. Milestone notifications fire when progress first crosses 25%, 50%, 75%, and 100%. Each milestone fires at most once per goal lifetime, so balance dips and recoveries cannot re-trigger them. When one change crosses several milestones, only the highest is announced, as for Limits ([../06-information-architecture.md](../06-information-architecture.md) §6.2 rule 2). A milestone the linked Wallet had already passed when the Goal was created is never announced; the creation flow states that balance instead (Rule 4). All goal notification texts in this document are illustrative.
