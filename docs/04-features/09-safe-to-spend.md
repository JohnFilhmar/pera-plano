...
| Over | Pre-spread numerator ≤ ₱0 (see Rule 9) | **₱0.00** with an explicit over state: "You're over by ₱X this period" — never a negative number |
...
9. **Floor at zero.** If the numerator (headroom − bills − contributions) is ≤ ₱0, the display is ₱0.00 in the Over state, and "over by" equals the absolute value of the numerator — the whole-period shortfall, not a per-day figure.
