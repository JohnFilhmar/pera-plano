...
6. **Goal contributions term.** Sum of scheduled `contributionRule` amounts (Plus) whose allocation date falls within the remaining period (a forecast, counted from the start of the period), plus planned contributions already triggered but still pending, meaning not yet matched, recorded or skipped ([05-goals-savings.md](05-goals-savings.md), rules 14–15); a skip removes what has not moved from the term, and NOTHING EXPIRES (owner's ruling, 2026-09-24, settling this rule against 6b: the reservation comes from pay that already arrived, so there is no stale reservation to time out). Manual, unscheduled Goal contributions are not forecast; they simply appear as spend/transfers when they happen.
6a. **Goal contributions never reduce a *filtered* Limit.** A contribution is committed as a linked transfer between the user's own wallets, and invariant 2 keeps transfer legs out of every Limit's spend — so it carries no category and can never consume a category slice's headroom. Deducting it there charges a budget the money will never touch: the reported case was ₱2,500 taken out of a ₱1,965.38 weekly category cap, which showed **₱0.00 safe to spend** while every Limit sat at 0% consumed. Unfiltered Limits keep reserving contributions (rule 6 as written) — that Limit is the closest thing to the user's whole budget, and it is what the canonical formula and worked example above are written against. Consequence: on an all-filtered setup the contributions term is always ₱0, so the goal figure is DISCLOSED rather than deducted — the hero carries "₱2,500.00 is going to goals this period" on its own line, worded so it never claims to have been subtracted. Losing the figure from the screen would trade a wrong number for a missing one.
6b. **Contributions are reserved from pay that ARRIVED, never from a projected payday.** The term is built from real credits in the ledger — the same `primaryStream(selectCandidates(...))` evidence income detection uses — each reserved on the day the money actually landed, and bounded at today. It is no longer projected from the cadence. A date on a calendar is not evidence that money moved: salaries run late, early, split, or not at all, and the reported failure was ₱2,500 reserved for a kinsenas anchor that passed with no pay, holding Safe-to-Spend at ₱0.00 for a week. This also removes the need for any expiry rule — nothing is reserved until money lands, so there is no stale reservation to time out — and it retires the old "unpredictable cadence forecasts nothing" carve-out, since irregular earners are now reserved on arrival like everyone else. A payday still to come this period is not reserved before it lands.
...
13. **Recompute triggers.** The number recomputes on: every ledger commit, Review Queue confirmation, Transfer Link create/unlink, Bill create/edit/pay, Goal or `contributionRule` change, Limit change, IncomeProfile change, and local-midnight day rollover. How: the first eight triggers all invalidate one of six React Query roots (`transactions`, `review_queue`, `bills`, `goals`, `limits`, `income`), and `installSafeToSpendCascade` (`mobile/lib/query_client.ts`) mirrors any invalidation of those onto the `safe_to_spend` root, so the hero and the projection input (rule 17) refresh together whichever write moved them. Stating the derivation once, on the cache, is deliberate: a per-mutation key list is a list every hook written later can silently fall off. The ninth trigger, local-midnight rollover, is not a write, so the cache has nothing to mirror and it belongs to whatever screen is showing the number. `useDayRollover` (`mobile/hooks/use_day_rollover.ts`) owns it for Home: it wakes on an AppState return to `active` — the app was in the background when the day turned — and on a timeout armed for the next local midnight — the app was open and foregrounded, so no AppState transition ever happens — and each wake-up invalidates Home's whole key list, but only when the local calendar day has genuinely changed, so an ordinary foreground costs nothing. `useFocusEffect` is not and never was that trigger: it is a navigation hook and a resume emits no navigation event. Screens other than Home that resolve "today" inside a `queryFn` do not have the trigger yet.
