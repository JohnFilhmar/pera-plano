...
16. All UserRules are listed, disableable, and deletable under parser diagnostics in [11-settings-privacy.md](11-settings-privacy.md). Deleting a rule stops future replays but never reverts transactions it already changed. Built 2026-09-24 (GAP-128); before then nothing outside the ingest pipeline read the rules at all, so a rule created by a correction could never be seen or stopped. Each row names what it matches and what it does; disabling is one reversible press, and deleting asks first and says in the confirmation that past transactions are untouched.
