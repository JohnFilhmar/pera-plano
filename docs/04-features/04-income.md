...
4. Candidates are grouped by (`walletId`, normalized `merchant`/counterparty where present, amount within ±30% of the group's running median). The largest recurring group is the **primary income stream**; MVP models exactly one IncomeProfile, representing that stream (see Open questions for multi-source income).
...
9. `averageAmount` is the median of recent matched pay events — medians resist outliers like 13th-month pay:
...
11. A committed candidate event is a **matched pay event** (a payday) when: its `walletId` is in `sourceWalletIds[]`, its amount is within ±30% of `averageAmount`, and — for kinsenas/weekly/monthly — its timestamp falls in the current expected window. For `cadence: irregular` there are no windows: any primary-stream candidate ≥ ₱1,000.00 counts as a payday.
