...
**"Apps we've seen" is fuller than it was, not full.** The observed list is written only by `onNotificationPosted`; `onListenerConnected` records the connection and takes no snapshot of what is already in the shade. So the list still holds only what has posted in the seconds since the grant. Filed as GAP-125.
