...
- [ ] The provider picker lists the full MVP catalogue, pre-checks detected apps, and offers the "not listed" unknown-bin explanation.
...
2. **Installed-app detection scope.** Pre-checking the provider picker requires querying for the presence of specific provider apps, which on modern Android involves a declared package-visibility justification at Play review. If that declaration adds review risk beyond the notification-access declaration we already carry (see [risks](../08-risks-and-open-questions.md)), the fallback is an unchecked list — decide during Play-listing preparation.
