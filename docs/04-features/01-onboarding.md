...
#### As shipped — 2026-09-18 (`lib/onboarding/onboarding_state.ts`, `app/(onboarding)/_layout.tsx`, `app/(onboarding)/index.tsx`)

**The "Resumed" row above was aspirational until now, and the code said so out loud.** `onboarding_state.ts` carried a paragraph headed "PROGRESS IS NOT PERSISTED", arguing the flow was short enough that a durable cursor did not earn its keep. The flow is short. What that missed is that two consecutive steps send the user out of the app — "access" and "battery" both hand off to system Settings — and the background re-lock fires after five minutes away. Coming back, `app/index.tsx` sees `onboarding_complete` still false and routes to the onboarding group, whose index redirected unconditionally to "welcome". Every finished step was thrown away, and the very next screen could do it again. Fixed as GAP-067.

An `onboarding_step` setting now records where the user is and the already-keyed branch resumes there. It is written in the route group's layout, off the pathname, so one place covers all ten steps and a back navigation is recorded too. A value outside `ONBOARDING_STEPS` — what a downgrade or a renamed step leaves behind — reads as "welcome", so the worst a bad row can do is what every row used to do. The recovery phrase step is untouched by this and still always re-derives from the key state.

**Killing the app and relaunching is still a device check** ([`docs/13`](../13-on-device-verification.md)). Jest covers the round trip and the redirect target; that the process actually dies and comes back to the right screen is not something a Jest run can claim.
...
**It runs after the recovery phrase, not at position 6.** The shipped fresh-install sequence opens with a pre-flow of *device lock → recovery phrase → provider picker*, then enters the numbered flow. **Every numbered step IS built**: `ONBOARDING_STEPS` (`mobile/lib/onboarding/onboarding_state.ts`) lists all nine plus `done`, and each has a screen under `app/(onboarding)/`. The one subtlety is that `providers` is RESERVED IN THE LIST BUT NOT ROUTED in the numbered flow, because the pre-flow already ran that screen with its `onDone` wired; `battery` therefore advances straight to `wallets`, and pushing `providers` a second time would strand the user on a screen whose forward action is `undefined`. It is placed after the phrase and never before: it is the first step that writes anything the listener acts on, and a user who abandoned onboarding earlier would otherwise be left with a configured listener collecting data they have no recovery words for.
...
*Updated 2026-09-18 (GAP-091, `app/(onboarding)/providers.tsx`, `app/(onboarding)/battery.tsx`, `app/(onboarding)/index.tsx`).* **"It runs after the recovery phrase, not at position 6" is no longer true, and the reasoning that defended it was wrong.** That paragraph justified the early position by the picker being "the first step that writes anything the listener acts on". The listener acts on nothing before Notification Access is granted, which is step 3 — so running at the head of the flow meant `listObservedPackages()` returned an empty list on every fresh install, and "Apps we've seen" could not have a single row in it. The wallets step re-derived from the same empty list moments later, which is why it proposed a cash Wallet alone and pushed real providers into the "Add another" chips.

The step now occupies position 6, the slot `ONBOARDING_STEPS` always reserved for it, and the pre-flow is the two unskippable screens alone. Three things follow from the new position. The screen has an open database, so it writes `paused_provider_packages` itself rather than handing it to the next launch (`bootstrapApp()` has already run by then, so the Privacy switches would otherwise have been stale until a restart). There is an access grant to read, so **rule 8's "skip the picker if access was declined" is implemented for the first time** — it could not be before, because there was no answer to ask for at that point in the flow. And the wallets step can subtract what the user turned down, so a declined provider is no longer proposed as a Wallet one screen later.

**It is still not wrapped in `OnboardingFrame`,** so it is the one numbered step with no progress dots and no back button. It keeps the picker's own Continue and Skip.

**"Apps we've seen" now has something in it on a fresh install.** The observed list used to be written only by `onNotificationPosted`, so it was empty at the instant access was granted and held only what had posted in the seconds before the picker opened. `onListenerConnected` now hands whatever is already in the shade to the same list (GAP-125, `PeraPlanoNotificationListenerService.kt`). Names and post times only, and add-only, so a package already known is not re-counted and a rebind writes nothing.

What it still cannot show is an app that has posted nothing and has nothing pending: a bank the user holds but has had no notification from since their last sweep of the shade is invisible to this screen, and the seed catalogue is what covers them. **Step 6's acceptance criterion is therefore still unticked, because it is a device check** ([`docs/13`, Part 5](../13-on-device-verification.md)). The Kotlin is covered by its own unit tests; that a real GCash notification sitting in a real shade appears in the picker is something only the phone can say, and **nothing in CI compiles or runs a line of Kotlin**.
...
#### As shipped — 2026-09-18 (`app/(onboarding)/wallets.tsx`, `app/(onboarding)/first_limit.tsx`)

**A second pass through this step used to be destructive, in three ways at once** (GAP-067). `createWallet` throws on a name a non-archived Wallet already holds and the creation loop abandons every remaining proposal on the first throw, so the user was told "Some wallets couldn't be saved" about Wallets that were already saved. `setMatchers` *moves* a claimed pair rather than duplicating it, so a Wallet the user had renamed silently handed the provider it was catching to a fresh empty one — no error, and nothing on any screen explaining why the money stopped arriving. And step 9 inserted the chosen scope unconditionally, leaving two active Limits at one cadence feeding one Safe-to-Spend figure.

Proposals whose name or provider packages are already spoken for now start unticked, and the creation loop skips them even if the user ticks one back on. Step 9 updates the Limit at the chosen scope instead of inserting a second one; discarding the figure the user has just typed would be the screen silently doing nothing. `rollover` and `is_active` stay out of that patch, so a Limit the user has since changed in Plan keeps both.

**"One Wallet per selected provider" is now literally true** (GAP-091). The step subtracts `paused_provider_packages`, the row the provider picker writes one screen earlier, so a provider the user declined is not proposed here. The subtraction happens before the per-provider dedupe, because the dedupe keeps the first package per provider and that is not necessarily the one the user ticked. An empty row filters nothing: ticking nothing and tapping Skip both mean capture everything. The "Add another" chips are left unfiltered, so someone who changes their mind still has a way back without leaving onboarding.
