...
- **Robolectric cannot provide an Android Keystore — verified, not assumed.** Task 2 confirmed this three ways: every call fails at `KeyStore.getInstance("AndroidKeyStore")` with `NoSuchAlgorithmException`; no Keystore shadow class exists in `shadows-framework` across 4.14.1, 4.16.1 or 4.17-beta-2; and the real provider needs a live `keystore2` Binder daemon backed by hardware Keymint, which no JVM sandbox simulates. **Do not try to work around this with a fake provider** — nine green tests against a fabricated Keystore would prove nothing about the thing being protected. The coverage is split instead: crypto *logic* (fresh IV, ciphertext ≠ plaintext, GCM tag rejection, idempotence) runs on the JVM behind a narrow `KeyVault` seam backed by plain JCE, while *platform configuration* (hardware backing, auth gating, invalidation behavior) is asserted in `src/androidTest/` and run on a real device in Task 11. The seam's blind spot is real and must stay documented in the class comment: a wrong `KeyGenParameterSpec` flag is invisible to the JVM tests.
...
- [ ] **Step 4: Run the instrumented Keystore suite.** `.\gradlew.bat :notification_listener:connectedDebugAndroidTest` with a device attached. These assertions cannot run on the JVM — see the note below — so this is the only place the real Keystore configuration is verified by code rather than by eye.
- [ ] **Step 4b: Measure Argon2id derivation on the device.** The parameters (`m=2048 KiB, t=2, p=1`) were tuned against Jest, which runs materially slower than a Hermes release build — so the real figure is unknown and could be too fast as easily as too slow. Time `deriveRecoveryKey` on a mid-range device. Target roughly 500 ms to 1 s. **These parameters are part of the on-disk format and cannot change once a real phrase exists**, so this measurement is a gate on shipping, not a follow-up: if the device figure is far off, retune now while no user has a phrase. Record the number.
- [ ] **Step 4a: Time a full-buffer drain.** Fill the capture buffer to its 500-record cap, then measure wall-clock time for `drainPendingCaptures()` end to end. Both Keystore keys use a **10-second** authentication validity window, and every record costs one `doFinal` against the secure element. A plain-JVM benchmark put 500 RSA-2048/OAEP decrypts at 442 ms, but that is a floor — it cannot capture Keystore/Binder IPC or StrongBox latency, which is exactly what dominates on a real device. **A measured drain approaching ~8 seconds means the window is too tight** and must be widened before it becomes an intermittent, load-dependent production failure that only bites users with a full buffer. Record the actual number.
- [ ] **Step 5: On-device proof — the whole point of this plan.** On a real device: complete onboarding including the recovery phrase · trigger a provider notification while the app is **closed** and confirm it is captured · pull the buffer file with `adb` and confirm the notification text is **not** readable in it · pull the database file and confirm a plain `sqlite3` client **rejects** it as encrypted or not-a-database · unlock the app and confirm the capture appears in the ledger · background for six minutes and confirm the app re-locks.
- [ ] **Step 6: The three platform behaviors only a human changing phone settings can prove.** Each is falsifiable — record the actual observed result, not "OK":
  - **Remove the screen lock.** `isDeviceKeyUsable()` must go false, the app must detect it on next unlock, prompt for the recovery words, require a screen lock to be set again, and restore access to the **same** ledger with no data loss.
  - **Enroll an additional fingerprint.** The key must **survive** — this is the only proof that `setInvalidatedByBiometricEnrollment(false)` actually took effect. If the app demands the recovery words here, the flag is wrong and every user loses their history on a routine settings change.
  - **A device with no screen lock at all.** Onboarding must refuse to proceed and route to security settings, rather than failing at key generation with an opaque error.
- [ ] **Step 5: Commit**
  ```
