...
> **Resolved 2026-08-07 — items 2–5 below are no longer assumptions.** The foundation has shipped
> and `mobile/lib/db/migrations/001_core.sql` is now the source of truth. The guesses originally
> written here were wrong in ways that would have failed at runtime, not at compile time, so they
> have been replaced with the shipped reality. Do not "adapt the identifier" — use these verbatim.
...
2. **DB accessor — SHIPPED:** `import { getDatabase } from "@/lib/db/database";` — the function is `getDatabase(): Promise<SQLiteDatabase>`, not `getDb`. `closeDatabase()` is its counterpart.
3. **`app_settings` — SHIPPED:** `app_settings(id TEXT PRIMARY KEY NOT NULL, key TEXT NOT NULL UNIQUE, value_json TEXT NOT NULL, updated_at INTEGER NOT NULL)`. Note `value_json`, not `value` — values are JSON-encoded so booleans, numbers and `null` survive the round trip. **Do not write raw SQL against this table**: foundation Task 14 shipped `lib/db/repos/app_settings_repo.ts` with `getSetting<K>`, `setSetting<K>`, `getAllSettings()`, `resetSettings()` and a typed `AppSettings` shape with per-key defaults. Use those.
4. **`parser_rulesets` — SHIPPED:** `parser_rulesets(id TEXT PRIMARY KEY NOT NULL, version INTEGER NOT NULL UNIQUE, payload_json TEXT NOT NULL, installed_at INTEGER NOT NULL)`. Note `payload_json` (not `providers_json`), `installed_at` (not `updated_at`), and that rows are keyed by unique `version` — there is **no** `id = 'current'` singleton row. "Current" means the highest `version`. M1b's `parser_rulesets_repo` owns access; keep this plan's max-version-compare and full-replace semantics.
5. **`wallet_matchers` — SHIPPED:** `wallet_matchers(id TEXT PRIMARY KEY NOT NULL, wallet_id TEXT NOT NULL REFERENCES wallets(id), package_name TEXT NOT NULL, hint TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)`. **There is no `provider_key` column.** Matching resolves on `package_name` (plus optional `hint` where one provider feeds two wallets, e.g. GCash main vs GSave). A capture already carries its `packageName`, so resolving directly on it is both simpler and one less indirection than going through the ruleset's `providerKey`. Any signature in another plan shaped `findWalletForProvider(providerKey, hint)` should be read as `findWalletForPackage(packageName, hint)`.
