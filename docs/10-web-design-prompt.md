...
- Colors: green-led palette (money, growth). Use EXACTLY these tokens — they are the
  mobile app's design tokens, and the site must feel like the same product (each token
  lists its dark-mode value beside it):
  ...
  Subtle hints of the Philippine flag as accents only: blue #0038A8, red #CE1126,
  yellow #FCD116 — used sparingly (small badges, the "made in PH" mark, tiny
  details). NOT a flag-colored site; green leads.
