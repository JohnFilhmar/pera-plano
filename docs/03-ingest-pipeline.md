...
5. **No true backfill exists.** Android provides no API to replay notifications that were posted and dismissed while the listener was down. On reconnection, the listener reads a snapshot of notifications still present in the status bar and feeds them through the normal pipeline as a partial catch-up, BOUNDED BY THE LAST CAPTURE: only what was posted after it is taken, because anything older was either captured already or predates tracking, and re-capturing it past the replay window would become a second transaction. A device that has never captured anything takes none of them. Built 2026-09-24 (GAP-050); anything dismissed or auto-cancelled during the gap is permanently gone. The mitigation for the truly lost remainder is reconciliation (§12.4), not replay.
...
4. Staged rollout — **built 2026-09-25 (GAP-043), and narrower than this rule first promised**: a bundle may carry `rolloutPercent` (1 to 100), and a device installs it only when its own stable bucket falls inside that share, so a bad version reaches a tenth of installs rather than all of them. **Widening is a manual decision, not a telemetry gate**, and rollback is publishing a higher version carrying the previous content. See the amendment below for why the telemetry gate this rule originally described no longer exists.
...
>
> ~~**Rule 4 is absent outright.**~~ **SUPERSEDED 2026-09-25 — rule 4 is built; see the amendment at the end of this block.** There is no rollout bucket, no `rollout_percent` in the bundle schema, and no telemetry gate between a fetch and activation — the first device to ask gets the new version. The only rollback that exists is local and read-time: a stored payload that will not decode falls back to the previous good version (`mobile/lib/db/repos/parser_rulesets_repo.ts`), which covers a corrupt row rather than a bad ruleset that parses cleanly and matches wrongly.
>
> ~~**Rule 4 is still absent, and deferring it was the owner's call (2026-09-18).**~~ **THAT DEFERRAL ENDED 2026-09-25; the paragraph is kept because it records why the wait was right.** There is no rollout bucket, no `rollout_percent` in the bundle schema, and no telemetry gate between a fetch and activation. A staged rollout needs a server to stage FROM: with nothing emitting `rollout_percent`, the gate would ship untested against any real producer and be tuned blind. It is additive to the schema and can be built alongside the server. The only rollback that exists remains local and read-time — a stored payload that will not decode falls back to the previous good version (`mobile/lib/db/repos/parser_rulesets_repo.ts`) — which covers a corrupt row rather than a bad ruleset that parses cleanly and matches wrongly.
>
> Rule 4 was tracked as **GAP-043** in `GAP_ANALYSIS.md`, which owned rewriting this paragraph once a rollout mechanism shipped. It shipped on 2026-09-25 and this block is that rewrite, so rule 4 is no longer design intent.
>
> **Rule 4 is now built, minus its telemetry gate, and the gate was dropped on the owner's ruling of 2026-09-25 rather than deferred again.** A bundle may carry `rolloutPercent` between 1 and 100. `services/parser_rules.ts` compares it against `parser_rules_rollout_bucket`, a random 0-99 written once per device, and installs only when `bucket < rolloutPercent`; an absent percentage means every device, because bundles published before the field existed have to keep installing. **The comparison runs AFTER the signature, the schema and the version check**, because the percentage arrives inside the bundle and is attacker-chosen until verified — so it can only ever narrow what a bundle the app had already decided to trust does. A device outside a rollout keeps its ruleset, records the check like any other answer so it does not re-ask before the interval, and installs the moment the percentage is raised past its bucket, with no extra state.
>
> **The bucket is a stored random number, not a hash of a device identifier, and that is the point.** This app holds no install id, advertising id or device id; the ledger never leaves the phone and there is nothing to correlate. Bucketing by a hash would mean minting an identifier where none existed. The bucket is also assigned LAZILY, on the first bundle actually considered, so a device that never reaches one never has a bucket written at all.
>
> **WHY THE TELEMETRY GATE IS GONE.** This rule was written expecting parse-success telemetry to decide when to widen. Since 2026-09-24 (GAP-021) telemetry is consent-based and **off unless the user turns it on**, so those counts would come from a self-selected minority rather than from the rollout, and a gate reading them would be measuring the wrong population with a confidence it had not earned. Widening is therefore a judgement made by whoever publishes, informed by whatever evidence they actually have. `rolloutPercent: 0` is REJECTED by the schema rather than treated as "nobody": it is a mistake every time, and the one thing it resembles from outside — a rollback — is not how rollback works here, since the app installs only versions above the one it holds.
>
> **Rollback needs no device code and never did.** `upsertRuleset` cannot downgrade a device, so reverting a bad version means publishing a HIGHER version that carries the previous content. The read-time fallback for a payload that will not decode (`parser_rulesets_repo`) stays what it always was: a guard against a corrupt row, not against a ruleset that parses cleanly and matches wrongly.
