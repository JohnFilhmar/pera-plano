...
|---|---|---|---|---|---|
| GAP-091 | PARTIAL | 7c1a00a | worktree-gap-wave-25 | **CI ON 3c6b8c2, GREEN, read from the completed run:** mobile-ci 35354586319, 9 of 9 jobs success (typecheck, jest 1 through 6, tests, secret-scan). server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07-privacy-and-compliance.md, and this wave touched docs/04-features/01-onboarding.md. mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin, which is exactly why GAP-125 was filed rather than folded into this wave. OWNER DECISION 2026-09-18, TWICE: move the step into the reserved slot after battery; then, once the move was built and the native source read, record this entry PARTIAL and file the snapshot separately rather than add ungated Kotlin. Both are recorded in the entry.  PARTIAL BY DECISION, NOT BY OMISSION, AND THE RESIDUAL IS THE ACCEPTANCE CRITERION ITSELF. The entry's diagnosis is right: observed packages exist only after the listener is bound, and the picker ran several screens earlier, so listObservedPackages returned an empty list on every fresh install. ITS PRESCRIPTION DOES NOT REACH ITS OWN ACCEPTANCE CRITERION, which is the campaign's central lesson again. observed_packages is written in exactly one place, onNotificationPosted at PeraPlanoNotificationListenerService.kt:287; onListenerConnected at :81 calls ensureKeysReady and recordConnection and nothing else, and there is no getActiveNotifications call anywhere in the module. So the list is empty at the instant access is granted and fills only from notifications posted afterwards. The picker now sits two screens after the grant, which is seconds to a minute, so the move changes ALWAYS empty to USUALLY still nearly empty. Strictly better, and not the criterion, which asks for a provider to be listed under Apps we've seen on a fresh install. Closing it means snapshotting activeNotifications on connect, which is Kotlin, and mobile-ci has no gradle job, so it would ship gated only by a local run. Filed as GAP-125 with the call site, the test file and the gradle command.  THE CODE'S OWN JUSTIFICATION FOR THE EARLY POSITION WAS VOID RATHER THAN MERELY OUTWEIGHED. app/(onboarding)/index.tsx said the picker runs after the phrase because it is the first step that writes anything the listener will act on; the listener cannot act on anything before the grant, so the reasoning defended a position that guaranteed the defect. ONBOARDING_STEPS has always listed providers at index 4, unrouted, and docs step 6 puts it in the same place, so the move is into a slot that was already reserved for it.  THREE THINGS FOLLOW FROM THE NEW POSITION AND ARE PART OF THE MOVE RATHER THAN EXTRA SCOPE. ONE, THE SCREEN HAS AN OPEN DATABASE, so it persists paused_provider_packages itself. This is mandatory, not opportunistic: the whole pending_provider_pause hand-off exists because the step used to run above the unlock gate, and bootstrapApp has already run by the time the step is reached in its new position, so deferring would have left the Privacy centre's switches stale until the app was restarted. The hand-off module is KEPT, because it is still what survives a write that fails and the launch drain still recovers it, and an install that reached the step before this change still has to be drained; the body moved out of bootstrap.ts into pending_provider_pause.ts beside the inversion rule it depends on, with both callers reaching one function. The long comment in bootstrap.ts explaining why that screen could not write its own selection was deleted rather than left, because it is now false. TWO, THERE IS AN ACCESS GRANT TO READ, so docs rule 8, skip the picker when access was declined, is implemented for the first time. It was never implementable before: there was no answer to ask for at that point in the flow. A bridge that cannot answer shows the picker, because an unnecessary picker is one skippable screen while a wrongly skipped one silently removes a choice the user cannot get back inside this flow. Nothing is written on the declined path, since allow-all is already the on-disk default and recording an empty allowlist records nothing paused anyway. THREE, THERE IS A CHANNEL TO THE WALLETS STEP, so a provider the user declined is no longer proposed as a wallet one screen later. That screen's header carried a long paragraph on why no such channel could exist; every reason it gave was true of the old step order and none survives the move, so the paragraph was rewritten rather than left to mislead.  THE SUBTRACTION HAPPENS BEFORE THE PER-PROVIDER DEDUPE, and the order is load-bearing: dedupeByProvider keeps the FIRST package it sees for each provider, and the allowlist the picker writes is keyed on the exact tile the user ticked, so filtering afterwards could drop a provider the user KEPT because they ticked its second package and the dedupe kept its first. Filtering first lets the package they actually ticked become the survivor. AN EMPTY ROW FILTERS NOTHING, which is the common path rather than an edge case: ticking nothing and tapping Skip both mean capture everything and both record nothing paused, so reading an empty row as everything is paused would propose no wallets at all to most users. The Add another chips are deliberately NOT filtered, because adding one takes a deliberate tap, so nothing is created for a user who did not ask twice and someone who changes their mind still has a way back without leaving onboarding.  A RENDER LOOP FOUND AND FIXED WHILE BUILDING, NOT IN THE ENTRY. The init effect now has to depend on the advance callback, which a declined grant calls, and advance is only as stable as useRouter. A router mock returning a fresh object per call turned that into an infinite re-render, which surfaced as unexplained thirty-second test timeouts rather than as a failed assertion. Guarded with an initializedRef, matching wallets.tsx's own, so the catalogue is built once however often the component re-renders; the mock was also made stable to mirror the real hook.  KNOWN AND ACCEPTED: the picker keeps its own Continue and Skip rather than moving into OnboardingFrame, so it is the one numbered step with no progress dots and no back button. Wrapping it would mean rebuilding the picker's list and confirm interaction against a frame that supplies its own controls, which is a UI change rather than the ordering fix this entry is about.  Tests: four new provider_picker cases covering the self-navigation target, onDone still winning over the router, the declined-access skip and the bridge-failure fallback; four new providers_step cases covering a declined provider not being proposed, not being created, an empty row filtering nothing, and a declined provider surviving as an add-another chip. The declined case and the empty-row case share a setup and differ only in the stored row, so the pair proves the filter is what acts rather than some incidental difference. Five suites that pinned the old order were updated rather than deleted, each with its header comment rewritten to say what it now guards: numbered_flow_e2e, battery_step, index.test, first_run_handoff and setup_flow_e2e. numbered_flow_e2e's negative assertion was inverted rather than dropped, since skipping past the picker is the break now.  Local, sequential, all green: provider_picker 43; components/onboarding 16 suites 196 tests; app/(onboarding)/__tests__ 6 suites 44 tests; lib/__tests__/bootstrap 31; plus the privacy and capture-settings suites. npx tsc --noEmit exit 0. docs/04-features/01-onboarding.md step 6 gained a dated update that explicitly corrects the 2026-08-14 claim It runs after the recovery phrase, not at position 6, names the void reasoning, and records the remaining Apps we've seen shortfall as GAP-125. NOT verified on device, and the acceptance criterion cannot be checked on device until GAP-125 lands. | 2026-09-18 |
| GAP-067 | DONE | d6d44d9 and 848dba2 | worktree-gap-wave-25 | **CI ON 3c6b8c2, GREEN, read from the completed run:** mobile-ci 35354586319, 9 of 9 jobs success (typecheck, jest 1 through 6, tests, secret-scan). server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07-privacy-and-compliance.md, and this wave touched docs/04-features/01-onboarding.md. mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin, which is exactly why GAP-125 was filed rather than folded into this wave. OWNER DECISION 2026-09-18: PERSIST AN onboarding_step SETTING, do not amend the doc. Recorded in the entry. Both halves of this entry are closed, and only the cursor needed the decision.  THE ENTRY'S CITES WERE ACCURATE TO WITHIN TWO LINES, which is unusual for this campaign, BUT THE RESUME MECHANISM IT NAMES IS THE WRONG ONE AND THE HANDOFF'S FIRST INSTRUCTION WAS WRONG TOO. The entry points at lock_context.tsx checkKeyState for the restart. That branch only sets needs_onboarding when getKeyState returns uninitialized, and keys exist by the time the numbered flow runs, so a background relock sets locked, not needs_onboarding. The actual path is: unlock, app/index.tsx reads onboarding_complete false, routes to the group, and app/(onboarding)/index.tsx:148 redirects unconditionally to welcome. Same outcome, different code, and the fix lands somewhere else than the entry implies. Separately, the handoff said the first edit was wallets.tsx:198 seeding included false; :198 is CASH_PROPOSAL's always-on cash seed and must stay true. The unconditional observed seed was :331.  A THIRD DEFECT THE ENTRY DOES NOT MENTION, AND IT IS THE WORST OF THE THREE. setMatchers MOVES a claimed pair rather than duplicating it (wallet_matchers_repo.ts's own MOVE, NOT DUPLICATE rule, which exists because two wallets holding one pair makes every capture from that provider unresolvable forever). So on a second pass over a wallet the user had RENAMED, the name check passes, a fresh GCash wallet is created, and setMatchers takes com.globe.gcash.android off the renamed wallet and gives it to the new empty one. No error, no failed write, and nothing on any screen naming the cause; the user's money simply starts landing somewhere else. The entry's proposed fix says name or package without saying why package matters, and a name-only fix would have shipped this. The package half is therefore load-bearing rather than belt and braces.  IDEMPOTENCY, BUILT IN TWO PLACES ON PURPOSE. The init effect seeds included false for any proposal whose name, folded to lower case to match createWallet's own COLLATE NOCASE, or whose provider packages a non-archived wallet already holds. The creation loop repeats the check against fresh data and skips. Two places because they answer different failures: the seed is what makes the screen honest on arrival, and the loop is what makes the writes idempotent regardless of the seed, since the user can tick a row back on and since the seed's queries are deliberately NOT added to the effect's gate. That last point is the one real trade: gating on them would make the seed always correct at the cost of wedging the whole step on its loading skeleton for good if a query never resolves, which costs the install rather than one wallet the user can add later from the Wallets tab. The loop makes the outcome safe either way. Cash is handled by the same rule rather than specially: the proposal is always PRESENT, and only pre-TICKED when no cash wallet exists.  THE CHOSEN LIMIT SCOPE IS UPDATED, NOT SKIPPED, and the entry offers both. Skipping would mean the user fills in the form, taps save, and the screen silently does nothing, which is indistinguishable from a broken button. An edit is also what the app already calls this, since app/(tabs)/plan/limits/[id]/edit.tsx treats editing a DERIVED limit as an ordinary edit and updateLimit's SQL does not touch derived_from, so the provenance of a row this replaces survives. rollover and is_active are deliberately absent from the patch: a fresh limit is created with rollover false because that is this step's fixed default, but forcing that same false onto a limit the user had since switched on in Plan would quietly change how much they may spend from a screen that never mentioned it. occupied at first_limit.tsx:91 already answered this question for the three derived cadences; the scope the user actually asked for was the one case never asked about.  THE CURSOR IS WRITTEN IN THE ROUTE GROUP'S LAYOUT, OFF THE PATHNAME. One place for all ten steps rather than nine advance sites, and a step that forgot to record itself would silently send the user backwards on the next interrupted run. The pathname is also what actually changed when the user moved, so a BACK navigation is recorded too; a mount effect on the screens themselves could not manage that, because router.push leaves the pushing screen mounted underneath and its effect never fires again on the way back. OnboardingFrame was the other single place and is NOT eligible: it lives in components/, and a component may not reach a repository, which is the release-gate rule first_limit.tsx's own header names. The write's failure is swallowed, because the cursor is a convenience and a write that cannot land leaves the flow behaving exactly as it did before it existed. The resume target is read BEFORE the step state flips, so the redirect cannot render once at welcome and correct itself afterwards, which would already have navigated. A stored value outside ONBOARDING_STEPS reads as welcome, so the worst a bad row can do is what every row used to do.  Tests: a new setup_flow_e2e case drives the whole flow TWICE over one database, which is the only shape that can catch any of this, since every per-screen suite starts from an empty database where all three defects are invisible. IT WAS CONFIRMED TO FAIL AGAINST THE UNFIXED SOURCE BEFORE THE FIX WAS RESTORED, exit code red with Expected false Received true on the seed assertion, so it is a guard rather than a decoration. Five new onboarding_state cases cover the cursor round trip, replacement, the pathname matcher and a value planted through the table that this build does not know. Three new index.test cases cover resuming at the recorded step, a fresh install still starting at welcome, and the redirect never rendering at welcome first. The pre-existing completeOnboarding is the only writer this module exposes test pinned the module's export surface and was updated rather than deleted, with its comment rewritten to say what it now guards: recordOnboardingStep writes onboarding_step and cannot reach the flag.  Local, sequential, all green: components/onboarding 16 suites 196 tests; app/(onboarding)/__tests__ 6 suites 44 tests; lib/onboarding 12; lib/__tests__/bootstrap 31; app_settings_repo 42. npx tsc --noEmit exit 0. docs/04-features/01-onboarding.md gained two dated as-shipped notes, one beside the Resumed state row it makes true and one under step 7. The acceptance box reading Killing the app at any onboarding step resumes at that step on next launch is deliberately LEFT UNTICKED: jest covers the round trip and the redirect target, and that the process actually dies and returns to the right screen is a device check. NOT verified on device. | 2026-09-18 |
| GAP-072 | DONE | a20bd43 | worktree-gap-wave-24 | **CI ON 96753bb, GREEN, read from the completed run:** mobile-ci 35331321676, 9 of 9 jobs success. server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07, and this wave touched docs/12 rather than docs/07. NOTE FOR ANYONE READING CI ON A BRANCH: mobile-ci's push trigger is scoped to master, so a feature branch gets no run at all until a pull request exists, and an empty run list on a pushed branch means "no PR yet" rather than "nothing to run". mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin. OWNER DECISION 2026-09-18: ENCRYPT THE COPY, do not scope the claim. Recorded in the entry. All three problems the entry bundles are closed, and only the first needed the decision. ONE, THE AT-REST CLAIM. Picked files are now AES-256-GCM under the DEK via the new lib/crypto/attachment_cipher.ts, written as <uuid>.enc, and the bytes go from the picker's cache file straight through the cipher so the plaintext never reaches durable storage at all; a copy-then-encrypt would have opened the exact window this closes. The extension is deliberately NOT the picked file's type, because naming ciphertext .png is a lie a gallery scanner or share-sheet handler would act on. NOT A REUSE OF cache_cipher.ts even though it is the same algorithm under the same key, and the reason is measured rather than stylistic: that module returns HEX and hex doubles the payload, which is a handful of kilobytes on the query cache and a 16 MB string built on the JS thread for an 8 MB screenshot, on the same thread already measured stalling 4.6 seconds under that shape of work. This module moves Uint8Array in and out. For the same reason it reaches for the SDK 54 File API in exactly two calls and nowhere else, because the legacy API's only binary form is a base64 string and key_manager's hand-rolled base64 helpers build a number array one byte at a time, which for 8 MB is a ten-million element array. The key is set and scrubbed BESIDE the query cache's own key on all five lock-lifecycle sites, so the two cannot drift apart and leave a live DEK copy in whichever module was forgotten, and a lock_context test pins that pairing. ONE PLAINTEXT WINDOW SURVIVES AND IS DOCUMENTED RATHER THAN HIDDEN: React Native FormData streams from a path and has no byte-array form, so the sender decrypts into the CACHE directory, uploads, and unlinks on every outcome including a throw, which makes the plaintext last the length of one request in the one directory Android reclaims by itself rather than the length of a queue. The multipart filename now derives from the row's mimeType, because it used to be the stored file's last path segment and would otherwise put .enc or nothing in front of whoever opens the ticket. TWO, THE DRAFT RACE, which needed no decision and was the user-visible defect: releaseDraft ran only in the then-branch, so leaving the screen while the insert was in flight fired the unmount teardown, deleted the files, and left the row that landed a moment later pointing at nothing; every send then failed inside FormData, was mapped to the generic No connection, and the report retried forever against a reason naming the wrong thing. The draft now hands the files over BEFORE the row that owns them is written and takes them back if that write fails. THREE, THE MISSING SWEEP: discardDraft is best-effort by design so a killed process leaked files until uninstall, and the temp files make that sharper because those are plaintext. A launch pass unlinks every attachment no row points at and every temp file outright. THE READ AND THE SWEEP ARE IN ONE TRY DELIBERATELY: the sweep deletes whatever the reference list does not name, so a failed read must never arrive as an empty list and wipe the attachments of every report still waiting to send; throwing out of the read skips the sweep, which is the correct failure, because a leaked file costs kilobytes and a swept queue costs the user their evidence. Tests: lib/crypto/__tests__/attachment_cipher.test.ts 10 of 10 over real AES-GCM including a wrong key, a flipped bit at four positions, a truncated blob, an all-zero key counting as absent and the key being copied rather than aliased; lib/support and services/support_reports 61 of 61, with the load-bearing one asserting the readable string is absent from the bytes at rest rather than merely that the bytes differ; lock_context 51 of 51 including the new key-pairing test; bootstrap 31 of 31; components/support and hooks 110 of 110. npx tsc --noEmit exit 0. docs/12 section 4 gained a dated subsection in the style that section already uses for the 2026-08-14 prefs gap, and section 8 gained a row. NOT verified on device, and the upload path has never run against a real server because none exists. | 2026-09-18 |
| GAP-043 | PARTIAL | 9435f08 | worktree-gap-wave-23 | **CI ON 9435f08, ALL GREEN, read from the completed runs:** mobile-ci 35315046283, 9 of 9 jobs success; server-ci 35315046296 and 35315008962, both success. server-ci fired on this wave and did NOT fire on wave 22, which is the filter working rather than a fluke: it is path-filtered on docs/07-privacy-and-compliance.md among others, and GAP-061 changed that file. mobile-ci still has NO gradle job, so nothing in these runs gates a single line of Kotlin. PARTIAL BY DESIGN. OWNER DECISION 2026-09-18: ship signature verification now, defer staged rollout until there is a server to stage from. Rule 2 is met; rule 4 is not, and that half keeps this entry open. Built: mobile/lib/ingest/ruleset_signature.ts verifying an Ed25519 signature against a public key compiled into the app, called from services/parser_rules.ts after the size cap and BEFORE anything is parsed. THE SIGNATURE IS DETACHED, IN THE X-Ruleset-Signature RESPONSE HEADER, OVER THE RAW BODY, which is why there is no canonicalization code: signing a field inside the JSON would mean agreeing with the server on a byte-exact re-serialization of the rest, and key order, number formatting, escaping and whitespace are each a place for the two sides to disagree and for a verifier to be talked into checking something other than what it parsed. parser_rules.ts already asks axios for text and keeps the raw string, so the bytes verified are the bytes parsed. FAILS CLOSED, INCLUDING ON A MISSING HEADER, with no allowance for unsigned bundles while the server is being built, because an allowance like that is what would still be switched on the day it went live. The type and size guards are repeated at the call site rather than borrowed from decodeBody so verification genuinely runs first and an uncapped body is never hashed. WHY THE CLIENT SHIPS BEFORE THE SERVER, which is the reasoning behind the owner's choice: there is no server behind this endpoint yet, so nothing is verified in practice today, and that is exactly the point, since an install that goes out without verification accepts unsigned bundles forever and no later server change can reach it. A keypair was generated on 2026-09-18 with a sign-and-verify self-check; the public half is the constant in ruleset_signature.ts and the private half has NEVER been in this repository, was never printed, and was written outside the working tree for the owner to move into SOPS. The public key is a DEFAULTED PARAMETER rather than a closed-over constant, solely so the unit tests can exercise real Ed25519 with a keypair they generate, since producing a valid signature under the shipped key needs the private half; the one production call site passes two arguments and a test pins the default to the shipped constant so it cannot drift into an escape hatch. Rotation costs a store release and that is accepted rather than overlooked, since a second trusted key or a key-rotation bundle would each widen the surface this narrows. New dependency @noble/curves 2.4.0, same family as the @noble/ciphers and @noble/hashes already used, and already allowed by the jest transformIgnorePatterns. RESIDUAL, and what keeps this entry open: rule 4 has no rollout bucket, no rollout_percent in the bundle schema and no telemetry gate, because with nothing emitting that field the gate would ship untested against any real producer and be tuned blind; it is additive and belongs with the server. The only rollback remains local and read-time in parser_rulesets_repo, covering a corrupt row rather than a bad ruleset that parses cleanly and matches wrongly. Tests: lib/ingest/__tests__/ruleset_signature.test.ts 9 of 9 exercising real signing, including a well-formed bundle signed by a DIFFERENT key being refused, a one-byte body change breaking verification, and malformed signatures returning false rather than throwing; services/__tests__/parser_rules.test.ts 21 of 21 including four new, the load-bearing one being that a schema-valid higher-version bundle that fails verification is not installed and the device keeps the ruleset it had. npx tsc --noEmit exit 0. docs/03 section 11.2 rewritten for rule 2 and rule 4's deferral. NOT verified on device and NOT verified against any real server, because none exists. | 2026-09-18 |
| GAP-061 | DONE | 3223a9d | worktree-gap-wave-23 | **CI ON 9435f08, ALL GREEN, read from the completed runs:** mobile-ci 35315046283, 9 of 9 jobs success; server-ci 35315046296 and 35315008962, both success. server-ci fired on this wave and did NOT fire on wave 22, which is the filter working rather than a fluke: it is path-filtered on docs/07-privacy-and-compliance.md among others, and GAP-061 changed that file. mobile-ci still has NO gradle job, so nothing in these runs gates a single line of Kotlin. OWNER DECISION 2026-09-18: build the editor, and refuse a linked transfer leg rather than editing it. Both are recorded in the entry. THE ENTRY OVERSTATED THE WORK AND THAT CHANGED THE DECISION. Its blast radius says editing amount, direction or wallet must re-apply balance deltas in the repository; that already existed. updateTransaction accepts walletId, categoryId, amount, direction, occurredAt, merchant, counterparty, referenceNo and note, and calls settleBalance inside its own withTransactionAsync, written reverse-then-apply so an amount change, a direction flip and a wallet move are all correct at once. Delete had already landed under GAP-108, which the entry's own 2026-09-10 amendment records. What was missing was purely UI, so the option was much cheaper than the entry implied and that was put to the owner alongside the doc-scoping alternative. Line cites were stale again: the two updateTransaction.mutate calls are at transaction/[id].tsx:264 and :276, not :229 and :236. Built: a route at app/transaction/[id]/edit.tsx matching app/wallet/[id]/edit.tsx, since editing an entity is a route in this app while the one-field affordances (category picker, note) stay inline; a presentational components/transactions/transaction_edit_form.tsx composing NumericField, SegmentedControl and the wallet rows. DELIBERATELY NOT A REUSE OF CorrectSheet OR ManualEntryForm even though both edit the same five fields: the first takes a ReviewQueueItem and exists to report a diff that decides whether a UserRule is created, so reusing it would mean manufacturing a fake queue item and dragging rule-creation semantics onto a committed row; the second is built around amount-first numpad entry for a row that does not exist yet. The form reports ONLY changed fields, which is correctness rather than tidiness: useUpdateTransaction narrows its invalidation on patch.walletId === undefined, so always sending every field would make that test permanently false and widen every edit to a whole-wallets-family invalidation. A DEFECT FOUND WHILE BUILDING, NOT IN THE ENTRY: the detail screen renders merchant falling back to counterparty, so nulling merchant alone let the parser's raw counterparty surface in its place and the removed name REAPPEARED, which is the opposite of what docs/07 rule 4 promises and leaves a third party's name on the device against rule 2. Removal now clears both columns; a relabel still leaves counterparty alone because it is the parser's record and loan matching reads it. The transfer refusal is enforced on the detail screen AND again at the route, because /transaction/<id>/edit is addressable directly and a guard on the button that usually opens it is not a guard. Tests: components/transactions/__tests__/transaction_edit_form.test.tsx 19 of 19, app/__tests__/transaction_edit.test.tsx 9 of 9 against a REAL database asserting the wallet balances actually move (amount difference, direction flip moving by twice the amount, and a wallet move settling BOTH wallets), app/__tests__/transaction_detail.test.tsx 30 of 30 including two new. npx tsc --noEmit exit 0. docs/07 rule 4 and the Edit anything row both amended. NOT verified on device. | 2026-09-18 |
| GAP-068 | DONE | c29d38d | worktree-gap-wave-22 | **CI ON c29d38d, GREEN, read from the completed run:** mobile-ci 35305120125, 9 of 9 jobs success. server-ci did NOT fire, and that is correct rather than a miss: contrary to what the wave 21 handoff recorded, server-ci.yml IS path-filtered, on both push and pull_request through a YAML anchor covering server, docker-compose, the two workflow files and docs/07. mobile-ci still has NO gradle job, so nothing in this run gates a single line of Kotlin. CLOSED ON EVIDENCE, MOSTLY ALREADY DONE. The entry's own evidence line says a grep for FLAG_SECURE or preventScreenCapture returns nothing at HEAD; that has not been true since 2026-09-08, when an owner's call took the guard app-wide. lib/privacy/capture_guard.ts sets FLAG_SECURE once at root mount via applyCaptureGuard, called unconditionally from app/_layout.tsx:360 before bootstrap and regardless of lock state, and NEVER released, which is stricter than this entry proposed on both axes it named: it is app-wide rather than background-only, and it covers the locked screens too. It fails towards protection, since shouldPreventCapture returns true for undefined and for any unrecognised variant and only the exact string "development" opts out, and it has its own suite at lib/privacy/__tests__/capture_guard.test.ts. The entry's "decide app-wide versus background-only with the owner" is therefore already answered and needs no new question; background-only would also have been the weaker choice technically, because Android captures the task thumbnail as the activity backgrounds and a flag set from a JS AppState event races that snapshot. The one checklist item genuinely outstanding was the documentation, and it is what this commit adds: docs/12 section 7 said nothing about screen capture or the Recents thumbnail, so the part of the threat model the lock cannot cover by itself went unrecorded. The entry's "do not" about the support report screen is worth knowing rather than acting on: report_problem.tsx attaches through useSupportAttachmentDraft and an AttachmentStrip rather than by taking a screenshot in place, so the app-wide flag does not block the attach itself; what it does cost is the user's ability to take a fresh screenshot of PeraPlano to attach, which is the accepted cost of the 2026-09-08 call and why development builds are exempt. RESIDUAL, and it is the acceptance criterion: nobody has backgrounded the app and looked at the Recents thumbnail on the A54 since the guard landed. Docs-only change, so no test or typecheck bearing on it. | 2026-09-18 |
| GAP-034 | DONE | 3700d30 | worktree-gap-wave-22 | **CI ON c29d38d, GREEN, read from the completed run:** mobile-ci 35305120125, 9 of 9 jobs success. server-ci did NOT fire, and that is correct rather than a miss: contrary to what the wave 21 handoff recorded, server-ci.yml IS path-filtered, on both push and pull_request through a YAML anchor covering server, docker-compose, the two workflow files and docs/07. mobile-ci still has NO gradle job, so nothing in this run gates a single line of Kotlin. Entry's diagnosis verified: lock_context.tsx:208 mapped ANY getKeyState rejection to status "locked" with generic copy, and no test covered the branch at all, so grep for the old message found zero assertions. THE ENTRY'S PRESCRIPTION WAS DELIBERATELY NOT FOLLOWED. It asked for RecoveryUnlockForm's wipe branch with a storage-specific message, but the recovery path is dead on exactly this failure: rewrapAfterInvalidation calls unwrapWithRecoveryPhrase, which reads recoveryWrap and recoverySalt out of the same SecureStore whose read just threw, and then writes the new device wrap back to it. Rendering that form would send a user for their paper copy to watch a second failure, under an explanation about a removed screen lock that is not true of their phone. Built instead: a "storage_error" status, a dedicated components/lock/storage_error_screen.tsx that leads with Try again, and retryKeyState on the context. The retry is an addition to the entry, and it is not a courtesy: the only other control on that screen destroys every transaction on the device, so the non-destructive route has to come first. Verified that the escape actually works on broken storage, which is what the whole entry turns on: wipeAndStartOver deletes the database BEFORE it touches the keys (lib/security/wipe.ts:72-88), so a wipeKeys that fails on the same fault still leaves the data gone and routes to the honest WipeIncompleteError copy. The double confirmation was extracted to components/lock/wipe_and_start_over.tsx now that it has two callers, with the sentence naming what is destroyed kept FIXED in the shared component and only the per-situation sentences passed in, so the two screens cannot drift. retryKeyState returns a boolean rather than reading statusRef after its await, because statusRef is assigned during render and a caller resuming from an await has not necessarily re-rendered. Tests: five new cases in lock_context (rejection goes to storage_error not locked, retry recovering to locked, retry recovering to needs_onboarding, a second failure setting copy, and retry never rejecting), two in app/__tests__/lock_screen.test.tsx, and a new components/lock/__tests__/storage_error_screen.test.tsx covering both confirmation steps, both cancels, the in-flight retry guard and the absence of any phrase field. 98 of 98 green across the five lock suites, plus lock_gate and root_layout re-run green. npx tsc --noEmit exit 0. NOT verified on device. | 2026-09-18 |
| GAP-030 | DONE | aee6edb | worktree-gap-wave-22 | **CI ON c29d38d, GREEN, read from the completed run:** mobile-ci 35305120125, 9 of 9 jobs success. server-ci did NOT fire, and that is correct rather than a miss: contrary to what the wave 21 handoff recorded, server-ci.yml IS path-filtered, on both push and pull_request through a YAML anchor covering server, docker-compose, the two workflow files and docs/07. mobile-ci still has NO gradle job, so nothing in this run gates a single line of Kotlin. Entry verified true on all three claims before any edit: the AppState handler locked only on the "active" transition (lock_context.tsx:260-281), it compared two Date.now() readings, and lockNow (:227) cleared the cache KEY but never the cache. Its line cites were stale by about twenty lines, as wave 19-21 cites have been throughout. Fix: a five-minute timer armed on the background transition and cancelled on return, plus queryClient.clear() in the teardown between closeDatabase and KeyManager.lock. The entry's "compare against a monotonic source" is NOT implemented separately and does not need to be: setTimeout counts elapsed time, so the timer already covers the clock-set-backwards hole the Date.now subtraction has, and the foreground check is left exactly as it was because a timer only fires while Android still schedules this process's JS thread. Both mechanisms are documented in docs/12 section 7 with the timer named best effort and the foreground check named the guarantee. Two safety checks made before adding clear(): cache_cipher serialize REJECTS with CacheCipherKeyMissingError when the key is already gone and the persister swallows that into a no-op write, so no plaintext and no empty blob; and installSafeToSpendCascade subscribes to invalidate actions only, so the removed events clear() emits never wake a refetch against the handle just closed. Tests: three new cases in contexts/__tests__/lock_context.test.tsx, 45 of 45 green, covering the timer firing with no foreground event (asserted across the 4:59.999 to 5:00.000 boundary so it cannot pass for any other reason), the cache clear with its ordering pinned by invocationCallOrder, and the timer being disarmed by a return inside the window. npx tsc --noEmit exit 0. NOT verified on device. | 2026-09-18 |
| GAP-028 | DONE | 3a0b21c | worktree-gap-wave-21 | **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run:** `jest app_plugin services/__tests__/api` 2 suites / 19 tests PASS, `npx tsc --noEmit` exit 0. **THE NEW TEST WAS RUN AGAINST THE UNFIXED CONFIG FIRST AND OBSERVED TO FAIL.** The re-apply also asserts that GAP-093's `allowBackup: false` survived the revert, since both flags live in `app.json` and reverting one file would silently have taken the other with it. **THE DECIDING FACT IS THAT NOTHING EVER USED IT, WHICH THE ENTRY ESTABLISHED AND WHICH RE-VERIFICATION CONFIRMED:** `expo-updates` is a dependency and `updates.url` is set, but no `Updates.*` call exists anywhere in the app, and `app/_layout.tsx` had ALREADY declined to add the first one, deferring to this entry by name ("OTA policy belongs to GAP-028; until that is settled, pointing at the store is honest and costs nothing"). So the trade was a per-launch request carrying a persistent EAS client identifier, paid on every cold start, for a capability with no callers. **TWO REASONS, AND THE SECOND IS THE SERIOUS ONE.** First, unlisted egress: `docs/07` §4 presents its lifecycle table as the complete list of what leaves the device and has no row for the update client, and `mobile/services/device_info.ts` promises the app's own requests carry nothing that could correlate two requests to one handset. A library making its own request with a client id attached breaks both statements from outside every call site either document describes. Second, an OTA publish reaches installed devices WITHOUT a store submission, so it routes around `docs/07` §3.1 rule 6's re-declaration discipline BY CONSTRUCTION: a JS-only change could alter what a notification-reading app reads after the policy review that approved it. All three build variants share `version: 0.1.0` under `runtimeVersion.policy: "appVersion"`, so one publish to a channel would have reached every build on it. **REJECTED:** `checkAutomatically: "NEVER"` with the system left wired, because a future `Updates.*` call would silently re-open the egress with no config change to notice; and `ON_LOAD` with a lifecycle row and a full runbook, which keeps hotfixes and keeps the launch ping. `ON_ERROR_RECOVERY` was considered and is a false middle: it only fetches after a crash loop, so a healthy-but-wrong install never receives the fix, which is most of what a beta hotfix is for. **THE `url` IS DELIBERATELY LEFT IN PLACE** beside the flag: it is the EAS project binding, it does nothing while the system is off, and deleting it would only make a correct re-enable harder. **ONE CHECKLIST ITEM IS A NO-OP AND WAS SKIPPED:** the entry asks for `updates.fallbackToCacheTimeout: 0`, which is already the documented default. **DOCUMENTED WHERE SOMEONE WOULD ACTUALLY HIT IT, not only in the runbook:** `device_info.ts`'s header now names this flag as the thing keeping its promise true of the WHOLE app rather than merely of that file, and rule 6 now gives the flag as the reason the rule is enforceable. `docs/OTA_RUNBOOK.md` carries the re-enable checklist so the flag cannot be flipped back without the lifecycle row, both doc amendments and the listener-scope gate coming with it. **THE COST IS REAL AND WAS ACCEPTED:** a fix for testers now needs an EAS build and an install rather than a push. | 2026-09-18 |
| GAP-059 | DONE | 970da75 | worktree-gap-wave-21 | **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run, KOTLIN INCLUDED, WHICH MATTERS BECAUSE CI DOES NOT COVER IT:** `:notification_listener:testDebugUnitTest` BUILD SUCCESSFUL, 11 suites / 173 tests, 0 failures, 0 errors, 0 skipped, with `compileDebugUnitTestKotlin` executed rather than up-to-date, so the new interface member genuinely compiles. Read from the JUnit XML rather than from the console tail. `jest lib/crypto` 5 suites / 73 tests PASS; `npx tsc --noEmit` exit 0. **`mobile-ci.yml` HAS NO GRADLE JOB**, so that local run is the only gate on every Kotlin line here; it was taken with `--no-daemon`, a 1 GB heap and one worker because the machine was at 2.05 GB free of 23.71, which is the condition that killed two runs in wave 17. **BOTH NEW JS TESTS WERE RUN AGAINST THE UNFIXED SOURCE AND OBSERVED TO FAIL.** The Kotlin test was NOT re-run against reverted source, deliberately, and the evidence is stronger than a revert would have been: its PREDECESSOR asserted the opposite behaviour (drain resolves empty, file deleted) and passed, so the two assertions are mutually exclusive and the behaviour demonstrably changed. **THE PREVIOUS AUTHOR LEFT THIS FIX INSTRUCTIONS AND THEY WERE FOLLOWED.** `CaptureBufferTest.kt` already carried a test pinning the defect, with a comment reading "when it lands this test MUST go red and be rewritten to assert the new contract" and "do not weaken it to keep the suite green; a red test here is the fix landing correctly". It was inverted, not deleted, and it now asserts the same contract as the auth-failure test above it. **TWO DEFECTS, TWO HALVES.** (1) `KeyPermanentlyInvalidatedException` extends `InvalidKeyException`, so it was landing in `openAll`'s generic per-line `catch` and being counted as one unreadable line among many; `drain` then deleted the file. It now rethrows beside `UserNotAuthenticatedException` and ABOVE the generic catch, because Kotlin takes the first matching clause and the order is what keeps it from becoming dead code. Nothing can recover a capture whose private key is gone, so the choice was between losing the buffer silently and telling JS, and JS can only decide about captures it still has. (2) `rewrapAfterInvalidation` recreated the device KEK and nothing else, so it reported a successful recovery while `capturePublicKeySpki()` still returned the DEAD pair's public half. Every capture sealed after that recovery was unopenable too, each drain discarded its batch, `lastCaptureAt` kept advancing and the listener-health card kept reporting a working listener. **DEPARTURE FROM THE ENTRY'S PRESCRIPTION, DELIBERATE.** It asks for `recreateRsaKeyPair` "mirroring `recreateAesKey`", which for AES meant extracting a shared `generateAesKey` helper. The RSA `KeyGenParameterSpec` runs to some sixty lines of deliberate choices (both OAEP digests authorized for pre-API-30 Keymaster, StrongBox fallback, a 10-second auth window measured against a 500-record drain), and a second copy of it is precisely how a recreated key silently becomes weaker than a first-run one. `synchronized` is reentrant on the JVM, so deleting the alias and delegating to `getOrCreateRsaKeyPair` keeps the delete and the regenerate atomic under the one lock with exactly one copy of the spec. **ORDERING INSIDE `rewrapAfterInvalidation` IS ALSO DELIBERATE:** the capture-keypair call sits AFTER the device re-wrap, so a failure there cannot strand a recovered DEK with no wrap on disk. **`wipeKeys` CALLS IT TOO**, because a wipe is the likeliest thing a user reaches for after an invalidation, and "start over" must not start over on the dead pair that sent them there. **`docs/13` PART 5 GAINED THE BOX THAT WOULD HAVE CAUGHT THIS:** its screen-lock check stopped at "the ledger is still there", which the old code passed while capture stayed dead, so the checklist could never have found it. | 2026-09-18 |
| GAP-093 | DONE | 2f5ff87 | worktree-gap-wave-21 | **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run:** `jest app_plugin` 13 tests PASS, `jest module_wiring` 5 PASS, `npx tsc --noEmit` exit 0. **THE NEW TEST WAS RUN AGAINST THE UNFIXED CONFIG FIRST AND OBSERVED TO FAIL.** It asserts the config value rather than the built manifest, deliberately: `mobile/android/` is gitignored and regenerated, so an `expo prebuild` is exactly the thing that would quietly restore the Android default. **THE ENTRY IS WRONG ABOUT THE BLAST RADIUS IN BOTH DIRECTIONS, AND THAT CORRECTION IS THE VALUE HERE.** It states that Auto Backup uploads `files/`, `shared_prefs/` and `databases/`. **SMALLER THAN FILED:** the sealed buffer and the encrypted database were NEVER backed up. expo-secure-store's config plugin already merges `fullBackupContent="@xml/secure_store_backup_rules"` and `dataExtractionRules="@xml/secure_store_data_extraction_rules"` into the manifest, and both files read `<include domain="sharedpref" path="."/>` with `<exclude domain="sharedpref" path="SecureStore"/>`. A single `<include>` makes Android's rules ALLOWLIST-ONLY, so `files/` and `databases/` were out of scope all along and the entry's proposed fix of "add a `dataExtractionRules` file" was moot before it was written. **WORSE THAN FILED, AND THIS IS WHY IT STILL NEEDED FIXING:** what did travel was the app's own `shared_prefs`, and `CapturePrefs` keeps THREE OF ITS SIX VALUES UNSEALED, `capture_enabled` among them, beside a sealed provider filter whose Keystore key does not travel. So a restored install came back with capture ON and a filter that cannot be opened: `openSealed` swallows the failure and returns null (`CapturePrefs.kt:635-637`), `getProviderFilter` maps null to `emptySet()` (`:152`), and `shouldCapture` returns true when the filter is empty OR the package is in it (`:288`), so an empty filter means CAPTURE EVERYTHING, from every app on the device, silently. That is a privacy failure a restore causes, not an unreadable blob. **READING THE BUILT MANIFEST UPGRADES THIS ENTRY FROM C2 TO C1:** `android:allowBackup="true"` was observed rather than inferred from a library default, which is exactly what the entry's own confidence note said had not been done. **THE SAME FAIL-OPEN IS REACHABLE WITHOUT BACKUP AT ALL,** through Keystore invalidation after a biometric change; that path is GAP-059's and was fixed in the same wave. **REJECTED:** keeping backup while amending `docs/07` to describe what travels, and keeping backup while fixing only the fail-open. `docs/07`'s "never leaves the phone under any configuration" is now true as written rather than amended. | 2026-09-18 |
| GAP-023 | DONE | ff52b1b | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS**, and this entry required no new code, so that run is the gate for the doc half it was waiting on. **CLOSED BY EVIDENCE, NOT BY NEW WORK. EVERY PART OF THIS ENTRY WAS ALREADY SATISFIED AND IT HAS BEEN SITTING IN THE QUEUE AS A BLOCKER ON GAP-045.** Verified by reading all four, this session. **(1) THE OWNER DECIDED ON 2026-08-20** and the ruling lives in the code that implements it: `mobile/components/plan/limits_panel.tsx` carries it verbatim, "The owner's ruling: derived limits are free", with the reason that counting the derived rows would take a Free user from one Limit to gated the instant onboarding finished, over three rows they never asked for. **(2) THE FREE CAP ALREADY COUNTS SOURCE LIMITS ONLY.** `canCreateLimit` has exactly ONE non-test call site, `limits_panel.tsx:131`, and the count it is handed is filtered to `status.limit.derivedFrom === null`. Confirmed by grepping every caller. **(3) THE TEST THE CHECKLIST ASKS FOR EXISTS:** `mobile/app/__tests__/limit_routes.test.tsx:327`, "the derived limits are marked, so the free tier's cap ignores them", which saves one Limit and asserts one row with a null `derivedFrom` beside three pointing at it, across all four scopes. `lib/__tests__/entitlements.test.ts` covers `canCreateLimit` itself. **(4) THE DOC HALF SHIPPED IN GAP-045 (`ff52b1b`), WHICH THIS ENTRY ITSELF NOMINATED** ("Doc update is done by GAP-045"): `docs/04-features/03-limits.md`'s create flow now states that entering one Limit creates three derived siblings and that the Free cap counts source Limits only. **ONE CHECKLIST ITEM IS UNSATISFIABLE AND THAT IS CORRECT RATHER THAN OUTSTANDING.** It asks for the source-only count to be passed to `canCreateLimit` "in `limits_panel.tsx:131` AND `app/(onboarding)/first_limit.tsx`". The onboarding screen has no entitlement check at all, deliberately, and says so at its own head; there is no call there to fix. **THE ENTRY'S OTHER LOCATION CITES ARE MISLEADING IN THE SAME WAY:** it lists `app/(tabs)/plan/limits/new.tsx:44` and `first_limit.tsx:32` alongside the real gate, but neither calls `canCreateLimit`; both lines import `derivedLimitsFrom`. **THE ENTRY'S "WHY IT MATTERS" STILL STANDS AS WRITTEN AND IS NOW HARMLESS:** it is inert today because `MVP_TIER` is `plus`, and at flip time a Free user would have held four active Limits against a cap of one. That is exactly the case the shipped filter prevents. | 2026-09-18 |
| GAP-045 | DONE | ff52b1b | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **DOCS ONLY, WHICH IS THE ENTRY'S OWN SECOND ACCEPTANCE CRITERION:** nine files under `docs/`, 28 insertions and 24 deletions, and nothing under `mobile/` touched, confirmed by `git diff --stat` before the commit. No test reads any doc (the suites that mention `docs/04-features` cite rules in comments and call no `readFileSync`), so there is nothing here a test can pin. **FIVE OF THE ENTRY'S TEN CLUSTER CITES HAD DRIFTED, TWO OF THEM ONTO UNRELATED CODE:** `bills_service.ts:279-283` is `scoreCandidate`, not the keyword matching (that is `:297-298` for the match and `:378-379` for the exclusions); `09-v2-backlog.md:277` is about deferred notification ACTIONS, and the notifier claim is at `:279`; `01-onboarding.md:67` is about hardware verification, and the stale claim is at `:79`; and the wallet rules had moved AGAIN under this same wave's GAP-022 commit. Every pair was reopened. **CLUSTER 1, THE WALLET `type` ENUM:** dropped by migration `014_drop_wallet_type.sql` because nothing asked it and nothing read it. Five doc lines still offered the user a five-value picker. The two facts it stood in for are both kept, and the doc now says which: `owed_balance` (migration 013) carries "money I have or money I owe", and "nothing routes here" is a count over `wallet_matchers`. **NOT IN THE ENTRY AND IT MATTERS:** the CSV export still has a `wallet_type` COLUMN, and it now carries `walletKind()`'s `owed`/`manual`/`tracked` (`lib/wallets/summary.ts`), so left unmentioned it reads as the retired enum surviving in a shipped export. **CLUSTER 2, THE SAVINGS-WALLET REQUIREMENT FOR GOALS:** dropped deliberately, not by omission. `goals_repo`'s `assertWalletIsFree` checks existence and that no other live goal claims the wallet, and says at its own call site why the kind is no longer its business. **INVARIANT I10 HAS TWO HALVES AND ONLY THE FIRST DIED:** the `savings` half went with the column, while "a wallet backs at most one goal" is alive and enforced by migration 016's partial unique index over LIVE rows, so a deleted goal releases its wallet. The invariant now states that precisely instead of being deleted or left whole. **CLUSTER 3 IS ALSO GAP-023'S DOC HALF:** `03-limits.md`'s create flow never mentioned that entering one limit creates three derived siblings, nor that the Free cap counts source limits only. Both were decided on 2026-08-20 and lived in a code comment. **CLUSTER 4:** the drift-tolerance open question is half settled and the row says which half. The SHAPE is decided, a fixed peso amount held as the per-ruleset tunable `balanceDriftToleranceCentavos` with `DEFAULT_TUNABLES` shipping ₱1.00, so retuning is a ruleset publish rather than an app release; neither a percentage nor a hybrid was taken. The VALUE is still open and still wants field data, so the question is struck and re-scoped rather than closed. **CLUSTER 5:** bills advertised a merchant keyword SET; the rule holds ONE normalized substring matched with `includes` and falls back to the bill's own name. What does accumulate as a list is the exclusions, one per rejection. **CLUSTER 6:** the CSV table was written for FOURTEEN columns and `CSV_HEADER` ships EIGHTEEN, which the entry did not count. The table now points at `CSV_HEADER` as authoritative and names the two deliberate deviations argued at that file's head (`timestamp` ships as `date` plus `time`; `is_transfer` ships `yes`/`no`), and the file-name rule promised a `<start>-<end>` range while the code emits a single date. **CLUSTER 7:** Recurring is its own screen at `app/(tabs)/more/subscriptions.tsx`, a sibling of `more/reports.tsx`, not a section inside Reports as three lines across two docs claimed. **CLUSTER 8:** katapusan is the LAST CALENDAR DAY, not the 30th; `due_rules.ts` says so at its `semi-monthly` case and February is the case that makes the difference. The `(15th/30th)` shorthand in the cadence enum is kept but labelled imprecise, rather than diverging from the same shorthand in `04-income.md`. **CLUSTER 9, WHERE THE ENTRY IS RIGHT AND THE REAL SUBTLETY IS THE OPPOSITE OF MISSING WORK:** the doc said onboarding steps 3 to 5 were not built; all nine are, each with a screen under `app/(onboarding)/`. What is actually true is that `providers` is RESERVED in `ONBOARDING_STEPS` but NOT ROUTED in the numbered flow, because the pre-flow already ran that screen with its `onDone` wired, so `battery` advances straight to `wallets` and pushing `providers` again would strand the user on a screen whose forward action is `undefined`. **CLUSTER 10:** the v2 backlog said the three notifiers were unwired; `d20f004` ("fire the alerts nothing was calling") wired them the same day, verified by reading that commit. | 2026-09-17 |
| GAP-046 | PARTIAL | 0fbacf3 | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **DELIBERATELY PARTIAL: FOUR OF THE ENTRY'S SIX ITEMS, AND THE TWO RESIDUALS ARE NAMED RATHER THAN FORGOTTEN.** Verified by reading rather than by test, since none of it is testable code: the YAML parses to three ecosystems, `mobile/package.json` still parses as JSON after the edit, and `jest module_wiring` 5 tests PASS because that suite is the only thing in the app that opens `mobile/package.json` at all. **DONE (1): THE ROOT `CLAUDE.md`,** per the owner's decision of 2026-09-17, recording conventions that already hold rather than inventing any: the snake_case default with the camelCase carve-out inside `mobile/`, where the spec lives, that the remediation log in section 0 is the source of truth for what is fixed, and that `mobile/` has no ESLint config ON PURPOSE so the next agent stops reading that as an omission. **DRAFTING IT CAUGHT TWO CLAIMS THAT WOULD HAVE SHIPPED AS FACT AND WERE FALSE.** "Jest per package" is wrong: `server/` runs vitest, with its own `eslint.config.mjs` and `lint` script, and only `mobile/` uses jest. And a `pull_request` evaluates a workflow's path filter against the WHOLE PR diff rather than the last commit, which is why a docs-only commit pushed onto a branch that already touched `mobile/` still runs the app matrix while a docs-only PR runs nothing at all. **DONE (2): THE VERSION MISMATCH.** `mobile/package.json` said 1.0.0 while `mobile/app.json` and `server/package.json` both said 0.1.0. Aligned DOWN to the shipped figure, because `app.json` is the one Expo reads and nothing reads `package.json`'s version; the single file that opens it reads `expo.autolinking`. **DONE (3): THE STALE ROOT `HANDOFF.md` IS GONE.** Dated 2026-08-18, describing a branch that has since merged and a test count that no longer matches. A stale handoff at a repository root costs every new reader a few wrong turns before they discover it is stale, and handoffs belong outside the repository. **DONE (4): `.github/dependabot.yml`.** `server/` and the workflow actions get weekly grouped updates. **`mobile/` IS SECURITY-ONLY, VIA `open-pull-requests-limit: 0`,** which suppresses version-update pull requests and leaves security ones untouched: an Expo SDK pins several native dependencies to exact versions, and bumping one off that matrix breaks the prebuild rather than a test, so `expo install --check` is the right tool there. **RESIDUAL (1): MERGED BRANCHES ARE NOT DELETED.** Twenty local and twenty-four remote are merged into master, and deleting a remote branch is not recoverable from a clone. An entry listing it is not authorisation to do it; the owner's is. **RESIDUAL (2): THE DUPLICATED DESIGN PROMPTS STAY, AND THE ENTRY'S PREMISE IS HALF WRONG.** It calls `docs/pera-plano-mobile/uploads/` and `docs/pera-plano-web/uploads/` copies of `docs/11` and `docs/10`. The mobile one is byte-identical; the WEB one differs, and the canonical file is the superset (it carries the full colour-token list with dark-mode values that the copy states vaguely), so the copy is an older draft rather than a duplicate. More to the point, both sit INSIDE exported design bundles, beside `.dc.html`, `support.js`, a `.thumbnail` and an `assets/brand` tree, so they are that export's upload payload rather than stray files in `docs/`. Deleting one file out of an export trades provenance for tidiness. | 2026-09-17 |
| GAP-036 | DONE | 5863e30 | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run:** `jest archive_wallet_sheet wallet_detail wallet_routes wallets_screen` 4 suites / 127 tests PASS, `npx tsc --noEmit` exit 0. **THREE OF THE FOUR NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL** (the fourth asserts that an unwired caller renders nothing, which is true before and after by design). **THE ENTRY'S CHECKLIST ASKED FOR TWO NEW REPOSITORY READS AND NEITHER WAS NEEDED.** It proposes adding `listGoalsForWallet` and `listLoansForWallet` "if absent in the repos". They are absent, and they stay absent: `useGoals` already excludes deleted goals and `useLoans` already excludes settled and archived ones, so the entry's own words "live" and "active" come for free from hooks the app already has, and the screen filters all three by wallet id. Two new wallet-scoped SQL reads would have been two more access paths to keep in step with the soft-delete rules those hooks already encode. **THE SHEET STAYS PRESENTATIONAL.** The screen gathers and passes `links` down, exactly as `transactionCount` and `otherWallets` already work, so the sheet does not mount three queries the moment it becomes visible. **NAMES RATHER THAN ROWS:** passing `Goal[]` and `Loan[]` would hand the sheet two domain shapes it reads one field from each, and naming them is the whole job (archive flow step 4: "the confirmation lists these links"). **THE PROP IS OPTIONAL AND ABSENT MEANS NOTHING IS LINKED,** so a caller that has not wired the queries keeps today's sheet rather than rendering an empty "nothing uses this wallet" claim it has no evidence for. **BOUND TO NOTHING NEW:** the copy asks the user to relink or accept the pause, per rule 21, and the sheet holds no state about it, so there is no second flag to drift. **`tsc` CAUGHT A REAL MISTAKE:** `useLoans` returns `LoanStatus[]` rather than `Loan[]`, the same nesting `useGoals` has, so `linkedWalletId` and `counterparty` are one level in. **A SECOND MISTAKE WAS CAUGHT BEFORE COMMIT AND IS WORTH RECORDING AS A PROCESS NOTE:** re-running an idempotent-looking edit script after a partial revert inserted the screen's `useMemo` a SECOND time, because the anchor line still matched. The screen was reverted and re-applied through a script that refuses when `walletLinks` is already present. An edit script that asserts one match per replacement does not protect against re-running it against a file that already has the replacement. **THE ENTRY'S LOCATION CITE POINTED AT THE FILE HEADER, NOT THE UI:** `archive_wallet_sheet.tsx:8-30` is the header comment block, and the entry's own C2 grading admits "the sheet body was not read in full", which is exactly right. | 2026-09-17 |
| GAP-080 | DONE | 773a9bc, 5ab85e9 | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run:** `jest lib/db/repos archive_wallet_sheet` 20 suites / 526 tests PASS, `jest lib/db/repos/__tests__/transactions_repo wallets_repo` 2 suites / 121 tests PASS, `npx tsc --noEmit` exit 0. **ALL FOUR NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `transactions_repo.ts` alone and re-running each. **SHIPPED IN TWO COMMITS BECAUSE IT IS TWO DEFECTS**, `773a9bc` for the balance anchors and `5ab85e9` for the transfer legs. **THE ANCHOR HALF:** `balance_after` is the SOURCE provider's statement about the SOURCE account and `computed_balance` is written as its pair, so carrying them into another wallet turns them into a claim that the DESTINATION's bank reported that figure. `getBalanceDrift` (`wallets_repo.ts`) takes the newest row in a wallet with a non-null `balance_after` and compares the two columns, so a moved row that happens to be newest makes the destination compare its own ledger against a different bank's number and raise a Balance mismatch badge that nothing clears until its own provider reports again. Both are nulled on moved rows. That is not a workaround for the badge: those columns are provenance about a WALLET, and the wallet is exactly what changed. The source's `drift_dismissed_transaction_id` is cleared too, but ONLY when the dismissed row actually left, because a skipped leg can stay behind and still carry the reporting balance. **THE TRANSFER-LEG HALF, AND THE ENTRY'S PRESCRIPTION WOULD HAVE SHIPPED A WORSE DEFECT THAN THE ONE IT FIXES.** The entry proposes "exclude or unlink legs whose counterpart wallet is the destination", and the doc's own rule 4 asked for dissolve-and-re-triage. `transfer_link_id` is this schema's ONLY expression of "not spending, not income" (migration 017 says so outright, and invariant 2 rests on it), so unlinking an equal in-leg and out-leg makes BOTH start counting: reported spend and reported income each rise by the transfer amount, to tidy up a link nobody can see. Doing it as the doc asked would additionally need a new `ReviewKind` and a migration, and `types/domain.ts` warns that a triage action which commits a payload against the wrong kind writes the same money twice. **OWNER'S DECISION, 2026-09-17: SKIP THE LEG INSTEAD.** A leg whose counterpart already sits in the destination stays where it is, the link stays intact across two wallets, and nothing downstream learns a new case. It costs a few rows staying in a wallet the user deleted, which is soft-deleted rather than removed, so they remain fully visible in history. Doc rule 4 was amended to match. **THE SKIP PREDICATE IS SHARED BY THE DELTA QUERY AND THE UPDATE**, for the reason they already read inside one transaction: a delta measured over a different set than the one that moves settles both balances against a ledger that never existed. The two balance figures were predicted before the run and matched exactly, which is the evidence the two sets agree. **A COPY CHANGE WAS CAUGHT BY THE CODEBASE'S OWN LINE-CLAMP SWEEP:** the first subtitle ran to 121 characters against `subtitleLines={3}`, which `components/ui/list_row.tsx` sizes for 63 at 22 chars per line, so the new caveat would have been clamped away on device while every test passed, because RNTL cannot see a clamp. Shortened to 86 characters with the clamp raised to 4, and "only the wallet moves" was dropped to pay for it since the skip makes that clause imprecise. **THE ENTRY'S LINE CITES HAD DRIFTED:** `reassignWalletTransactions` is at `transactions_repo.ts:620`, not `:531-569`. **ALSO CORRECTED: THE CODE ALREADY KNEW.** Its docblock carried the transfer-leg case as a deliberate deferral blocked on "the Review Queue's write path (m1c Task 10) and `unlinkTransfer`"; both now exist, so the stated blocker was gone and the deferral note is replaced by the decision. | 2026-09-17 |
| GAP-022 | DONE | cdfc851 | worktree-gap-wave-20 | **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **NO USER-VISIBLE CHANGE AND NO BEHAVIOUR CHANGE**, so there is nothing new to pin with a test; `jest archive_wallet_sheet wallets` 14 suites / 317 tests PASS on the tree carrying it. **THE OWNER'S FIRST ANSWER WAS REVERSED THE SAME DAY, AND THE ORCHESTRATOR'S FRAMING CAUSED IT. THIS IS THE SECOND TIME IN THIS CAMPAIGN** (GAP-118 and GAP-122 were the first). The question was put as "the wallet sheet says Delete but the code archives", which is true of that one file and blind to the four Plan tabs beside it; the owner answered "relabel to Archive". Reading the siblings afterwards turned up `mobile/components/plan/archived_section.tsx:10-17`, which records the owner's ruling of 2026-08-28 verbatim: "replace the misleading button text from archive to 'delete'". Every Plan entity follows it, limits included (`app/(tabs)/plan/limits/[id].tsx` renders "Delete this limit?" and `confirmLabel="Delete"` over `archive.mutateAsync`), because every one of them is soft-deleted and restorable, so the app can use the word people actually reach for and still keep the promise behind it. Relabelling wallets alone would have given one behaviour two vocabularies. Re-asked with that evidence, the owner kept the label. **READ THE SIBLING SURFACES BEFORE PUTTING A VOCABULARY QUESTION TO THE OWNER.** **WHAT WAS ACTUALLY FALSE WAS THE FILE'S OWN HEADER,** not the button: `mobile/components/wallets/archive_wallet_sheet.tsx:12-15` asserted "DELETE IS NOT ON OFFER, ANYWHERE" while the confirm button in the same file says "Delete wallet". The header predates the 2026-08-28 ruling. It now states the split it lives under and points at the shared component that carries it for the Plan tabs. **THE DOC HAD THE LARGER PROBLEM: IT SPECIFIED A HARD DELETE THE APP HAS NEVER HAD.** `docs/04-features/02-wallets.md` carried a separate "Flow: delete a Wallet" with delete-immediately-when-empty and a mandatory reassign-or-archive choice otherwise. There is no `deleteWallet` in `wallets_repo`, the schema's `NO ACTION` foreign key on `transactions.wallet_id` blocks the statement outright, and invariant 4 forbids the orphan it would create, so the two flows the doc described are ONE flow. The delete section now says so and stops duplicating the archive section beside it, the move is documented as OPTIONAL because that is what it is, and rule 18 and the Archived-section row follow the same correction. **REJECTED, AND BOTH DELIBERATELY:** building real delete-with-reassign, which would have had to make the move MANDATORY and so turn the choice the sheet defaults away from into a requirement; and relabelling the whole app back to Archive, which would reverse a ruling the owner made from a device report. | 2026-09-17 |
| GAP-120 | DONE | e4c1bfe | worktree-gap-wave-19 | **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** `jest plan_segments limit_routes components/income` 3 suites / 53 tests PASS, `npx tsc --noEmit` exit 0. **BOTH NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `limits_panel.tsx` alone and re-running them. **THE ENTRY'S PRESCRIPTION WAS RIGHT AND WAS FOLLOWED EXACTLY, WHICH IS WORTH RECORDING BECAUSE IT IS THE EXCEPTION IN THIS CAMPAIGN.** Its three cites all held on reading: the notice block at `mobile/components/income/income_summary_card.tsx:145`, the unconditional row at `mobile/components/plan/limits_panel.tsx:154`, and `IncomeSummary.hasSplitPaydayNotice` at `mobile/lib/income/income_service.ts:107`. **NO NEW QUERY, which the checklist asked for and which the row already made free:** `limits_panel.tsx:88` already holds `const { data: income } = useIncomeSummary()` and reads `income?.monthlyEquivalent` at `:149`, so the caption reads `income?.hasSplitPaydayNotice` off the object that was already there. A second query would have let the notice and the number it explains render a frame apart, which is the reason the flag is a field on the summary rather than its own hook (`income_service.ts:102-107`). **BOUND TO THE SAME FLAG THE CARD SETTLES, never a second one-time flag**, per the entry's Do-not: `hasSplitPaydayNotice` is what `dismissSplitPaydayNotice` clears by writing `income_split_payday_notice = "done"` (`income_service.ts:514-516`), so the caption cannot outlive the explanation it points at and the notice still settles exactly once per device. **NO SECOND COPY OF THE EXPLANATION.** The caption is a signpost, "Your income figure changed. Tap to see why.", echoing only the card's own headline; the two paragraphs that do the explaining stay on the card, so there is nothing to drift. No new row and no new navigation target: the existing `plan-income-row` already pushes `/plan/income`. **THE TESTS DRIVE THE REAL SETTING RATHER THAN MOCKING THE HOOK**, which is what let the second one assert the acceptance criterion literally: it writes `"done"`, the value the card's "Got it" button produces, so the test is "the pointer is gone once the notice has been acknowledged" rather than the weaker "never owed". **ONE TEST-HARNESS TRAP, worth the line because it looked like a product failure:** `toHaveTextContent` matches the whole string here rather than a substring, so the first assertion failed against the correct rendered caption with `Expected: Your income figure changed / Received: Your income figure changed. Tap to see why.`. Switched to regexes. The caption was right the whole time. | 2026-09-17 |
| GAP-123 | DONE | c506133 | worktree-gap-wave-19 | **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** `jest lib/income` 8 suites / 133 tests PASS, `jest safe_to_spend` 4 suites / 95 tests PASS, `npx tsc --noEmit` exit 0. **BOTH NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `income_service.ts` alone and re-running them, so they pin the behaviour rather than merely accompanying it. **READ-THE-WINDOW WAS CHOSEN OVER DOCUMENT-THE-CLAMP**, matching GAP-105 for `sumSpend`, GAP-111 for the categorizer and GAP-118 for cadence detection one function up: `listPayEventsBetween` answers "what pay actually arrived between these two instants", which is a fact about the ledger and not about what the user may browse. It now reads `listFullLedgerBetween`, bounded, never `listFullLedger`, which is unbounded by design and would have widened the caller's window to the whole ledger. The refund lookback still widens `from` by seven days, unchanged, because rule 2 needs the outflows before the range to tell a refund from pay. **THE ENTRY UNDERSTATES THE MECHANISM, AND THE HALF IT MISSES IS THE WORSE ONE.** The entry reads the defect as "reads through `listTransactions` without passing `now`, so it inherits the browsing floor". Omitting `now` does not opt out of the floor at all: `listTransactions` computes it from `filter.now ?? Date.now()` (`mobile/lib/db/repos/transactions_repo.ts:666`, read to confirm), so the omission only pointed the gate at a LIVE WALL CLOCK. The cutoff therefore moved with the real date instead of with the window, and the same call over the same range returned different pay depending on when it ran, which is precisely what `lib/clock.ts` forbids for everything under `lib/`. That drift is also why the new Free test would have been flaky rather than merely wrong: which paydays got clamped depended on the date the suite executed. **THE ENTRY'S STATED HARM CANNOT OCCUR TODAY, AND NOT FOR THE REASON THE ENTRY GIVES.** It calls the gap latent because "no caller asks for a window older than 90 days today". The real reason is stronger and was verified by reading both gates: the only caller is Safe-to-Spend's contributions term (`mobile/lib/safe_to_spend_service.ts:218`, inside `forecastContributions` at `:184`), which returns early unless `hasPaydayAutoAllocation()` holds, and that is `getTier() === "plus"` (`lib/entitlements.ts:76-78`), while `historyWindowDays()` returns a floor only OFF Plus (`:54-56`). The tier that clamps never reaches the call. So the under-reservation of Safe-to-Spend the entry describes was not reachable. The fix still stands: that is a property of one caller's gate, not of this function, and the next caller inherits the trap rather than the exemption. **`listPayEventsBetween` HAS NO BROWSING CALLER**, the entry's third checklist item, confirmed by grepping every `.ts`/`.tsx`: the export, the import in `safe_to_spend_service.ts:19`, and the single call at `:218`. **LINE CITES CORRECTED by reading:** `listFullLedgerBetween` is at `transactions_repo.ts:798` not `:788`, and `forecastContributions` at `safe_to_spend_service.ts:184` not `:183`. | 2026-09-17 |
| GAP-121 | DONE | 2ac2f3a | worktree-gap-wave-19 | **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** the change is three lines in one file and touches no code, confirmed by `git diff --stat`, which reports `docs/04-features/04-income.md` alone at 3 insertions and 3 deletions. `jest lib/income` 8 suites / 133 tests PASS and `npx tsc --noEmit` exit 0 were both run on a tree carrying this commit. **THE ENTRY SCOPED RULES 4 AND 9; RULE 11 WENT WITH THEM, AND LEAVING IT WOULD HAVE BEEN THE SAME DEFECT ONE RULE OVER.** Rule 11 said a candidate event is a matched pay event when "its amount is within ±30% of `averageAmount`", which reads as the credit's amount, while `maybeEmitPayday` calls `collapsePaydays(inScope)` and then applies the band to the day's combined total (`mobile/lib/income/income_service.ts:695`, read to confirm). The entry's own Evidence names rule 11 as changed by GAP-110 without any doc edit, and then its checklist omits it. **RULE 11'S IRREGULAR FLOOR IS DOCUMENTED AS THE EXCEPTION IT GENUINELY IS, which a doc-only rewrite would have got wrong by assuming the collapse applies everywhere:** `income_service.ts:702-711` keeps the ₱1,000.00 floor PER CREDIT, because there is no average to test against on that path and summing sub-floor credits until they clear the floor would manufacture paydays out of exactly the small credits the floor exists to exclude; the payday it produces still carries the day's total. **RULE 4 GAINED THE THREE LOAD-BEARING PARTS THAT WERE NOWHERE IN THE SPEC AND SURVIVED ONLY AS COMMENTS:** the collapse of one local date into a single banded candidate, a key's first payday seeding its group whole before any fallback can apply (seeding from a half would set the running median to half a payday and put every later whole payday out of band, which is the original bug reintroduced through the escape hatch), and the out-of-band-day fallback that re-tests that day credit by credit so a ₱50,000 13th month cannot evict the ₱18,500 salary it landed beside, which is rule 8's promise. "Largest recurring group" is now also stated as counting PAYDAYS rather than credits, or a stream paid in halves outranks a genuinely more frequent one on deposit habit alone. **RULE 6'S KINSENAS ROW WAS CONSIDERED AND DELIBERATELY LEFT ALONE.** Its wording is "credits landing alternately in the two pay windows", but its evidence thresholds already count matched WINDOWS ("3 consecutive matched windows", "4 of the last 5 expected windows"), and `tryKinsenas` scores windows too; once rule 11 says the unit is the payday, rule 6 inherits it. Changing it would have been churn, not correction. **THE ARCHIVED PLAN AT `docs/superpowers/plans/2026-08-02-mobile-control-m2.md` REPEATS THE OLD WORDING AND IS DELIBERATELY UNTOUCHED:** it records a plan that was executed, not the current spec, and editing it would falsify the history rather than fix a document anyone implements from. `grep -rn "running median" docs/04-features/` now returns rule 4 alone. **LINE CITES IN THE ENTRY HAD DRIFTED, corrected by reading:** `averageAmountFor` is at `income_math.ts:97` not `:131`, and `tryKinsenas` at `cadence_detector.ts:145` not `:159`. Rule 11 itself is at `04-income.md:101`. No code was changed, which is the entry's own second acceptance criterion. | 2026-09-17 |
| GAP-124 | DONE | a811029 | worktree-gap-wave-17 | **FILED AND CLOSED IN ONE PASS**, because the defect was found and fixed inside wave 17 before it had an entry; this row exists so the fix is not invisible to the campaign. **Orchestrator-run 2026-09-17:** `npx jest components/onboarding/__tests__/recovery_phrase --maxWorkers=1`, 1 suite / 36 tests PASS. **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS**, re-verified first-hand; that is the run carrying this fix. **THE FAILURE IT FIXES IS ON THE RECORD:** run `34443170128` on `59d813a` failed the `jest (3)` shard with `Expected pattern: not /share/`, on a branch that had not touched this screen. **THE RATE IS COMPUTED, NOT ESTIMATED.** `mobile/lib/crypto/recovery_phrase.ts:25` sets `WORD_COUNT = 12` and the bundled BIP39 list (`mobile/lib/crypto/wordlist.ts`) holds 2048 words including both `share` and `copy`, verified by reading the list rather than assuming it, so the false-failure probability is 1 - (2046/2048)^12 = **1.17%, about one run in 86**. **THIS RETRACTS AN EARLIER DIAGNOSIS.** Two local failures of this suite in wave 17 were attributed to memory contention and overlapping jest workers, and that was wrong; the suite was genuinely flaky for a reason having nothing to do with the machine. The contention warning still stands on its own evidence for other suites, but it was not what was happening here. **THE FIX NARROWS THE SCAN WITHOUT WEAKENING THE CLAIM:** the assertion serialized the whole rendered step with `screen.toJSON()`, generated words included, so it was pattern-matching random data against the two words it searched for. The `phrase-word-list` subtree (`mobile/components/onboarding/phrase_display.tsx:165`) is now excluded through a `JSON.stringify` replacer and everything else on the step is still scanned in full, since an affordance cannot live inside the generated words. A second assertion, `expect(tree).not.toMatch(/phrase-word-/)`, pins the exclusion itself, so renaming that `testID` fails loudly instead of quietly turning the replacer into a no-op and restoring the coin flip. `queryByTestId("phrase-share-button")` and the platform-share-sheet check are untouched; they were never the flaky part. | 2026-09-17 |
| GAP-122 | DONE | 7dc0c57 | worktree-gap-wave-17 | **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand; verification of record, because wave 17's full local sweep never completed (two attempts killed by the OS for low memory). **Orchestrator-run scoped suites, 2026-09-17:** `components/gates more_hub subscriptions_screen` 3 suites / 46 tests PASS and `lib/income lib/recurring lib/db/repos` 30 suites / 698 tests PASS; `npx tsc --noEmit` exit 0. **THREE THINGS HAD TO MOVE, NOT ONE, AND EACH ALONE LEAVES RULE 19 STILL BROKEN.** The Free skip goes (reversing GAP-118's recurring half within the same wave, an hour after it landed); the 800-day window is exempted through `listFullLedgerBetween` (`lib/recurring/recurring_service.ts:164`, window at `:148`, `LEDGER_WINDOW_DAYS = 800` at `:63`); and the More-tab row navigates on Free. **THE WINDOW EXEMPTION IS NOT OPTIONAL, and it is the piece most easily dropped as a nice-to-have.** A count computed from a floor-clamped 90 days understates the answer, and three instances of an annual charge span about two years, so the subscription a user most wants flagged could never be counted at all and the teaser would lie in the one direction that costs a conversion, "we found nothing". **`PlusGate` GAINED AN OPT-IN `lockedPress` PROP DEFAULTING TO `"intercept"`** (`components/gates/plus_gate.tsx:91`, passthrough branch at `:131`), so every existing gated call site behaves exactly as before and exactly ONE row opts into `"passthrough"` (`app/(tabs)/more/index.tsx:309`). Verified safe by reading the screen, not by trusting the prop: the data hiding was never the row's job, because `app/(tabs)/more/subscriptions.tsx:124-125` early-returns `<LockedPreview count={patterns?.length ?? 0} />` before any pattern reaches `PatternCard`, `LockedInHeader` or `monthlyLockedIn`. Rule 19 puts the gate at the entitlements call site, and that is now where it is. **`decayStalePatterns` NOW RUNS ON FREE, AND THE WINDOW EXEMPTION IS WHAT MAKES THAT CORRECT, NOT THE GUARD'S REMOVAL.** GAP-118 deliberately skipped decay so a Free period could not forget what a Plus period found, which would be the tier deleting data. With both tiers now detecting from the same 800 days, decay judges the same evidence either way, so a pattern can only be forgotten for having genuinely stopped. This was re-derived rather than assumed, as the entry's checklist demanded. Merchants, amounts and the total stay Plus-only, asserted by test on the screen itself. **ONE PROMISE IN RULE 19 IS NOT CLOSED BY THIS COMMIT AND CANNOT BE TODAY:** "on upgrade, patterns computed from the full retained history appear immediately". `MVP_TIER` is a `const` (`lib/entitlements.ts:13`) whose only mutator is `__setTierForTests`, so there is no runtime tier switch to hang "immediately" on; the promise is unimplementable and unfalsifiable until one exists. Pre-existing, out of scope here, and owed its own entry when tier switching becomes real. | 2026-09-17 |
| GAP-118 | DONE | 5ccfe66 | worktree-gap-wave-17 | **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand; verification of record, because wave 17's full local sweep never completed (two attempts killed by the OS for low memory). **Orchestrator-run scoped suites, 2026-09-17:** `lib/income lib/recurring lib/db/repos` 30 suites / 698 tests PASS; `npx tsc --noEmit` exit 0. **THIS WAS A SPLIT DECISION AND ONE HALF WAS REVERSED INSIDE THE SAME WAVE, BY GAP-122 (`7dc0c57`), ABOUT AN HOUR LATER. THE ORCHESTRATOR'S OWN FRAMING CAUSED THE REVERSAL, and that is the durable lesson here.** The recurring half ("do not run the pass at all on Free") was put to the owner as "the results are invisible on Free anyway", which is true of the shipped build and FALSE of the spec: reports rule 19 (`docs/04-features/10-reports.md:98`) promises Free "the count of detected patterns only", and `LockedPreview({ count })` was already written and already wired at its call site. Asked again once the conflict surfaced, the owner chose to honour rule 19, so `5ccfe66`'s Free skip in `recurring_ledger_subscriber` no longer exists. A question framed from the code rather than the doc produced a decision the doc had already settled. **THE INCOME HALF STANDS AND IS THE DURABLE PART OF THIS COMMIT:** cadence detection reads its full 130 days regardless of tier, through `listFullLedgerBetween` (`lib/income/income_service.ts:200`, window built at `:171`, `LEDGER_WINDOW_DAYS = 130` at `:111`). Not sent to the owner, because GAP-105 (`sumSpend`) and GAP-111 (`listFullLedger`) already settled the same computation-versus-browsing question, and unlike recurring detection income cadence is tier-gated NOWHERE, so its sample must not move with the tier. **THE ENTRY OVERSTATED THE INCOME DAMAGE, AND THE REAL DAMAGE LANDS IN THE SAME PLACE ANYWAY.** The entry predicted a monthly-paid Free user might never reach a confirmed cadence at all; measured, cadence still confirms on Free. What actually moved is `averageAmount`, which medians the last four paydays and on Free was handed three: the figure shifted 5% in the fixture, and it feeds `monthlyEquivalent`, `baseFor` and therefore every percent-of-income Limit, which is the blast radius GAP-117 describes. A narrower defect with the same reach. **`listFullLedgerBetween` IS A SIBLING OF `sumSpend` AND `listFullLedger`, NEVER A FLAG ON `TxFilter`** (`lib/db/repos/transactions_repo.ts:798`). A flag would hand every future caller a switch that turns the browsing gate off, and `hooks/queries/use_transactions.ts` passes a caller-supplied filter straight through to the repository. `listFullLedger` itself is deliberately UNBOUNDED, so reaching for it here would have widened detection from 130 days to the entire ledger, which nobody decided. The 90-day floor stays on `listTransactions` for every browsing caller, per the entry's Do-not. **LINE CITES IN THE ENTRY HAVE DRIFTED, corrected by reading the files on 2026-09-17:** `LEDGER_WINDOW_DAYS` is at `income_service.ts:111` and `recurring_service.ts:63`, not `:98` and `:47`, and `listFullLedgerBetween` is defined at `transactions_repo.ts:798`, not `:788`. | 2026-09-17 |
| GAP-082 | DONE | 847ca99 | worktree-gap-wave-17 | **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand. This is the verification of record because wave 17's full local sweep never completed: two attempts were killed by the OS for low memory. **Orchestrator-run scoped suites, 2026-09-17:** `components/loans/__tests__/loan_form lib/db/repos/__tests__/loans_repo lib/db/__tests__/migrations lib/db/__tests__/schema` 5 suites / 340 tests PASS; `npx tsc --noEmit` exit 0. **THE ENTRY'S PROPOSED FIX WAS REJECTED BEFORE DISPATCH AND THE OWNER'S SECOND BRANCH SHIPPED.** "Hide the principal field for flat and drop it from `canSave`" would have deleted a documented input and permanently foreclosed doc open question 3: `docs/04-features/06-loans.md:43` defines a flat loan as TWO figures ("borrowed P5,000.00, repay P6,000.00"), the worked example at `:82` enters both, and the implied-cost figure open question 3 asks about exists only as total repayable minus amount borrowed. **WHAT SHIPPED IS A MIGRATION, NOT THE FORM TWEAK THIS WAS FILED AS** (re-graded S/D2 to M/D3 in `e7fca86` before dispatch): `lib/db/migrations/020_loan_amount_borrowed.sql:53-54` adds `ALTER TABLE loans ADD COLUMN amount_borrowed INTEGER CHECK (amount_borrowed IS NULL OR amount_borrowed > 0)`, registered at `lib/db/migrations.ts:21`, surfaced as `amountBorrowed: Centavos or null` (`types/domain.ts:379`) and mapped in the form at `loan_form.tsx:192` (edit seed) and `:661` (submit). `principal` is untouched and still holds `installment * count` for flat, because the balance formula at `06-loans.md:89` counts outstanding down from total repayable; putting the borrowed P5,000 there would report a P6,000 utang settled with P1,000 still owed. **NO "principal" SUBSTRING IS IN THE COLUMN NAME, deliberately:** `principal` is a balance basis the app counts down, `amount_borrowed` is an EVENT, cash that changed hands once and never moves, and the two would otherwise be read past each other. **DELIBERATELY NOT BACKFILLED.** The borrowed figure cannot be recovered from the total, because loans rule 4 forbids deriving a rate for a flat loan and a rate is the only thing that could bridge them, so copying `principal` in would assert the user borrowed the total repayable, which is false for every flat loan carrying any add-on; a confident wrong number is worse than an admitted unknown. NULL also means "no separate figure" for amortized and free-form loans, where `principal` already IS the amount borrowed. **NO `CHECK (amount_borrowed <= principal)`:** repaying less than was borrowed is unusual, not impossible, and a constraint refusing a real arrangement costs the user their record of it. The amount stays REQUIRED for every new loan, unchanged from before, because `:40` asks for it at step 2 before the schedule type is chosen at step 3; it is exempt ONLY when editing a flat row the app itself never recorded. Keeping the requirement there would be actively harmful: the only figure in front of the user is the total repayable, so the requirement would push them to type it and manufacture exactly the falsehood the nullable column exists to prevent. Switching such a loan to amortized re-requires it. A legacy null row shows an empty box and a line saying nobody recorded the figure, NOT a P0.00 preview, because P0.00 is a claim that nothing was borrowed. The migration test is proven against a real version-19 database holding a flat loan, asserting the column is ABSENT first so a fresh database cannot pass it, and that exactly [20] is applied. | 2026-09-17 |
...
| GAP-112 | DONE | 7b5cc28 | worktree-gap-wave-16 | jest lib/bills components/bills bills_screen 9 suites 181 PASS (was 171; +10 tests), re-run independently by the orchestrator. FULL LOCAL SWEEP: 271 suites / 4,746 tests PASS across all three chunks; `npx tsc --noEmit` exit 0. Reproduced red first: before the fix, `A WEEKLY BILL'S WINDOW IS CLAMPED TO HALF ITS PERIOD` received a candidate `{merchant: MERALCO TOO EARLY, score: 0.95}` where `[]` was expected. **THE ENTRY'S PRESCRIPTION WAS WRONG AND WAS NOT FOLLOWED.** Its checklist said "clamp both edges by half the period"; rule 26 (`docs/04-features/07-bills.md:126`) keeps an overdue cycle open 30 days "superseding rule 15's close", so clamping the close would have repealed rule 26 for every bill with a period under 60 days -- monthly included (half of 30.33 floors to 15, so the close would never reach 30), which is the late-Meralco case rule 26 exists for. Shipped instead: the OPEN edge is clamped in both states, the close is clamped only while the cycle is not yet overdue. Two further readings had to be settled and are documented in code at `rule_summary.ts:169-197`: (1) PER EDGE, NOT PER TOTAL WIDTH -- as a total-width cap the rule contradicts its own first clause, since 7+15=22 already exceeds half a monthly period and no bill could ever receive the figures the same sentence promises; per-edge makes the monthly 7/15 exactly satisfiable; (2) HALF IS FLOORED, because rule 15 says never *wider* than half, and two adjacent 4-day edges would overlap on two days and re-offer one payment to both cycles. `DueRule` has no period field, so `periodDays` (`due_rules.ts:264`) derives it from the rule's own occurrences across 52 weeks -- AVERAGED, not the gap to the next cycle, because the forward gap is 28 days from a February due date and would narrow every monthly window each February and change the detail screen's sentence month to month; a constant table per `kind` was rejected as a second model of the calendar free to drift from `due_rules.ts`. **DISCLOSED, NOT PAPERED OVER: the close-side clamp is behaviourally inert.** `bills_service.ts:359` caps the query at `Math.min(to, now + DAY_MS)` and a live cycle has `dueDate >= today`, so the close is clipped by the clock regardless; the OPEN clamp is the entire behavioural fix, and the close-side coverage therefore lives in `rule_summary.test.ts` rather than in a matcher test. The scoring band at `bills_service.ts:315` is deliberately left unclamped (it ranks what the window already admitted; following rule 26's 30 days would give a payment 29 days late the same +0.1 as one 4 days late). `autoMatchRule.dateWindowDays` (`:570`) left alone: written at two sites, read by none, and freezing a cycle-dependent clamped number into stored rule data would go stale the moment the due rule is edited. **RESIDUAL FOR THE OWNER, BY DESIGN NOT BY OVERSIGHT:** an overdue weekly cycle's window is 30 days by rule 26 and so still spans later cycles; rule 17 plus `claimedTransactionIds()` prevent double settlement and the gap band favours the nearer cycle, but whether rule 26 should itself be clamped for short-period bills is a spec question, not a code one. | 2026-09-09 |
| GAP-114 | DONE | 7fc30b0 | worktree-gap-wave-16 | npx jest use_set_provider_pause privacy_screen modules bootstrap provider_picker providers_step --maxWorkers=1: 9 suites, 176/176 PASS, re-run by the orchestrator. npx tsc --noEmit exits 0 across the whole project. KOTLIN RUN, NOT LEFT UNRUN, AND INDEPENDENTLY RECOUNTED: expo prebuild --no-install then ./gradlew :notification_listener:testDebugUnitTest, BUILD SUCCESSFUL; the orchestrator summed the worktree's JUnit XML itself -- 11 classes, tests=170 skipped=0 failures=0 errors=0 -- and confirmed the new cases present by name in the XML ('an allowlist that could not be sealed reports failure and leaves the stored one alone', 'a resume that could not be sealed reports failure and leaves the block standing', 'a deny-all that landed without its allowlist still reports success'). That is 163 + 7. REVERT WITNESS: flipping the seal-failure branch's 'return false' to 'return true' produced BUILD FAILED with exactly the three tests that assert it failing, then restored byte-identical. THE BRIEF'S PROPOSED CONTRACT WAS WRONG AND THE AGENT REFUSED IT. A boolean return is droppable, and two of the three callers already had a catch or try that would have swallowed it silently -- the orchestrator had flagged that trap for providers.tsx specifically; the agent generalized it and chose a REJECTION instead, which is why the onboarding caller needed no behavioural change at all: its existing .catch now fires and does what its comment already claimed. ACCEPTANCE CRITERION DELIBERATELY NARROWED, AND THE NARROWING IS CORRECT: a deny-all that lands without its allowlist still reports success, because the flag is plaintext, shouldCapture consults it before ever opening the filter (verified by the orchestrator at CapturePrefs.kt:260), and setProviderFilter is the flag's only writer whose only path back to false is the full write that replaces the filter in the same commit -- so the stale allowlist is unreachable and reporting failure would instead make the caller drop paused_provider_packages for a block the device really is applying, undoing half of GAP-103. Failure copy is screen-specific and replaces the app-wide card under the same dedupeKey rather than using meta.errorToast:false, so the worst case is the generic card, not silence. NOT VERIFIED: no on-device run -- the seal failure is simulated with a FakeKeyVault holding no prefs KEK, and a real Keystore refusing the key was not reproduced; the AsyncFunction DSL body is still covered only by module_wiring's text assertion, so 'the rejection crosses the real bridge into JS' is proven in two halves (Kotlin throws the code, JS maps it) but never end to end -- the same gap GAP-103's stage-3 device check closed for two-argument marshalling. FLAGGED FOR FILING: an onboarding provider selection that fails to seal is still lost silently, because resyncProviderFilter only knows paused_provider_packages and providers.tsx never writes it -- pre-existing, GAP-092 shaped, not filed yet | 2026-09-09 |
| GAP-108 | DONE | ba02d7b | worktree-gap-wave-16 | npx jest transactions lib/db/repos lib/review --maxWorkers=2: 27 suites, 734/734 PASS, re-run by the orchestrator and matching the agent's own figures exactly. npx tsc --noEmit exits 0. THE ENTRY AND THE ORCHESTRATOR'S BRIEF WERE BOTH WRONG IN THREE PLACES, all found by the agent and all verified first-hand before acceptance. (1) THE ORCHESTRATOR'S 'MISSED FOURTH FOREIGN KEY' WAS A FALSE ALARM, now struck through in the checklist above: deleteTransaction already NULLs wallets.drift_dismissed_transaction_id as the first statement of its own SQL transaction and its doc comment already said so; the orchestrator had grepped the signature and never read the body. (2) 'UNLINK FIRST' IS A DEAD END: unlinkTransfer DISSOLVES AND NEVER DELETES ('DISSOLVED, NOT DELETED' in its own header), so both legs stay NOT NULL references and the delete would still fail on a foreign key naming a link the user was just told they had broken -- the dissolved row itself has to go. Anyone working from the entry alone would have shipped that bug. (3) THE DEFECT NEITHER THE ENTRY NOR THE BRIEF SAW: the delete undid itself at the next launch. listUnprocessedRawCaptures selects captures referenced by neither a transaction nor a queue row, so a deliberately deleted notification-sourced row was indistinguishable from one stranded by a crash and startIngest re-committed it under a new id -- 'delete' was a lie until the 30-day TTL. Fixed with the same resolved marker mergeDuplicate already wrote, extracted to lib/review/capture_marker.ts. ORCHESTRATOR VERIFIED THE MARKER IS DURABLE: purgeExpired deletes only rows WHERE resolved_at IS NULL, and the marker is resolved at creation, so it is never purged and outlives the capture. FK decisions: loan_payments and bill_payments RELEASED (rows deleted; both deletePayment and deleteBillPayment had zero production callers, so refusing would have been a permanent dead end, and deleting a bill payment reopens its cycle); active transfer_links REFUSED with a message naming the counterpart wallet; dissolved links deleted under a SQL-guarded status check; drift dismissal already handled. The repo primitive is untouched and transactions_repo.test.ts:1663's raw /FOREIGN KEY/i rejection still passes, so the merge path is unaffected. NOT VERIFIED: nothing run on device, the delete button has never been tapped on the A54; the possible-duplicate marker was checked against listOpen, countOpen, listOpenPage, countOpenByKind, purgeExpired and undoResolution but not against every future consumer of resolved rows. PRE-EXISTING FLAKE RECORDED, NOT CAUSED HERE: two 'filters compose (AND)' tests in app/__tests__/transactions_screen.test.tsx exceed the 30 s timeout under 4 workers and pass alone at 24.4 s and 24.8 s, so worker contention tips them over a ceiling they were already close to | 2026-09-09 |
| GAP-033 | DONE | b065514 | worktree-gap-wave-16 | npx jest lib/loans user_rules_repo: 5 suites, 130/130 PASS, re-run by the orchestrator independently of the agent's own run (agent also reported lib/loans + lib/db/repos 23 suites 608 tests, and lib/review + lib/ingest 15 suites 487 tests). npx tsc --noEmit exits 0. THE FILED ACCEPTANCE CRITERION WAS WRONG AND WAS NOT IMPLEMENTED -- see the corrected criterion below; rule 9 forbids a taught rule from auto-matching, and the orchestrator verified 06-loans.md:96, :147 and bills_service.ts:11-29 first-hand rather than taking the agent's word. THE ORCHESTRATOR'S BRIEF REPEATED THE SAME ERROR, a fifth consecutive wave of defective briefs. Weight sized at 0.5 because rule 8 orders (b) above (c)=0.4 and 0.5 is the smallest round number that does, and because it alone clears CANDIDATE_FLOOR=0.35; the 0..1.5 scale is deliberately NOT renormalized, since dividing through would move every existing score relative to the floor. NO auto-match threshold exists in lib/loans to size against -- the only ladder in the repo is bills' consecutive-confirmation count. Rules are keyed on the provider's trail (merchant, falling back to counterparty for receive templates), deduped per loan per trail INCLUDING disabled rules so a switched-off rule is not resurrected, and taught from both the review card and the loan detail sheet but never from a manual payment, whose merchant is the user's own name for the counterparty. Decode skips rather than throws on an unknown kind or a blank loanId, verified by test. ONE DEFECT FOUND IN REVIEW AND CORRECTED: the new resolution parameter on closeLoanMatchesFor is inert -- review_queue_repo.resolve opens with 'void resolution' and its doc says the value is accepted for the pinned contract signature but not persisted -- so the comment was rewritten to say so outright, since as written it implied a void card is recorded differently from a confirmed one. NOT VERIFIED: nothing run on device; the stored pattern is the raw merchant string, so a provider that appends a per-transaction reference will not re-fire next month; the settings-list rendering of the new kind was not audited; rule 10's 'stop suggesting this merchant' prompt is still absent and needs a per-merchant-per-loan rejection count nothing stores | 2026-09-09 |
| GAP-115 | DONE | 829963a | worktree-gap-wave-16 | npx jest loan_card: 13/13 PASS. The card had NO progress test before this, so both new ones were proved against the code they guard rather than assumed: the adjustment case fails on the old principal-minus-outstanding formula with 'Expected: 33, Received: 17', and the amortized case fails on the paidTotal substitution the entry warns about with 'Expected: 50, Received: 55'. npx tsc --noEmit exits 0. The entry's line cite (loan_card.tsx:105) was ACCURATE, unlike its neighbours in this wave. principalApplied(loan.schedule ?? null, status.paidTotal) leaves flat and free-form loans unmoved because for those every peso is principal. The now-unused destructured outstanding was dropped; every remaining read already went through status | 2026-09-09 |
...
4. **GAP-075, undo scope: RULE 10 WINS.** A queue action never deletes a committed transaction.
   ...
   filed before GAP-075 can be closed.** **DEBT PAID 2026-09-10.** The ledger delete was filed as GAP-108 and shipped in `ba02d7b`: transaction detail now carries a delete behind a confirmation, so rule 9's "remains editable in the ledger indefinitely" is an affordance the user can reach rather than a promise with no code behind it. GAP-075's PARTIAL therefore stands on a real remedy now. Two things GAP-108 turned up that qualify what was written here: a matched or transfer-linked row is no longer undeletable (loan and bill payments are released, only an ACTIVE transfer link refuses), and a deleted notification-sourced row used to be re-committed by the next ingest sweep, which would have made "just delete it in the ledger" quietly false until that was fixed.
...
- **FOUR OWNER DECISIONS WERE TAKEN, unblocking work that had been stalled for six waves.** They
  ...
  **GAP-075's answer carries a debt**: rule 9's remedy says a committed result "remains editable
  ...
  closed.** **DEBT PAID 2026-09-10.** The ledger delete was filed as GAP-108 and shipped in `ba02d7b`: transaction detail now carries a delete behind a confirmation, so rule 9's "remains editable in the ledger indefinitely" is an affordance the user can reach rather than a promise with no code behind it. GAP-075's PARTIAL therefore stands on a real remedy now. Two things GAP-108 turned up that qualify what was written here: a matched or transfer-linked row is no longer undeletable (loan and bill payments are released, only an ACTIVE transfer link refuses), and a deleted notification-sourced row used to be re-committed by the next ingest sweep, which would have made "just delete it in the ledger" quietly false until that was fixed.
...
> **REMEDIATION: DONE** (2026-09-17) - commit cdfc851, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **NO USER-VISIBLE CHANGE AND NO BEHAVIOUR CHANGE**, so there is nothing new to pin with a test; `jest archive_wallet_sheet wallets` 14 suites / 317 tests PASS on the tree carrying it. **THE OWNER'S FIRST ANSWER WAS REVERSED THE SAME DAY, AND THE ORCHESTRATOR'S FRAMING CAUSED IT. THIS IS THE SECOND TIME IN THIS CAMPAIGN** (GAP-118 and GAP-122 were the first). The question was put as "the wallet sheet says Delete but the code archives", which is true of that one file and blind to the four Plan tabs beside it; the owner answered "relabel to Archive". Reading the siblings afterwards turned up `mobile/components/plan/archived_section.tsx:10-17`, which records the owner's ruling of 2026-08-28 verbatim: "replace the misleading button text from archive to 'delete'". Every Plan entity follows it, limits included (`app/(tabs)/plan/limits/[id].tsx` renders "Delete this limit?" and `confirmLabel="Delete"` over `archive.mutateAsync`), because every one of them is soft-deleted and restorable, so the app can use the word people actually reach for and still keep the promise behind it. Relabelling wallets alone would have given one behaviour two vocabularies. Re-asked with that evidence, the owner kept the label. **READ THE SIBLING SURFACES BEFORE PUTTING A VOCABULARY QUESTION TO THE OWNER.** **WHAT WAS ACTUALLY FALSE WAS THE FILE'S OWN HEADER,** not the button: `mobile/components/wallets/archive_wallet_sheet.tsx:12-15` asserted "DELETE IS NOT ON OFFER, ANYWHERE" while the confirm button in the same file says "Delete wallet". The header predates the 2026-08-28 ruling. It now states the split it lives under and points at the shared component that carries it for the Plan tabs. **THE DOC HAD THE LARGER PROBLEM: IT SPECIFIED A HARD DELETE THE APP HAS NEVER HAD.** `docs/04-features/02-wallets.md` carried a separate "Flow: delete a Wallet" with delete-immediately-when-empty and a mandatory reassign-or-archive choice otherwise. There is no `deleteWallet` in `wallets_repo`, the schema's `NO ACTION` foreign key on `transactions.wallet_id` blocks the statement outright, and invariant 4 forbids the orphan it would create, so the two flows the doc described are ONE flow. The delete section now says so and stops duplicating the archive section beside it, the move is documented as OPTIONAL because that is what it is, and rule 18 and the Archived-section row follow the same correction. **REJECTED, AND BOTH DELIBERATELY:** building real delete-with-reassign, which would have had to make the move MANDATORY and so turn the choice the sheet defaults away from into a requirement; and relabelling the whole app back to Archive, which would reverse a ruling the owner made from a device report.
...
> **OWNER DECISION (2026-09-17):** **KEEP THE "Delete" LABEL; FIX THE STALE HEADER AND THE DOC. NO USER-VISIBLE CHANGE.** `docs/04-features/02-wallets.md`'s delete-with-reassign rule is amended to describe what the app actually does: a SOFT delete that keeps every Transaction, with an OPTIONAL move to another wallet, restorable from the Wallets tab. The file header at `mobile/components/wallets/archive_wallet_sheet.tsx:12-15` ("DELETE IS NOT ON OFFER, ANYWHERE") is rewritten, because it is the only statement in this entry that is actually false today. **THIS REVERSES A FIRST ANSWER GIVEN THE SAME DAY, AND THE ORCHESTRATOR'S FRAMING CAUSED IT.** The question was first put as "the wallet sheet says Delete but the code archives", which is true of that one file and blind to the rest of the app. The owner had ALREADY ruled on 2026-08-28, recorded at `mobile/components/plan/archived_section.tsx:10-17`: "replace the misleading button text from archive to 'delete'". Every Plan tab follows it, limits included (`app/(tabs)/plan/limits/[id].tsx:222-223` renders "Delete this limit?" and `confirmLabel="Delete"` over `archive.mutateAsync`), because every Plan entity is soft-deleted and restorable, so the app can use the word people actually mean and still keep the promise behind it. **REJECTED: relabelling wallets alone to Archive**, which would have given one behaviour two vocabularies and diverged from four tabs; and **relabelling the whole app back to Archive**, which would reverse a ruling the owner made from a device report. **THIS IS THE SECOND TIME IN THIS CAMPAIGN THAT A QUESTION FRAMED FROM ONE FILE RATHER THAN THE SPEC OR THE APP PRODUCED A WRONG ANSWER** (see GAP-118 and GAP-122). Read the sibling surfaces before putting a vocabulary question to the owner.
...
> **REMEDIATION: DONE** (2026-09-18) - commit ff52b1b, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS**, and this entry required no new code, so that run is the gate for the doc half it was waiting on. **CLOSED BY EVIDENCE, NOT BY NEW WORK. EVERY PART OF THIS ENTRY WAS ALREADY SATISFIED AND IT HAS BEEN SITTING IN THE QUEUE AS A BLOCKER ON GAP-045.** Verified by reading all four, this session. **(1) THE OWNER DECIDED ON 2026-08-20** and the ruling lives in the code that implements it: `mobile/components/plan/limits_panel.tsx` carries it verbatim, "The owner's ruling: derived limits are free", with the reason that counting the derived rows would take a Free user from one Limit to gated the instant onboarding finished, over three rows they never asked for. **(2) THE FREE CAP ALREADY COUNTS SOURCE LIMITS ONLY.** `canCreateLimit` has exactly ONE non-test call site, `limits_panel.tsx:131`, and the count it is handed is filtered to `status.limit.derivedFrom === null`. Confirmed by grepping every caller. **(3) THE TEST THE CHECKLIST ASKS FOR EXISTS:** `mobile/app/__tests__/limit_routes.test.tsx:327`, "the derived limits are marked, so the free tier's cap ignores them", which saves one Limit and asserts one row with a null `derivedFrom` beside three pointing at it, across all four scopes. `lib/__tests__/entitlements.test.ts` covers `canCreateLimit` itself. **(4) THE DOC HALF SHIPPED IN GAP-045 (`ff52b1b`), WHICH THIS ENTRY ITSELF NOMINATED** ("Doc update is done by GAP-045"): `docs/04-features/03-limits.md`'s create flow now states that entering one Limit creates three derived siblings and that the Free cap counts source Limits only. **ONE CHECKLIST ITEM IS UNSATISFIABLE AND THAT IS CORRECT RATHER THAN OUTSTANDING.** It asks for the source-only count to be passed to `canCreateLimit` "in `limits_panel.tsx:131` AND `app/(onboarding)/first_limit.tsx`". The onboarding screen has no entitlement check at all, deliberately, and says so at its own head; there is no call there to fix. **THE ENTRY'S OTHER LOCATION CITES ARE MISLEADING IN THE SAME WAY:** it lists `app/(tabs)/plan/limits/new.tsx:44` and `first_limit.tsx:32` alongside the real gate, but neither calls `canCreateLimit`; both lines import `derivedLimitsFrom`. **THE ENTRY'S "WHY IT MATTERS" STILL STANDS AS WRITTEN AND IS NOW HARMLESS:** it is inert today because `MVP_TIER` is `plus`, and at flip time a Free user would have held four active Limits against a cap of one. That is exactly the case the shipped filter prevents.
...
> **REMEDIATION: DONE** (2026-09-18) - commit 3a0b21c, branch worktree-gap-wave-21. Verification: **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run:** `jest app_plugin services/__tests__/api` 2 suites / 19 tests PASS, `npx tsc --noEmit` exit 0. **THE NEW TEST WAS RUN AGAINST THE UNFIXED CONFIG FIRST AND OBSERVED TO FAIL.** The re-apply also asserts that GAP-093's `allowBackup: false` survived the revert, since both flags live in `app.json` and reverting one file would silently have taken the other with it. **THE DECIDING FACT IS THAT NOTHING EVER USED IT, WHICH THE ENTRY ESTABLISHED AND WHICH RE-VERIFICATION CONFIRMED:** `expo-updates` is a dependency and `updates.url` is set, but no `Updates.*` call exists anywhere in the app, and `app/_layout.tsx` had ALREADY declined to add the first one, deferring to this entry by name ("OTA policy belongs to GAP-028; until that is settled, pointing at the store is honest and costs nothing"). So the trade was a per-launch request carrying a persistent EAS client identifier, paid on every cold start, for a capability with no callers. **TWO REASONS, AND THE SECOND IS THE SERIOUS ONE.** First, unlisted egress: `docs/07` §4 presents its lifecycle table as the complete list of what leaves the device and has no row for the update client, and `mobile/services/device_info.ts` promises the app's own requests carry nothing that could correlate two requests to one handset. A library making its own request with a client id attached breaks both statements from outside every call site either document describes. Second, an OTA publish reaches installed devices WITHOUT a store submission, so it routes around `docs/07` §3.1 rule 6's re-declaration discipline BY CONSTRUCTION: a JS-only change could alter what a notification-reading app reads after the policy review that approved it. All three build variants share `version: 0.1.0` under `runtimeVersion.policy: "appVersion"`, so one publish to a channel would have reached every build on it. **REJECTED:** `checkAutomatically: "NEVER"` with the system left wired, because a future `Updates.*` call would silently re-open the egress with no config change to notice; and `ON_LOAD` with a lifecycle row and a full runbook, which keeps hotfixes and keeps the launch ping. `ON_ERROR_RECOVERY` was considered and is a false middle: it only fetches after a crash loop, so a healthy-but-wrong install never receives the fix, which is most of what a beta hotfix is for. **THE `url` IS DELIBERATELY LEFT IN PLACE** beside the flag: it is the EAS project binding, it does nothing while the system is off, and deleting it would only make a correct re-enable harder. **ONE CHECKLIST ITEM IS A NO-OP AND WAS SKIPPED:** the entry asks for `updates.fallbackToCacheTimeout: 0`, which is already the documented default. **DOCUMENTED WHERE SOMEONE WOULD ACTUALLY HIT IT, not only in the runbook:** `device_info.ts`'s header now names this flag as the thing keeping its promise true of the WHOLE app rather than merely of that file, and rule 6 now gives the flag as the reason the rule is enforceable. `docs/OTA_RUNBOOK.md` carries the re-enable checklist so the flag cannot be flipped back without the lifecycle row, both doc amendments and the listener-scope gate coming with it. **THE COST IS REAL AND WAS ACCEPTED:** a fix for testers now needs an EAS build and an install rather than a push.
...
> **OWNER DECISION (2026-09-18):** **TURN THE UPDATES SYSTEM OFF: `updates.enabled: false` IN `mobile/app.json`,** pinned by a test. Rejected: `checkAutomatically: "NEVER"` with the system left wired (a later `Updates.*` call would silently re-open the egress), and `ON_LOAD` with a full lifecycle row and runbook (keeps hotfixes, keeps the every-launch ping). **THE DECIDING FACT IS THAT NOTHING EVER USED IT.** `expo-updates` is a dependency and `updates.url` is set, but no `Updates.*` call exists anywhere in the app, and `app/_layout.tsx` had already declined to add the first one, deferring to this entry in as many words. So the choice was between paying a per-launch request with a persistent EAS client identifier for a capability nobody had used, and switching it off. **TWO REASONS RATHER THAN ONE, AND THE SECOND IS THE SERIOUS ONE.** First, it is unlisted egress: `docs/07` §4 presents its lifecycle table as the complete list of what leaves the device, and `mobile/services/device_info.ts` promises the app's own requests carry nothing correlatable, and neither statement survives a library making its own per-launch request with a client id attached. Second, an OTA publish reaches installed devices WITHOUT a store submission, so it routes around `docs/07` §3.1 rule 6's re-declaration discipline by construction: a JS-only change could alter what a notification-reading app reads AFTER the policy review that approved it. **THE ENTRY'S CHECKLIST CONTAINS ONE NO-OP:** it asks for `updates.fallbackToCacheTimeout: 0`, which is already the documented default. **THE `url` IS DELIBERATELY LEFT IN PLACE** beside the flag: it is the EAS project binding, it does nothing while the system is off, and removing it would only make a correct re-enable harder. `docs/OTA_RUNBOOK.md` carries the re-enable checklist so the flag cannot be flipped back without the lifecycle row, the two doc amendments and the listener-scope gate coming with it.
...
> **REMEDIATION: DONE** (2026-09-09) - commit b065514, branch worktree-gap-wave-16. Verification: npx jest lib/loans user_rules_repo: 5 suites, 130/130 PASS, re-run by the orchestrator independently of the agent's own run (agent also reported lib/loans + lib/db/repos 23 suites 608 tests, and lib/review + lib/ingest 15 suites 487 tests). npx tsc --noEmit exits 0. THE FILED ACCEPTANCE CRITERION WAS WRONG AND WAS NOT IMPLEMENTED -- see the corrected criterion below; rule 9 forbids a taught rule from auto-matching, and the orchestrator verified 06-loans.md:96, :147 and bills_service.ts:11-29 first-hand rather than taking the agent's word. THE ORCHESTRATOR'S BRIEF REPEATED THE SAME ERROR, a fifth consecutive wave of defective briefs. Weight sized at 0.5 because rule 8 orders (b) above (c)=0.4 and 0.5 is the smallest round number that does, and because it alone clears CANDIDATE_FLOOR=0.35; the 0..1.5 scale is deliberately NOT renormalized, since dividing through would move every existing score relative to the floor. NO auto-match threshold exists in lib/loans to size against -- the only ladder in the repo is bills' consecutive-confirmation count. Rules are keyed on the provider's trail (merchant, falling back to counterparty for receive templates), deduped per loan per trail INCLUDING disabled rules so a switched-off rule is not resurrected, and taught from both the review card and the loan detail sheet but never from a manual payment, whose merchant is the user's own name for the counterparty. Decode skips rather than throws on an unknown kind or a blank loanId, verified by test. ONE DEFECT FOUND IN REVIEW AND CORRECTED: the new resolution parameter on closeLoanMatchesFor is inert -- review_queue_repo.resolve opens with 'void resolution' and its doc says the value is accepted for the pinned contract signature but not persisted -- so the comment was rewritten to say so outright, since as written it implied a void card is recorded differently from a confirmed one. NOT VERIFIED: nothing run on device; the stored pattern is the raw merchant string, so a provider that appends a per-transaction reference will not re-fire next month; the settings-list rendering of the new kind was not audited; rule 10's 'stop suggesting this merchant' prompt is still absent and needs a per-merchant-per-loan rejection count nothing stores
...
**Acceptance criteria**
- [x] Second month's payment for the same counterparty scores higher, is ranked first, and carries a reason naming the user's own earlier confirmation. **CRITERION CORRECTED 2026-09-10 -- AS FILED IT CONTRADICTED THE SPEC.** It read "links without a card", which loans rule 9 forbids: `docs/04-features/06-loans.md:96` says "Only signal (a), with an unambiguous single-loan mapping, may auto-match. Every other combination produces a suggestion requiring confirmation", and signal (b) IS the UserRule this entry adds. The same doc's own acceptance line `:147` asks only that "the next matching payment scores higher"; `:145` is the separate provider-event auto-match, which is an ingest concern this entry's Do-not excludes. `mobile/lib/bills/bills_service.ts:11-29` already litigated the difference under the heading "BILLS AUTO-MATCH; LOANS DO NOT. That difference is deliberate", because a wrong loan match corrupts the balance AND pulls the row out of income cadence detection, and neither failure announces itself. The agent refused the criterion and argued it from these three sources; the orchestrator verified all three independently before accepting.
...
> **REMEDIATION: DONE** (2026-09-18) - commit 3700d30, branch worktree-gap-wave-22. Verification: **CI ON c29d38d, GREEN, read from the completed run:** mobile-ci 35305120125, 9 of 9 jobs success. server-ci did NOT fire, and that is correct rather than a miss: contrary to what the wave 21 handoff recorded, server-ci.yml IS path-filtered, on both push and pull_request through a YAML anchor covering server, docker-compose, the two workflow files and docs/07. mobile-ci still has NO gradle job, so nothing in this run gates a single line of Kotlin. Entry's diagnosis verified: lock_context.tsx:208 mapped ANY getKeyState rejection to status "locked" with generic copy, and no test covered the branch at all, so grep for the old message found zero assertions. THE ENTRY'S PRESCRIPTION WAS DELIBERATELY NOT FOLLOWED. It asked for RecoveryUnlockForm's wipe branch with a storage-specific message, but the recovery path is dead on exactly this failure: rewrapAfterInvalidation calls unwrapWithRecoveryPhrase, which reads recoveryWrap and recoverySalt out of the same SecureStore whose read just threw, and then writes the new device wrap back to it. Rendering that form would send a user for their paper copy to watch a second failure, under an explanation about a removed screen lock that is not true of their phone. Built instead: a "storage_error" status, a dedicated components/lock/storage_error_screen.tsx that leads with Try again, and retryKeyState on the context. The retry is an addition to the entry, and it is not a courtesy: the only other control on that screen destroys every transaction on the device, so the non-destructive route has to come first. Verified that the escape actually works on broken storage, which is what the whole entry turns on: wipeAndStartOver deletes the database BEFORE it touches the keys (lib/security/wipe.ts:72-88), so a wipeKeys that fails on the same fault still leaves the data gone and routes to the honest WipeIncompleteError copy. The double confirmation was extracted to components/lock/wipe_and_start_over.tsx now that it has two callers, with the sentence naming what is destroyed kept FIXED in the shared component and only the per-situation sentences passed in, so the two screens cannot drift. retryKeyState returns a boolean rather than reading statusRef after its await, because statusRef is assigned during render and a caller resuming from an await has not necessarily re-rendered. Tests: five new cases in lock_context (rejection goes to storage_error not locked, retry recovering to locked, retry recovering to needs_onboarding, a second failure setting copy, and retry never rejecting), two in app/__tests__/lock_screen.test.tsx, and a new components/lock/__tests__/storage_error_screen.test.tsx covering both confirmation steps, both cancels, the in-flight retry guard and the absence of any phrase field. 98 of 98 green across the five lock suites, plus lock_gate and root_layout re-run green. npx tsc --noEmit exit 0. NOT verified on device.
...
> **REMEDIATION: DONE** (2026-09-17) - commit 5863e30, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run:** `jest archive_wallet_sheet wallet_detail wallet_routes wallets_screen` 4 suites / 127 tests PASS, `npx tsc --noEmit` exit 0. **THREE OF THE FOUR NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL** (the fourth asserts that an unwired caller renders nothing, which is true before and after by design). **THE ENTRY'S CHECKLIST ASKED FOR TWO NEW REPOSITORY READS AND NEITHER WAS NEEDED.** It proposes adding `listGoalsForWallet` and `listLoansForWallet` "if absent in the repos". They are absent, and they stay absent: `useGoals` already excludes deleted goals and `useLoans` already excludes settled and archived ones, so the entry's own words "live" and "active" come for free from hooks the app already has, and the screen filters all three by wallet id. Two new wallet-scoped SQL reads would have been two more access paths to keep in step with the soft-delete rules those hooks already encode. **THE SHEET STAYS PRESENTATIONAL.** The screen gathers and passes `links` down, exactly as `transactionCount` and `otherWallets` already work, so the sheet does not mount three queries the moment it becomes visible. **NAMES RATHER THAN ROWS:** passing `Goal[]` and `Loan[]` would hand the sheet two domain shapes it reads one field from each, and naming them is the whole job (archive flow step 4: "the confirmation lists these links"). **THE PROP IS OPTIONAL AND ABSENT MEANS NOTHING IS LINKED,** so a caller that has not wired the queries keeps today's sheet rather than rendering an empty "nothing uses this wallet" claim it has no evidence for. **BOUND TO NOTHING NEW:** the copy asks the user to relink or accept the pause, per rule 21, and the sheet holds no state about it, so there is no second flag to drift. **`tsc` CAUGHT A REAL MISTAKE:** `useLoans` returns `LoanStatus[]` rather than `Loan[]`, the same nesting `useGoals` has, so `linkedWalletId` and `counterparty` are one level in. **A SECOND MISTAKE WAS CAUGHT BEFORE COMMIT AND IS WORTH RECORDING AS A PROCESS NOTE:** re-running an idempotent-looking edit script after a partial revert inserted the screen's `useMemo` a SECOND time, because the anchor line still matched. The screen was reverted and re-applied through a script that refuses when `walletLinks` is already present. An edit script that asserts one match per replacement does not protect against re-running it against a file that already has the replacement. **THE ENTRY'S LOCATION CITE POINTED AT THE FILE HEADER, NOT THE UI:** `archive_wallet_sheet.tsx:8-30` is the header comment block, and the entry's own C2 grading admits "the sheet body was not read in full", which is exactly right.
...
> **REMEDIATION: PARTIAL** (2026-09-18) - commit 9435f08, branch worktree-gap-wave-23. Verification: **CI ON 9435f08, ALL GREEN, read from the completed runs:** mobile-ci 35315046283, 9 of 9 jobs success; server-ci 35315046296 and 35315008962, both success. server-ci fired on this wave and did NOT fire on wave 22, which is the filter working rather than a fluke: it is path-filtered on docs/07-privacy-and-compliance.md among others, and GAP-061 changed that file. mobile-ci still has NO gradle job, so nothing in these runs gates a single line of Kotlin. PARTIAL BY DESIGN. OWNER DECISION 2026-09-18: ship signature verification now, defer staged rollout until there is a server to stage from. Rule 2 is met; rule 4 is not, and that half keeps this entry open. Built: mobile/lib/ingest/ruleset_signature.ts verifying an Ed25519 signature against a public key compiled into the app, called from services/parser_rules.ts after the size cap and BEFORE anything is parsed. THE SIGNATURE IS DETACHED, IN THE X-Ruleset-Signature RESPONSE HEADER, OVER THE RAW BODY, which is why there is no canonicalization code: signing a field inside the JSON would mean agreeing with the server on a byte-exact re-serialization of the rest, and key order, number formatting, escaping and whitespace are each a place for the two sides to disagree and for a verifier to be talked into checking something other than what it parsed. parser_rules.ts already asks axios for text and keeps the raw string, so the bytes verified are the bytes parsed. FAILS CLOSED, INCLUDING ON A MISSING HEADER, with no allowance for unsigned bundles while the server is being built, because an allowance like that is what would still be switched on the day it went live. The type and size guards are repeated at the call site rather than borrowed from decodeBody so verification genuinely runs first and an uncapped body is never hashed. WHY THE CLIENT SHIPS BEFORE THE SERVER, which is the reasoning behind the owner's choice: there is no server behind this endpoint yet, so nothing is verified in practice today, and that is exactly the point, since an install that goes out without verification accepts unsigned bundles forever and no later server change can reach it. A keypair was generated on 2026-09-18 with a sign-and-verify self-check; the public half is the constant in ruleset_signature.ts and the private half has NEVER been in this repository, was never printed, and was written outside the working tree for the owner to move into SOPS. The public key is a DEFAULTED PARAMETER rather than a closed-over constant, solely so the unit tests can exercise real Ed25519 with a keypair they generate, since producing a valid signature under the shipped key needs the private half; the one production call site passes two arguments and a test pins the default to the shipped constant so it cannot drift into an escape hatch. Rotation costs a store release and that is accepted rather than overlooked, since a second trusted key or a key-rotation bundle would each widen the surface this narrows. New dependency @noble/curves 2.4.0, same family as the @noble/ciphers and @noble/hashes already used, and already allowed by the jest transformIgnorePatterns. RESIDUAL, and what keeps this entry open: rule 4 has no rollout bucket, no rollout_percent in the bundle schema and no telemetry gate, because with nothing emitting that field the gate would ship untested against any real producer and be tuned blind; it is additive and belongs with the server. The only rollback remains local and read-time in parser_rulesets_repo, covering a corrupt row rather than a bad ruleset that parses cleanly and matches wrongly. Tests: lib/ingest/__tests__/ruleset_signature.test.ts 9 of 9 exercising real signing, including a well-formed bundle signed by a DIFFERENT key being refused, a one-byte body change breaking verification, and malformed signatures returning false rather than throwing; services/__tests__/parser_rules.test.ts 21 of 21 including four new, the load-bearing one being that a schema-valid higher-version bundle that fails verification is not installed and the device keeps the ruleset it had. npx tsc --noEmit exit 0. docs/03 section 11.2 rewritten for rule 2 and rule 4's deferral. NOT verified on device and NOT verified against any real server, because none exists.
...
> **OWNER DECISION (2026-09-18): SHIP SIGNATURE VERIFICATION NOW, DEFER STAGED ROLLOUT.** Rule 2 is
> built; rule 4 is not, and this entry stays open on that half alone. **Ed25519, detached, in the
> `X-Ruleset-Signature` response header, over the RAW body**, verified after the size cap and before
> anything is parsed. A signature field INSIDE the JSON was rejected: it would mean agreeing with
> the server on a byte-exact re-serialization of the rest, and key order, number formatting,
> escaping and whitespace are each a place for the two sides to disagree and for a verifier to be
> talked into checking something other than what it parsed.
>
> **WHY THE CLIENT SHIPS BEFORE THE SERVER, WHICH IS THE WHOLE REASONING.** There is no server
> behind this endpoint yet, so nothing is verified in practice today — and that is the argument
> FOR shipping now rather than against it. An install that goes out without verification accepts
> unsigned bundles forever, and no later server change can reach it; the build that ships before
> the server exists is the one that has to already know the key. **REJECTED: deferring both until
> the server exists**, which accepts that permanently-unverified-install window.
>
> **It fails closed, including when the header is simply absent.** There is deliberately no
> "unsigned bundles are allowed while the server is being built" allowance, because that allowance
> is what would still be switched on the day the server went live.
>
> **REJECTED: shipping rule 4 as well.** A rollout gate needs a server to stage FROM. With nothing
> emitting `rollout_percent`, the bucket and the gate would ship untested against any real producer
> and be tuned blind. Both are additive and belong with the server.
>
> **The keypair was generated on 2026-09-18** with a sign-and-verify self-check. The public half is
> the constant in `mobile/lib/ingest/ruleset_signature.ts`; **the private half has never been in
> this repository**, was never printed, and was written outside the working tree for the owner to
> move into SOPS. **Rotation costs a store release, and that is accepted**: a second trusted key, or
> a rotation bundle signed by the current key, would each remove that cost and each widen the very
> surface the check exists to narrow, so neither ships until there is a server to need one.

**Implementation checklist**
- [x] Owner chooses the integrity mechanism and where the signing key lives. **Ed25519; private key outside the repo, bound for SOPS.**
...
> **REMEDIATION: DONE** (2026-09-17) - commit ff52b1b, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **DOCS ONLY, WHICH IS THE ENTRY'S OWN SECOND ACCEPTANCE CRITERION:** nine files under `docs/`, 28 insertions and 24 deletions, and nothing under `mobile/` touched, confirmed by `git diff --stat` before the commit. No test reads any doc (the suites that mention `docs/04-features` cite rules in comments and call no `readFileSync`), so there is nothing here a test can pin. **FIVE OF THE ENTRY'S TEN CLUSTER CITES HAD DRIFTED, TWO OF THEM ONTO UNRELATED CODE:** `bills_service.ts:279-283` is `scoreCandidate`, not the keyword matching (that is `:297-298` for the match and `:378-379` for the exclusions); `09-v2-backlog.md:277` is about deferred notification ACTIONS, and the notifier claim is at `:279`; `01-onboarding.md:67` is about hardware verification, and the stale claim is at `:79`; and the wallet rules had moved AGAIN under this same wave's GAP-022 commit. Every pair was reopened. **CLUSTER 1, THE WALLET `type` ENUM:** dropped by migration `014_drop_wallet_type.sql` because nothing asked it and nothing read it. Five doc lines still offered the user a five-value picker. The two facts it stood in for are both kept, and the doc now says which: `owed_balance` (migration 013) carries "money I have or money I owe", and "nothing routes here" is a count over `wallet_matchers`. **NOT IN THE ENTRY AND IT MATTERS:** the CSV export still has a `wallet_type` COLUMN, and it now carries `walletKind()`'s `owed`/`manual`/`tracked` (`lib/wallets/summary.ts`), so left unmentioned it reads as the retired enum surviving in a shipped export. **CLUSTER 2, THE SAVINGS-WALLET REQUIREMENT FOR GOALS:** dropped deliberately, not by omission. `goals_repo`'s `assertWalletIsFree` checks existence and that no other live goal claims the wallet, and says at its own call site why the kind is no longer its business. **INVARIANT I10 HAS TWO HALVES AND ONLY THE FIRST DIED:** the `savings` half went with the column, while "a wallet backs at most one goal" is alive and enforced by migration 016's partial unique index over LIVE rows, so a deleted goal releases its wallet. The invariant now states that precisely instead of being deleted or left whole. **CLUSTER 3 IS ALSO GAP-023'S DOC HALF:** `03-limits.md`'s create flow never mentioned that entering one limit creates three derived siblings, nor that the Free cap counts source limits only. Both were decided on 2026-08-20 and lived in a code comment. **CLUSTER 4:** the drift-tolerance open question is half settled and the row says which half. The SHAPE is decided, a fixed peso amount held as the per-ruleset tunable `balanceDriftToleranceCentavos` with `DEFAULT_TUNABLES` shipping ₱1.00, so retuning is a ruleset publish rather than an app release; neither a percentage nor a hybrid was taken. The VALUE is still open and still wants field data, so the question is struck and re-scoped rather than closed. **CLUSTER 5:** bills advertised a merchant keyword SET; the rule holds ONE normalized substring matched with `includes` and falls back to the bill's own name. What does accumulate as a list is the exclusions, one per rejection. **CLUSTER 6:** the CSV table was written for FOURTEEN columns and `CSV_HEADER` ships EIGHTEEN, which the entry did not count. The table now points at `CSV_HEADER` as authoritative and names the two deliberate deviations argued at that file's head (`timestamp` ships as `date` plus `time`; `is_transfer` ships `yes`/`no`), and the file-name rule promised a `<start>-<end>` range while the code emits a single date. **CLUSTER 7:** Recurring is its own screen at `app/(tabs)/more/subscriptions.tsx`, a sibling of `more/reports.tsx`, not a section inside Reports as three lines across two docs claimed. **CLUSTER 8:** katapusan is the LAST CALENDAR DAY, not the 30th; `due_rules.ts` says so at its `semi-monthly` case and February is the case that makes the difference. The `(15th/30th)` shorthand in the cadence enum is kept but labelled imprecise, rather than diverging from the same shorthand in `04-income.md`. **CLUSTER 9, WHERE THE ENTRY IS RIGHT AND THE REAL SUBTLETY IS THE OPPOSITE OF MISSING WORK:** the doc said onboarding steps 3 to 5 were not built; all nine are, each with a screen under `app/(onboarding)/`. What is actually true is that `providers` is RESERVED in `ONBOARDING_STEPS` but NOT ROUTED in the numbered flow, because the pre-flow already ran that screen with its `onDone` wired, so `battery` advances straight to `wallets` and pushing `providers` again would strand the user on a screen whose forward action is `undefined`. **CLUSTER 10:** the v2 backlog said the three notifiers were unwired; `d20f004` ("fire the alerts nothing was calling") wired them the same day, verified by reading that commit.
...
> **REMEDIATION: PARTIAL** (2026-09-17) - commit 0fbacf3, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **DELIBERATELY PARTIAL: FOUR OF THE ENTRY'S SIX ITEMS, AND THE TWO RESIDUALS ARE NAMED RATHER THAN FORGOTTEN.** Verified by reading rather than by test, since none of it is testable code: the YAML parses to three ecosystems, `mobile/package.json` still parses as JSON after the edit, and `jest module_wiring` 5 tests PASS because that suite is the only thing in the app that opens `mobile/package.json` at all. **DONE (1): THE ROOT `CLAUDE.md`,** per the owner's decision of 2026-09-17, recording conventions that already hold rather than inventing any: the snake_case default with the camelCase carve-out inside `mobile/`, where the spec lives, that the remediation log in section 0 is the source of truth for what is fixed, and that `mobile/` has no ESLint config ON PURPOSE so the next agent stops reading that as an omission. **DRAFTING IT CAUGHT TWO CLAIMS THAT WOULD HAVE SHIPPED AS FACT AND WERE FALSE.** "Jest per package" is wrong: `server/` runs vitest, with its own `eslint.config.mjs` and `lint` script, and only `mobile/` uses jest. And a `pull_request` evaluates a workflow's path filter against the WHOLE PR diff rather than the last commit, which is why a docs-only commit pushed onto a branch that already touched `mobile/` still runs the app matrix while a docs-only PR runs nothing at all. **DONE (2): THE VERSION MISMATCH.** `mobile/package.json` said 1.0.0 while `mobile/app.json` and `server/package.json` both said 0.1.0. Aligned DOWN to the shipped figure, because `app.json` is the one Expo reads and nothing reads `package.json`'s version; the single file that opens it reads `expo.autolinking`. **DONE (3): THE STALE ROOT `HANDOFF.md` IS GONE.** Dated 2026-08-18, describing a branch that has since merged and a test count that no longer matches. A stale handoff at a repository root costs every new reader a few wrong turns before they discover it is stale, and handoffs belong outside the repository. **DONE (4): `.github/dependabot.yml`.** `server/` and the workflow actions get weekly grouped updates. **`mobile/` IS SECURITY-ONLY, VIA `open-pull-requests-limit: 0`,** which suppresses version-update pull requests and leaves security ones untouched: an Expo SDK pins several native dependencies to exact versions, and bumping one off that matrix breaks the prebuild rather than a test, so `expo install --check` is the right tool there. **RESIDUAL (1): MERGED BRANCHES ARE NOT DELETED.** Twenty local and twenty-four remote are merged into master, and deleting a remote branch is not recoverable from a clone. An entry listing it is not authorisation to do it; the owner's is. **RESIDUAL (2): THE DUPLICATED DESIGN PROMPTS STAY, AND THE ENTRY'S PREMISE IS HALF WRONG.** It calls `docs/pera-plano-mobile/uploads/` and `docs/pera-plano-web/uploads/` copies of `docs/11` and `docs/10`. The mobile one is byte-identical; the WEB one differs, and the canonical file is the superset (it carries the full colour-token list with dark-mode values that the copy states vaguely), so the copy is an older draft rather than a duplicate. More to the point, both sit INSIDE exported design bundles, beside `.dc.html`, `support.js`, a `.thumbnail` and an `assets/brand` tree, so they are that export's upload payload rather than stray files in `docs/`. Deleting one file out of an export trades provenance for tidiness.
...
> **REMEDIATION: DONE** (2026-09-18) - commit 970da75, branch worktree-gap-wave-21. Verification: **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run, KOTLIN INCLUDED, WHICH MATTERS BECAUSE CI DOES NOT COVER IT:** `:notification_listener:testDebugUnitTest` BUILD SUCCESSFUL, 11 suites / 173 tests, 0 failures, 0 errors, 0 skipped, with `compileDebugUnitTestKotlin` executed rather than up-to-date, so the new interface member genuinely compiles. Read from the JUnit XML rather than from the console tail. `jest lib/crypto` 5 suites / 73 tests PASS; `npx tsc --noEmit` exit 0. **`mobile-ci.yml` HAS NO GRADLE JOB**, so that local run is the only gate on every Kotlin line here; it was taken with `--no-daemon`, a 1 GB heap and one worker because the machine was at 2.05 GB free of 23.71, which is the condition that killed two runs in wave 17. **BOTH NEW JS TESTS WERE RUN AGAINST THE UNFIXED SOURCE AND OBSERVED TO FAIL.** The Kotlin test was NOT re-run against reverted source, deliberately, and the evidence is stronger than a revert would have been: its PREDECESSOR asserted the opposite behaviour (drain resolves empty, file deleted) and passed, so the two assertions are mutually exclusive and the behaviour demonstrably changed. **THE PREVIOUS AUTHOR LEFT THIS FIX INSTRUCTIONS AND THEY WERE FOLLOWED.** `CaptureBufferTest.kt` already carried a test pinning the defect, with a comment reading "when it lands this test MUST go red and be rewritten to assert the new contract" and "do not weaken it to keep the suite green; a red test here is the fix landing correctly". It was inverted, not deleted, and it now asserts the same contract as the auth-failure test above it. **TWO DEFECTS, TWO HALVES.** (1) `KeyPermanentlyInvalidatedException` extends `InvalidKeyException`, so it was landing in `openAll`'s generic per-line `catch` and being counted as one unreadable line among many; `drain` then deleted the file. It now rethrows beside `UserNotAuthenticatedException` and ABOVE the generic catch, because Kotlin takes the first matching clause and the order is what keeps it from becoming dead code. Nothing can recover a capture whose private key is gone, so the choice was between losing the buffer silently and telling JS, and JS can only decide about captures it still has. (2) `rewrapAfterInvalidation` recreated the device KEK and nothing else, so it reported a successful recovery while `capturePublicKeySpki()` still returned the DEAD pair's public half. Every capture sealed after that recovery was unopenable too, each drain discarded its batch, `lastCaptureAt` kept advancing and the listener-health card kept reporting a working listener. **DEPARTURE FROM THE ENTRY'S PRESCRIPTION, DELIBERATE.** It asks for `recreateRsaKeyPair` "mirroring `recreateAesKey`", which for AES meant extracting a shared `generateAesKey` helper. The RSA `KeyGenParameterSpec` runs to some sixty lines of deliberate choices (both OAEP digests authorized for pre-API-30 Keymaster, StrongBox fallback, a 10-second auth window measured against a 500-record drain), and a second copy of it is precisely how a recreated key silently becomes weaker than a first-run one. `synchronized` is reentrant on the JVM, so deleting the alias and delegating to `getOrCreateRsaKeyPair` keeps the delete and the regenerate atomic under the one lock with exactly one copy of the spec. **ORDERING INSIDE `rewrapAfterInvalidation` IS ALSO DELIBERATE:** the capture-keypair call sits AFTER the device re-wrap, so a failure there cannot strand a recovered DEK with no wrap on disk. **`wipeKeys` CALLS IT TOO**, because a wipe is the likeliest thing a user reaches for after an invalidation, and "start over" must not start over on the dead pair that sent them there. **`docs/13` PART 5 GAINED THE BOX THAT WOULD HAVE CAUGHT THIS:** its screen-lock check stopped at "the ledger is still there", which the old code passed while capture stayed dead, so the checklist could never have found it.
...
> **REMEDIATION: DONE** (2026-09-18) - commit 3223a9d, branch worktree-gap-wave-23. Verification: **CI ON 9435f08, ALL GREEN, read from the completed runs:** mobile-ci 35315046283, 9 of 9 jobs success; server-ci 35315046296 and 35315008962, both success. server-ci fired on this wave and did NOT fire on wave 22, which is the filter working rather than a fluke: it is path-filtered on docs/07-privacy-and-compliance.md among others, and GAP-061 changed that file. mobile-ci still has NO gradle job, so nothing in these runs gates a single line of Kotlin. OWNER DECISION 2026-09-18: build the editor, and refuse a linked transfer leg rather than editing it. Both are recorded in the entry. THE ENTRY OVERSTATED THE WORK AND THAT CHANGED THE DECISION. Its blast radius says editing amount, direction or wallet must re-apply balance deltas in the repository; that already existed. updateTransaction accepts walletId, categoryId, amount, direction, occurredAt, merchant, counterparty, referenceNo and note, and calls settleBalance inside its own withTransactionAsync, written reverse-then-apply so an amount change, a direction flip and a wallet move are all correct at once. Delete had already landed under GAP-108, which the entry's own 2026-09-10 amendment records. What was missing was purely UI, so the option was much cheaper than the entry implied and that was put to the owner alongside the doc-scoping alternative. Line cites were stale again: the two updateTransaction.mutate calls are at transaction/[id].tsx:264 and :276, not :229 and :236. Built: a route at app/transaction/[id]/edit.tsx matching app/wallet/[id]/edit.tsx, since editing an entity is a route in this app while the one-field affordances (category picker, note) stay inline; a presentational components/transactions/transaction_edit_form.tsx composing NumericField, SegmentedControl and the wallet rows. DELIBERATELY NOT A REUSE OF CorrectSheet OR ManualEntryForm even though both edit the same five fields: the first takes a ReviewQueueItem and exists to report a diff that decides whether a UserRule is created, so reusing it would mean manufacturing a fake queue item and dragging rule-creation semantics onto a committed row; the second is built around amount-first numpad entry for a row that does not exist yet. The form reports ONLY changed fields, which is correctness rather than tidiness: useUpdateTransaction narrows its invalidation on patch.walletId === undefined, so always sending every field would make that test permanently false and widen every edit to a whole-wallets-family invalidation. A DEFECT FOUND WHILE BUILDING, NOT IN THE ENTRY: the detail screen renders merchant falling back to counterparty, so nulling merchant alone let the parser's raw counterparty surface in its place and the removed name REAPPEARED, which is the opposite of what docs/07 rule 4 promises and leaves a third party's name on the device against rule 2. Removal now clears both columns; a relabel still leaves counterparty alone because it is the parser's record and loan matching reads it. The transfer refusal is enforced on the detail screen AND again at the route, because /transaction/<id>/edit is addressable directly and a guard on the button that usually opens it is not a guard. Tests: components/transactions/__tests__/transaction_edit_form.test.tsx 19 of 19, app/__tests__/transaction_edit.test.tsx 9 of 9 against a REAL database asserting the wallet balances actually move (amount difference, direction flip moving by twice the amount, and a wallet move settling BOTH wallets), app/__tests__/transaction_detail.test.tsx 30 of 30 including two new. npx tsc --noEmit exit 0. docs/07 rule 4 and the Edit anything row both amended. NOT verified on device.
...
**Evidence**
The detail screen mutates `{ note }` and `{ categoryId }` only. Amount, direction, wallet, merchant, date and reference render as static rows with no press handler. No delete action exists on the screen; `git grep -n "deleteTransaction\|useDeleteTransaction" mobile/app` returns nothing. docs/07:205 says every parsed field is user-editable on Transaction detail; :75 lists that as the DPA right-to-rectification mechanism. **EVIDENCE PARTLY SUPERSEDED 2026-09-10:** the grep quoted above no longer returns nothing. GAP-108 (`ba02d7b`) added the delete action, `useDeleteTransaction` and a confirmation naming what is removed, so POSITION B'S "no delete" HALF IS CLOSED. What still stands, and what this entry is now solely about, is the editing half: amount, direction, wallet, merchant, date and reference are still static rows with no press handler, and docs/07:205's "every parsed field is user-editable" and :75's right-to-rectification claim still have no code behind them. The owner decision this entry is waiting on is unchanged in substance but smaller in scope.
...
**Open questions**
- Build the editor or scope the doc? (Wave 0 decision.) **ANSWERED 2026-09-18 — see the decision below.**

> **OWNER DECISION (2026-09-18): BUILD THE EDITOR.** docs/07's rectification row stands as written;
> amount, direction, wallet and merchant become editable rather than the claim being scoped down to
> what shipped. **REJECTED: rewriting docs/07:75, :190 and :205 to name note, category and Review
> Queue correction as the mechanisms**, which was the cheap option and would have weakened a
> compliance statement; and **REJECTED: editing amount and direction only**, which still left the
> doc needing an amendment for wallet and merchant and so paid part of the cost anyway.
>
> **THE ENTRY OVERSTATED THE WORK, AND SAYING SO IS WHAT CHANGED THE ANSWER.** Its blast radius
> claims editing amount, direction or wallet "must re-apply balance deltas in the repository". That
> already existed: `updateTransaction` takes every field docs/07 names and calls `settleBalance`
> inside its own unit of work, written reverse-then-apply so an amount change, a direction flip and
> a wallet move are all correct at once. Delete had landed under GAP-108. The remaining work was
> UI alone, which made the expensive-looking option the cheap one.
>
> **OWNER DECISION (2026-09-18): A LINKED TRANSFER LEG IS REFUSED, NOT EDITED.** The editor is not
> offered on a leg whose `transferLinkId` is set; the screen says so and points at the Unlink action
> it already has. Both legs describe ONE movement, and `transfer_link_id` is the schema's only
> expression of "not spending, not income" (GAP-080), so editing one alone leaves the pair
> describing two different movements with nothing anywhere to detect it — both legs stay out of
> spend and income either way, so no screen would ever show the contradiction. **REJECTED: editing
> both legs together**, which founders on transfers carrying a FEE, where the legs legitimately
> differ and "keep them equal" is wrong, so the form would have to ask which number is being
> corrected; and **REJECTED: editing one leg silently**, which is the mismatch itself.
...
> **REMEDIATION: DONE** (2026-09-18) - commit d6d44d9 and 848dba2, branch worktree-gap-wave-25. Verification: **CI ON 3c6b8c2, GREEN, read from the completed run:** mobile-ci 35354586319, 9 of 9 jobs success (typecheck, jest 1 through 6, tests, secret-scan). server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07-privacy-and-compliance.md, and this wave touched docs/04-features/01-onboarding.md. mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin, which is exactly why GAP-125 was filed rather than folded into this wave. OWNER DECISION 2026-09-18: PERSIST AN onboarding_step SETTING, do not amend the doc. Recorded in the entry. Both halves of this entry are closed, and only the cursor needed the decision.  THE ENTRY'S CITES WERE ACCURATE TO WITHIN TWO LINES, which is unusual for this campaign, BUT THE RESUME MECHANISM IT NAMES IS THE WRONG ONE AND THE HANDOFF'S FIRST INSTRUCTION WAS WRONG TOO. The entry points at lock_context.tsx checkKeyState for the restart. That branch only sets needs_onboarding when getKeyState returns uninitialized, and keys exist by the time the numbered flow runs, so a background relock sets locked, not needs_onboarding. The actual path is: unlock, app/index.tsx reads onboarding_complete false, routes to the group, and app/(onboarding)/index.tsx:148 redirects unconditionally to welcome. Same outcome, different code, and the fix lands somewhere else than the entry implies. Separately, the handoff said the first edit was wallets.tsx:198 seeding included false; :198 is CASH_PROPOSAL's always-on cash seed and must stay true. The unconditional observed seed was :331.  A THIRD DEFECT THE ENTRY DOES NOT MENTION, AND IT IS THE WORST OF THE THREE. setMatchers MOVES a claimed pair rather than duplicating it (wallet_matchers_repo.ts's own MOVE, NOT DUPLICATE rule, which exists because two wallets holding one pair makes every capture from that provider unresolvable forever). So on a second pass over a wallet the user had RENAMED, the name check passes, a fresh GCash wallet is created, and setMatchers takes com.globe.gcash.android off the renamed wallet and gives it to the new empty one. No error, no failed write, and nothing on any screen naming the cause; the user's money simply starts landing somewhere else. The entry's proposed fix says name or package without saying why package matters, and a name-only fix would have shipped this. The package half is therefore load-bearing rather than belt and braces.  IDEMPOTENCY, BUILT IN TWO PLACES ON PURPOSE. The init effect seeds included false for any proposal whose name, folded to lower case to match createWallet's own COLLATE NOCASE, or whose provider packages a non-archived wallet already holds. The creation loop repeats the check against fresh data and skips. Two places because they answer different failures: the seed is what makes the screen honest on arrival, and the loop is what makes the writes idempotent regardless of the seed, since the user can tick a row back on and since the seed's queries are deliberately NOT added to the effect's gate. That last point is the one real trade: gating on them would make the seed always correct at the cost of wedging the whole step on its loading skeleton for good if a query never resolves, which costs the install rather than one wallet the user can add later from the Wallets tab. The loop makes the outcome safe either way. Cash is handled by the same rule rather than specially: the proposal is always PRESENT, and only pre-TICKED when no cash wallet exists.  THE CHOSEN LIMIT SCOPE IS UPDATED, NOT SKIPPED, and the entry offers both. Skipping would mean the user fills in the form, taps save, and the screen silently does nothing, which is indistinguishable from a broken button. An edit is also what the app already calls this, since app/(tabs)/plan/limits/[id]/edit.tsx treats editing a DERIVED limit as an ordinary edit and updateLimit's SQL does not touch derived_from, so the provenance of a row this replaces survives. rollover and is_active are deliberately absent from the patch: a fresh limit is created with rollover false because that is this step's fixed default, but forcing that same false onto a limit the user had since switched on in Plan would quietly change how much they may spend from a screen that never mentioned it. occupied at first_limit.tsx:91 already answered this question for the three derived cadences; the scope the user actually asked for was the one case never asked about.  THE CURSOR IS WRITTEN IN THE ROUTE GROUP'S LAYOUT, OFF THE PATHNAME. One place for all ten steps rather than nine advance sites, and a step that forgot to record itself would silently send the user backwards on the next interrupted run. The pathname is also what actually changed when the user moved, so a BACK navigation is recorded too; a mount effect on the screens themselves could not manage that, because router.push leaves the pushing screen mounted underneath and its effect never fires again on the way back. OnboardingFrame was the other single place and is NOT eligible: it lives in components/, and a component may not reach a repository, which is the release-gate rule first_limit.tsx's own header names. The write's failure is swallowed, because the cursor is a convenience and a write that cannot land leaves the flow behaving exactly as it did before it existed. The resume target is read BEFORE the step state flips, so the redirect cannot render once at welcome and correct itself afterwards, which would already have navigated. A stored value outside ONBOARDING_STEPS reads as welcome, so the worst a bad row can do is what every row used to do.  Tests: a new setup_flow_e2e case drives the whole flow TWICE over one database, which is the only shape that can catch any of this, since every per-screen suite starts from an empty database where all three defects are invisible. IT WAS CONFIRMED TO FAIL AGAINST THE UNFIXED SOURCE BEFORE THE FIX WAS RESTORED, exit code red with Expected false Received true on the seed assertion, so it is a guard rather than a decoration. Five new onboarding_state cases cover the cursor round trip, replacement, the pathname matcher and a value planted through the table that this build does not know. Three new index.test cases cover resuming at the recorded step, a fresh install still starting at welcome, and the redirect never rendering at welcome first. The pre-existing completeOnboarding is the only writer this module exposes test pinned the module's export surface and was updated rather than deleted, with its comment rewritten to say what it now guards: recordOnboardingStep writes onboarding_step and cannot reach the flag.  Local, sequential, all green: components/onboarding 16 suites 196 tests; app/(onboarding)/__tests__ 6 suites 44 tests; lib/onboarding 12; lib/__tests__/bootstrap 31; app_settings_repo 42. npx tsc --noEmit exit 0. docs/04-features/01-onboarding.md gained two dated as-shipped notes, one beside the Resumed state row it makes true and one under step 7. The acceptance box reading Killing the app at any onboarding step resumes at that step on next launch is deliberately LEFT UNTICKED: jest covers the round trip and the redirect target, and that the process actually dies and returns to the right screen is a device check. NOT verified on device.
...
> **REMEDIATION: DONE** (2026-09-18) - commit a20bd43, branch worktree-gap-wave-24. Verification: **CI ON 96753bb, GREEN, read from the completed run:** mobile-ci 35331321676, 9 of 9 jobs success. server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07, and this wave touched docs/12 rather than docs/07. NOTE FOR ANYONE READING CI ON A BRANCH: mobile-ci's push trigger is scoped to master, so a feature branch gets no run at all until a pull request exists, and an empty run list on a pushed branch means "no PR yet" rather than "nothing to run". mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin. OWNER DECISION 2026-09-18: ENCRYPT THE COPY, do not scope the claim. Recorded in the entry. All three problems the entry bundles are closed, and only the first needed the decision. ONE, THE AT-REST CLAIM. Picked files are now AES-256-GCM under the DEK via the new lib/crypto/attachment_cipher.ts, written as <uuid>.enc, and the bytes go from the picker's cache file straight through the cipher so the plaintext never reaches durable storage at all; a copy-then-encrypt would have opened the exact window this closes. The extension is deliberately NOT the picked file's type, because naming ciphertext .png is a lie a gallery scanner or share-sheet handler would act on. NOT A REUSE OF cache_cipher.ts even though it is the same algorithm under the same key, and the reason is measured rather than stylistic: that module returns HEX and hex doubles the payload, which is a handful of kilobytes on the query cache and a 16 MB string built on the JS thread for an 8 MB screenshot, on the same thread already measured stalling 4.6 seconds under that shape of work. This module moves Uint8Array in and out. For the same reason it reaches for the SDK 54 File API in exactly two calls and nowhere else, because the legacy API's only binary form is a base64 string and key_manager's hand-rolled base64 helpers build a number array one byte at a time, which for 8 MB is a ten-million element array. The key is set and scrubbed BESIDE the query cache's own key on all five lock-lifecycle sites, so the two cannot drift apart and leave a live DEK copy in whichever module was forgotten, and a lock_context test pins that pairing. ONE PLAINTEXT WINDOW SURVIVES AND IS DOCUMENTED RATHER THAN HIDDEN: React Native FormData streams from a path and has no byte-array form, so the sender decrypts into the CACHE directory, uploads, and unlinks on every outcome including a throw, which makes the plaintext last the length of one request in the one directory Android reclaims by itself rather than the length of a queue. The multipart filename now derives from the row's mimeType, because it used to be the stored file's last path segment and would otherwise put .enc or nothing in front of whoever opens the ticket. TWO, THE DRAFT RACE, which needed no decision and was the user-visible defect: releaseDraft ran only in the then-branch, so leaving the screen while the insert was in flight fired the unmount teardown, deleted the files, and left the row that landed a moment later pointing at nothing; every send then failed inside FormData, was mapped to the generic No connection, and the report retried forever against a reason naming the wrong thing. The draft now hands the files over BEFORE the row that owns them is written and takes them back if that write fails. THREE, THE MISSING SWEEP: discardDraft is best-effort by design so a killed process leaked files until uninstall, and the temp files make that sharper because those are plaintext. A launch pass unlinks every attachment no row points at and every temp file outright. THE READ AND THE SWEEP ARE IN ONE TRY DELIBERATELY: the sweep deletes whatever the reference list does not name, so a failed read must never arrive as an empty list and wipe the attachments of every report still waiting to send; throwing out of the read skips the sweep, which is the correct failure, because a leaked file costs kilobytes and a swept queue costs the user their evidence. Tests: lib/crypto/__tests__/attachment_cipher.test.ts 10 of 10 over real AES-GCM including a wrong key, a flipped bit at four positions, a truncated blob, an all-zero key counting as absent and the key being copied rather than aliased; lib/support and services/support_reports 61 of 61, with the load-bearing one asserting the readable string is absent from the bytes at rest rather than merely that the bytes differ; lock_context 51 of 51 including the new key-pairing test; bootstrap 31 of 31; components/support and hooks 110 of 110. npx tsc --noEmit exit 0. docs/12 section 4 gained a dated subsection in the style that section already uses for the 2026-08-14 prefs gap, and section 8 gained a row. NOT verified on device, and the upload path has never run against a real server because none exists.
...
> **OWNER DECISION (2026-09-18): ENCRYPT THE COPY. Do not scope the claim.** docs/12 §4's
> at-rest promise stands as written; support attachments become ciphertext rather than a named
> exception to it. **REJECTED: naming support attachments and CSV exports as the plaintext
> exceptions**, which was the cheap option and would have given the product's security stance a
> permanent carve-out; and **REJECTED: scoping now and encrypting later**, because "later" competes
> with twenty other open gaps at priority 0.5 and a carve-out outlives the intention behind it.
>
> **WHAT TIPPED IT: how long the plaintext actually lives.** The file is not a transient — it sits
> in the sandbox for as long as the report stays queued, which on a phone with no signal is
> indefinitely, and the content is screenshots of bank notifications carrying amounts, balances and
> counterparty names. "The user chose to upload it" does not cover that: they chose to send it to
> support, not to leave it readable on a rooted phone for a month.
>
> **REJECTED: storing the bytes in SQLCipher instead**, which would have inherited encryption for
> free and needed no new key lifecycle. An 8 MB blob base64'd across the React Native bridge is the
> exact JS-thread stall this app has already measured on the A54.
>
> **The plaintext window at send time is accepted and documented, not eliminated.** React Native's
> `FormData` streams from a path and has no byte-array form, so an upload needs a readable file.
> It is decrypted into the cache directory, uploaded, and unlinked on every outcome including a
> throw — the length of one request, in the one directory Android reclaims on its own.
>
> The two bugs this entry also carries — the draft race and the missing sweep — needed no decision
> and were fixed regardless.

**Implementation checklist**
- [x] Owner picks: scope the doc or encrypt. **ENCRYPT (2026-09-18).**
- [x] In `docs/12-encryption-and-app-lock.md` §4, name the exception (either way, until encryption lands). **Superseded: §4 records the encryption and the one remaining send-time window instead of an exception, and §8 gained a row.**
...
> **REMEDIATION: DONE** (2026-09-17) - commit 773a9bc, 5ab85e9, branch worktree-gap-wave-20. Verification: **CI RUN 35288657472 ON ff52b1b: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run:** `jest lib/db/repos archive_wallet_sheet` 20 suites / 526 tests PASS, `jest lib/db/repos/__tests__/transactions_repo wallets_repo` 2 suites / 121 tests PASS, `npx tsc --noEmit` exit 0. **ALL FOUR NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `transactions_repo.ts` alone and re-running each. **SHIPPED IN TWO COMMITS BECAUSE IT IS TWO DEFECTS**, `773a9bc` for the balance anchors and `5ab85e9` for the transfer legs. **THE ANCHOR HALF:** `balance_after` is the SOURCE provider's statement about the SOURCE account and `computed_balance` is written as its pair, so carrying them into another wallet turns them into a claim that the DESTINATION's bank reported that figure. `getBalanceDrift` (`wallets_repo.ts`) takes the newest row in a wallet with a non-null `balance_after` and compares the two columns, so a moved row that happens to be newest makes the destination compare its own ledger against a different bank's number and raise a Balance mismatch badge that nothing clears until its own provider reports again. Both are nulled on moved rows. That is not a workaround for the badge: those columns are provenance about a WALLET, and the wallet is exactly what changed. The source's `drift_dismissed_transaction_id` is cleared too, but ONLY when the dismissed row actually left, because a skipped leg can stay behind and still carry the reporting balance. **THE TRANSFER-LEG HALF, AND THE ENTRY'S PRESCRIPTION WOULD HAVE SHIPPED A WORSE DEFECT THAN THE ONE IT FIXES.** The entry proposes "exclude or unlink legs whose counterpart wallet is the destination", and the doc's own rule 4 asked for dissolve-and-re-triage. `transfer_link_id` is this schema's ONLY expression of "not spending, not income" (migration 017 says so outright, and invariant 2 rests on it), so unlinking an equal in-leg and out-leg makes BOTH start counting: reported spend and reported income each rise by the transfer amount, to tidy up a link nobody can see. Doing it as the doc asked would additionally need a new `ReviewKind` and a migration, and `types/domain.ts` warns that a triage action which commits a payload against the wrong kind writes the same money twice. **OWNER'S DECISION, 2026-09-17: SKIP THE LEG INSTEAD.** A leg whose counterpart already sits in the destination stays where it is, the link stays intact across two wallets, and nothing downstream learns a new case. It costs a few rows staying in a wallet the user deleted, which is soft-deleted rather than removed, so they remain fully visible in history. Doc rule 4 was amended to match. **THE SKIP PREDICATE IS SHARED BY THE DELTA QUERY AND THE UPDATE**, for the reason they already read inside one transaction: a delta measured over a different set than the one that moves settles both balances against a ledger that never existed. The two balance figures were predicted before the run and matched exactly, which is the evidence the two sets agree. **A COPY CHANGE WAS CAUGHT BY THE CODEBASE'S OWN LINE-CLAMP SWEEP:** the first subtitle ran to 121 characters against `subtitleLines={3}`, which `components/ui/list_row.tsx` sizes for 63 at 22 chars per line, so the new caveat would have been clamped away on device while every test passed, because RNTL cannot see a clamp. Shortened to 86 characters with the clamp raised to 4, and "only the wallet moves" was dropped to pay for it since the skip makes that clause imprecise. **THE ENTRY'S LINE CITES HAD DRIFTED:** `reassignWalletTransactions` is at `transactions_repo.ts:620`, not `:531-569`. **ALSO CORRECTED: THE CODE ALREADY KNEW.** Its docblock carried the transfer-leg case as a deliberate deferral blocked on "the Review Queue's write path (m1c Task 10) and `unlinkTransfer`"; both now exist, so the stated blocker was gone and the deferral note is replaced by the decision.
...
> **REMEDIATION: DONE** (2026-09-17) - commit 847ca99, branch worktree-gap-wave-17. Verification: **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand. This is the verification of record because wave 17's full local sweep never completed: two attempts were killed by the OS for low memory. **Orchestrator-run scoped suites, 2026-09-17:** `components/loans/__tests__/loan_form lib/db/repos/__tests__/loans_repo lib/db/__tests__/migrations lib/db/__tests__/schema` 5 suites / 340 tests PASS; `npx tsc --noEmit` exit 0. **THE ENTRY'S PROPOSED FIX WAS REJECTED BEFORE DISPATCH AND THE OWNER'S SECOND BRANCH SHIPPED.** "Hide the principal field for flat and drop it from `canSave`" would have deleted a documented input and permanently foreclosed doc open question 3: `docs/04-features/06-loans.md:43` defines a flat loan as TWO figures ("borrowed P5,000.00, repay P6,000.00"), the worked example at `:82` enters both, and the implied-cost figure open question 3 asks about exists only as total repayable minus amount borrowed. **WHAT SHIPPED IS A MIGRATION, NOT THE FORM TWEAK THIS WAS FILED AS** (re-graded S/D2 to M/D3 in `e7fca86` before dispatch): `lib/db/migrations/020_loan_amount_borrowed.sql:53-54` adds `ALTER TABLE loans ADD COLUMN amount_borrowed INTEGER CHECK (amount_borrowed IS NULL OR amount_borrowed > 0)`, registered at `lib/db/migrations.ts:21`, surfaced as `amountBorrowed: Centavos or null` (`types/domain.ts:379`) and mapped in the form at `loan_form.tsx:192` (edit seed) and `:661` (submit). `principal` is untouched and still holds `installment * count` for flat, because the balance formula at `06-loans.md:89` counts outstanding down from total repayable; putting the borrowed P5,000 there would report a P6,000 utang settled with P1,000 still owed. **NO "principal" SUBSTRING IS IN THE COLUMN NAME, deliberately:** `principal` is a balance basis the app counts down, `amount_borrowed` is an EVENT, cash that changed hands once and never moves, and the two would otherwise be read past each other. **DELIBERATELY NOT BACKFILLED.** The borrowed figure cannot be recovered from the total, because loans rule 4 forbids deriving a rate for a flat loan and a rate is the only thing that could bridge them, so copying `principal` in would assert the user borrowed the total repayable, which is false for every flat loan carrying any add-on; a confident wrong number is worse than an admitted unknown. NULL also means "no separate figure" for amortized and free-form loans, where `principal` already IS the amount borrowed. **NO `CHECK (amount_borrowed <= principal)`:** repaying less than was borrowed is unusual, not impossible, and a constraint refusing a real arrangement costs the user their record of it. The amount stays REQUIRED for every new loan, unchanged from before, because `:40` asks for it at step 2 before the schedule type is chosen at step 3; it is exempt ONLY when editing a flat row the app itself never recorded. Keeping the requirement there would be actively harmful: the only figure in front of the user is the total repayable, so the requirement would push them to type it and manufacture exactly the falsehood the nullable column exists to prevent. Switching such a loan to amortized re-requires it. A legacy null row shows an empty box and a line saying nobody recorded the figure, NOT a P0.00 preview, because P0.00 is a claim that nothing was borrowed. The migration test is proven against a real version-19 database holding a flat loan, asserting the column is ABSENT first so a fresh database cannot pass it, and that exactly [20] is applied.
...
> **REMEDIATION: PARTIAL** (2026-09-18) - commit 7c1a00a, branch worktree-gap-wave-25. Verification: **CI ON 3c6b8c2, GREEN, read from the completed run:** mobile-ci 35354586319, 9 of 9 jobs success (typecheck, jest 1 through 6, tests, secret-scan). server-ci correctly did NOT fire: its path filter covers server, docker-compose, the two workflow files and docs/07-privacy-and-compliance.md, and this wave touched docs/04-features/01-onboarding.md. mobile-ci still has NO gradle job, so nothing here gates a line of Kotlin, which is exactly why GAP-125 was filed rather than folded into this wave. OWNER DECISION 2026-09-18, TWICE: move the step into the reserved slot after battery; then, once the move was built and the native source read, record this entry PARTIAL and file the snapshot separately rather than add ungated Kotlin. Both are recorded in the entry.  PARTIAL BY DECISION, NOT BY OMISSION, AND THE RESIDUAL IS THE ACCEPTANCE CRITERION ITSELF. The entry's diagnosis is right: observed packages exist only after the listener is bound, and the picker ran several screens earlier, so listObservedPackages returned an empty list on every fresh install. ITS PRESCRIPTION DOES NOT REACH ITS OWN ACCEPTANCE CRITERION, which is the campaign's central lesson again. observed_packages is written in exactly one place, onNotificationPosted at PeraPlanoNotificationListenerService.kt:287; onListenerConnected at :81 calls ensureKeysReady and recordConnection and nothing else, and there is no getActiveNotifications call anywhere in the module. So the list is empty at the instant access is granted and fills only from notifications posted afterwards. The picker now sits two screens after the grant, which is seconds to a minute, so the move changes ALWAYS empty to USUALLY still nearly empty. Strictly better, and not the criterion, which asks for a provider to be listed under Apps we've seen on a fresh install. Closing it means snapshotting activeNotifications on connect, which is Kotlin, and mobile-ci has no gradle job, so it would ship gated only by a local run. Filed as GAP-125 with the call site, the test file and the gradle command.  THE CODE'S OWN JUSTIFICATION FOR THE EARLY POSITION WAS VOID RATHER THAN MERELY OUTWEIGHED. app/(onboarding)/index.tsx said the picker runs after the phrase because it is the first step that writes anything the listener will act on; the listener cannot act on anything before the grant, so the reasoning defended a position that guaranteed the defect. ONBOARDING_STEPS has always listed providers at index 4, unrouted, and docs step 6 puts it in the same place, so the move is into a slot that was already reserved for it.  THREE THINGS FOLLOW FROM THE NEW POSITION AND ARE PART OF THE MOVE RATHER THAN EXTRA SCOPE. ONE, THE SCREEN HAS AN OPEN DATABASE, so it persists paused_provider_packages itself. This is mandatory, not opportunistic: the whole pending_provider_pause hand-off exists because the step used to run above the unlock gate, and bootstrapApp has already run by the time the step is reached in its new position, so deferring would have left the Privacy centre's switches stale until the app was restarted. The hand-off module is KEPT, because it is still what survives a write that fails and the launch drain still recovers it, and an install that reached the step before this change still has to be drained; the body moved out of bootstrap.ts into pending_provider_pause.ts beside the inversion rule it depends on, with both callers reaching one function. The long comment in bootstrap.ts explaining why that screen could not write its own selection was deleted rather than left, because it is now false. TWO, THERE IS AN ACCESS GRANT TO READ, so docs rule 8, skip the picker when access was declined, is implemented for the first time. It was never implementable before: there was no answer to ask for at that point in the flow. A bridge that cannot answer shows the picker, because an unnecessary picker is one skippable screen while a wrongly skipped one silently removes a choice the user cannot get back inside this flow. Nothing is written on the declined path, since allow-all is already the on-disk default and recording an empty allowlist records nothing paused anyway. THREE, THERE IS A CHANNEL TO THE WALLETS STEP, so a provider the user declined is no longer proposed as a wallet one screen later. That screen's header carried a long paragraph on why no such channel could exist; every reason it gave was true of the old step order and none survives the move, so the paragraph was rewritten rather than left to mislead.  THE SUBTRACTION HAPPENS BEFORE THE PER-PROVIDER DEDUPE, and the order is load-bearing: dedupeByProvider keeps the FIRST package it sees for each provider, and the allowlist the picker writes is keyed on the exact tile the user ticked, so filtering afterwards could drop a provider the user KEPT because they ticked its second package and the dedupe kept its first. Filtering first lets the package they actually ticked become the survivor. AN EMPTY ROW FILTERS NOTHING, which is the common path rather than an edge case: ticking nothing and tapping Skip both mean capture everything and both record nothing paused, so reading an empty row as everything is paused would propose no wallets at all to most users. The Add another chips are deliberately NOT filtered, because adding one takes a deliberate tap, so nothing is created for a user who did not ask twice and someone who changes their mind still has a way back without leaving onboarding.  A RENDER LOOP FOUND AND FIXED WHILE BUILDING, NOT IN THE ENTRY. The init effect now has to depend on the advance callback, which a declined grant calls, and advance is only as stable as useRouter. A router mock returning a fresh object per call turned that into an infinite re-render, which surfaced as unexplained thirty-second test timeouts rather than as a failed assertion. Guarded with an initializedRef, matching wallets.tsx's own, so the catalogue is built once however often the component re-renders; the mock was also made stable to mirror the real hook.  KNOWN AND ACCEPTED: the picker keeps its own Continue and Skip rather than moving into OnboardingFrame, so it is the one numbered step with no progress dots and no back button. Wrapping it would mean rebuilding the picker's list and confirm interaction against a frame that supplies its own controls, which is a UI change rather than the ordering fix this entry is about.  Tests: four new provider_picker cases covering the self-navigation target, onDone still winning over the router, the declined-access skip and the bridge-failure fallback; four new providers_step cases covering a declined provider not being proposed, not being created, an empty row filtering nothing, and a declined provider surviving as an add-another chip. The declined case and the empty-row case share a setup and differ only in the stored row, so the pair proves the filter is what acts rather than some incidental difference. Five suites that pinned the old order were updated rather than deleted, each with its header comment rewritten to say what it now guards: numbered_flow_e2e, battery_step, index.test, first_run_handoff and setup_flow_e2e. numbered_flow_e2e's negative assertion was inverted rather than dropped, since skipping past the picker is the break now.  Local, sequential, all green: provider_picker 43; components/onboarding 16 suites 196 tests; app/(onboarding)/__tests__ 6 suites 44 tests; lib/__tests__/bootstrap 31; plus the privacy and capture-settings suites. npx tsc --noEmit exit 0. docs/04-features/01-onboarding.md step 6 gained a dated update that explicitly corrects the 2026-08-14 claim It runs after the recovery phrase, not at position 6, names the void reasoning, and records the remaining Apps we've seen shortfall as GAP-125. NOT verified on device, and the acceptance criterion cannot be checked on device until GAP-125 lands.
...
> **REMEDIATION: DONE** (2026-09-18) - commit 2f5ff87, branch worktree-gap-wave-21. Verification: **CI ON 3a0b21c, ALL GREEN, read from the completed runs:** mobile-ci 35302266639 9 of 9 jobs success, server-ci 35302266563 2 of 2 success. Note that server-ci is NOT path-filtered the way mobile-ci is, so it fires on this branch although nothing under `server/` changed, and mobile-ci has no gradle job at all, so neither run gates a single line of Kotlin. **Orchestrator-run:** `jest app_plugin` 13 tests PASS, `jest module_wiring` 5 PASS, `npx tsc --noEmit` exit 0. **THE NEW TEST WAS RUN AGAINST THE UNFIXED CONFIG FIRST AND OBSERVED TO FAIL.** It asserts the config value rather than the built manifest, deliberately: `mobile/android/` is gitignored and regenerated, so an `expo prebuild` is exactly the thing that would quietly restore the Android default. **THE ENTRY IS WRONG ABOUT THE BLAST RADIUS IN BOTH DIRECTIONS, AND THAT CORRECTION IS THE VALUE HERE.** It states that Auto Backup uploads `files/`, `shared_prefs/` and `databases/`. **SMALLER THAN FILED:** the sealed buffer and the encrypted database were NEVER backed up. expo-secure-store's config plugin already merges `fullBackupContent="@xml/secure_store_backup_rules"` and `dataExtractionRules="@xml/secure_store_data_extraction_rules"` into the manifest, and both files read `<include domain="sharedpref" path="."/>` with `<exclude domain="sharedpref" path="SecureStore"/>`. A single `<include>` makes Android's rules ALLOWLIST-ONLY, so `files/` and `databases/` were out of scope all along and the entry's proposed fix of "add a `dataExtractionRules` file" was moot before it was written. **WORSE THAN FILED, AND THIS IS WHY IT STILL NEEDED FIXING:** what did travel was the app's own `shared_prefs`, and `CapturePrefs` keeps THREE OF ITS SIX VALUES UNSEALED, `capture_enabled` among them, beside a sealed provider filter whose Keystore key does not travel. So a restored install came back with capture ON and a filter that cannot be opened: `openSealed` swallows the failure and returns null (`CapturePrefs.kt:635-637`), `getProviderFilter` maps null to `emptySet()` (`:152`), and `shouldCapture` returns true when the filter is empty OR the package is in it (`:288`), so an empty filter means CAPTURE EVERYTHING, from every app on the device, silently. That is a privacy failure a restore causes, not an unreadable blob. **READING THE BUILT MANIFEST UPGRADES THIS ENTRY FROM C2 TO C1:** `android:allowBackup="true"` was observed rather than inferred from a library default, which is exactly what the entry's own confidence note said had not been done. **THE SAME FAIL-OPEN IS REACHABLE WITHOUT BACKUP AT ALL,** through Keystore invalidation after a biometric change; that path is GAP-059's and was fixed in the same wave. **REJECTED:** keeping backup while amending `docs/07` to describe what travels, and keeping backup while fixing only the fail-open. `docs/07`'s "never leaves the phone under any configuration" is now true as written rather than amended.
...
> **OWNER DECISION (2026-09-18):** **DISABLE BACKUP: `"allowBackup": false` IN `mobile/app.json`,** with a test asserting it so a prebuild cannot quietly restore the default. Rejected: keeping backup and amending `docs/07`, and keeping backup while only fixing the fail-open. **THE ENTRY IS WRONG ABOUT THE BLAST RADIUS IN BOTH DIRECTIONS, AND THE CORRECTION IS WHY THIS WAS WORTH DOING.** It says Auto Backup uploads `files/`, `shared_prefs/` and `databases/`. It does not: expo-secure-store's config plugin already merges `fullBackupContent="@xml/secure_store_backup_rules"` and `dataExtractionRules="@xml/secure_store_data_extraction_rules"` into the manifest (observed in the built `AndroidManifest.xml`, which also confirms `android:allowBackup="true"` and upgrades this entry from C2 to C1), and both rules files read `<include domain="sharedpref" path="."/>` with `<exclude domain="sharedpref" path="SecureStore"/>`. A single `<include>` makes Android's rules ALLOWLIST-ONLY, so the sealed buffer under `files/` and the encrypted database under `databases/` never travelled at all. The entry's proposed fix of "add a `dataExtractionRules` file" was therefore already moot. **WHAT DID TRAVEL IS WORSE THAN THE CIPHERTEXT THE ENTRY SHRUGS AT.** `CapturePrefs` keeps three of its six values UNSEALED, `capture_enabled` among them, beside a sealed provider filter whose Keystore key does not travel. So a restored install came back with capture ON and a filter that cannot be opened: `openSealed` swallows the failure and returns null (`CapturePrefs.kt:635-637`), `getProviderFilter` maps null to `emptySet()` (`:152`), and `shouldCapture` returns `filter.isEmpty() || packageName in filter` (`:288`), so an empty filter means CAPTURE EVERYTHING, from every app on the device, silently. That is a privacy failure a restore causes, not a blob nobody can read. **THE SAME FAIL-OPEN IS REACHABLE WITHOUT BACKUP AT ALL**, through Keystore invalidation after a biometric change, which is what GAP-059 is about; this decision closes the backup path only, and GAP-059 still owns the other one.
...
> **REMEDIATION: DONE** (2026-09-09) - commit ba02d7b, branch worktree-gap-wave-16. Verification: npx jest transactions lib/db/repos lib/review --maxWorkers=2: 27 suites, 734/734 PASS, re-run by the orchestrator and matching the agent's own figures exactly. npx tsc --noEmit exits 0. THE ENTRY AND THE ORCHESTRATOR'S BRIEF WERE BOTH WRONG IN THREE PLACES, all found by the agent and all verified first-hand before acceptance. (1) THE ORCHESTRATOR'S 'MISSED FOURTH FOREIGN KEY' WAS A FALSE ALARM, now struck through in the checklist above: deleteTransaction already NULLs wallets.drift_dismissed_transaction_id as the first statement of its own SQL transaction and its doc comment already said so; the orchestrator had grepped the signature and never read the body. (2) 'UNLINK FIRST' IS A DEAD END: unlinkTransfer DISSOLVES AND NEVER DELETES ('DISSOLVED, NOT DELETED' in its own header), so both legs stay NOT NULL references and the delete would still fail on a foreign key naming a link the user was just told they had broken -- the dissolved row itself has to go. Anyone working from the entry alone would have shipped that bug. (3) THE DEFECT NEITHER THE ENTRY NOR THE BRIEF SAW: the delete undid itself at the next launch. listUnprocessedRawCaptures selects captures referenced by neither a transaction nor a queue row, so a deliberately deleted notification-sourced row was indistinguishable from one stranded by a crash and startIngest re-committed it under a new id -- 'delete' was a lie until the 30-day TTL. Fixed with the same resolved marker mergeDuplicate already wrote, extracted to lib/review/capture_marker.ts. ORCHESTRATOR VERIFIED THE MARKER IS DURABLE: purgeExpired deletes only rows WHERE resolved_at IS NULL, and the marker is resolved at creation, so it is never purged and outlives the capture. FK decisions: loan_payments and bill_payments RELEASED (rows deleted; both deletePayment and deleteBillPayment had zero production callers, so refusing would have been a permanent dead end, and deleting a bill payment reopens its cycle); active transfer_links REFUSED with a message naming the counterpart wallet; dissolved links deleted under a SQL-guarded status check; drift dismissal already handled. The repo primitive is untouched and transactions_repo.test.ts:1663's raw /FOREIGN KEY/i rejection still passes, so the merge path is unaffected. NOT VERIFIED: nothing run on device, the delete button has never been tapped on the A54; the possible-duplicate marker was checked against listOpen, countOpen, listOpenPage, countOpenByKind, purgeExpired and undoResolution but not against every future consumer of resolved rows. PRE-EXISTING FLAKE RECORDED, NOT CAUSED HERE: two 'filters compose (AND)' tests in app/__tests__/transactions_screen.test.tsx exceed the 30 s timeout under 4 workers and pass alone at 24.4 s and 24.8 s, so worker contention tips them over a ceiling they were already close to
...
- [ ] **Handle the foreign keys.** `PRAGMA foreign_keys = ON`, and FOUR references to `transactions(id)` block a naive delete, not the three first filed. `loan_payments.transaction_id` (`001_core.sql:148`) and `bill_payments.transaction_id` (`:169`) are NOT NULL UNIQUE REFERENCES; `transfer_links.out_transaction_id`/`in_transaction_id` (`:78-79`) are NOT NULL REFERENCES; none carry ON DELETE. ~~**ADDED 2026-09-10:** `wallets.drift_dismissed_transaction_id` (`014_drop_wallet_type.sql:38`, restating `003_drift_dismissal.sql`) is a fourth reference with no ON DELETE. It is nullable, so it can simply be cleared -- a dismissal naming a deleted transaction is meaningless -- but it raises the same FOREIGN KEY error if ignored.~~ **RETRACTED 2026-09-10, THE SAME DAY IT WAS ADDED. THIS WAS A FALSE ALARM BY THE ORCHESTRATOR.** The fourth reference is real, but it was ALREADY HANDLED before this wave began: `deleteTransaction` clears it as the first statement inside its own SQL transaction (`transactions_repo.ts`, `UPDATE wallets SET drift_dismissed_transaction_id = NULL WHERE drift_dismissed_transaction_id = ?`), and the function's doc comment already said so outright -- "the ONE reference this function clears instead of refusing, because it is a note about what the user has read rather than a claim about money". The orchestrator grepped for the function's signature line, never read its body or the comment directly above it, and filed a defect that did not exist. The lesson is the campaign's own: READ THE BODY, NOT THE SIGNATURE. Nothing needed doing; the implementing agent added a test pinning the behaviour, because nothing had pinned it before. `loan_match_rejections.transaction_id` (`019_loan_match_rejections.sql:31`) is ON DELETE CASCADE and needs nothing. A naive delete raises a raw error naming nothing the user can act on. Decide per kind: refuse with an explanation, or unlink first.
...
> **REMEDIATION: PARTIAL** (2026-09-09) - commit 808811e, branch worktree-gap-wave-16. Verification: jest lib/income safe_to_spend 12 suites 216 PASS, jest lib/goals lib/events 3 suites 53 PASS, jest payday components/income 4 suites 31 PASS. FULL LOCAL SWEEP: 271 suites / 4,746 tests PASS; `npx tsc --noEmit` exit 0. **DELIBERATELY PARTIAL: THE ENTRY'S TITLE NAMES A CASE THAT CANNOT BE FIXED AT THIS LOCATION, AND THE DEFECT ACTUALLY FIXED IS A DIFFERENT AND WORSE ONE THE ENTRY NEVER MENTIONS.** The entry says a split payday "fires no payday prompt, because each credit is screened against the average". Both halves of that are wrong, and both were checked against running code before anything was changed. (1) HABITUAL SPLIT -- an employer who splits EVERY payday: a prompt DID fire, exactly once, but for HALF the pay. `detectCadence` records one matched credit per expected window, so `averageAmount` is itself half (probe: `averageAmount=925000` against a real payday of 1850000). Goals rule 13 makes a percent contribution's base "the sum of income Transactions detected on that payday date", so `proposePaydayAllocations` was taking its cut of half the money and capping `budget` at half. THAT is the money defect, and it is fixed. (2) OCCASIONAL SPLIT -- a history of whole paydays with one arriving halved, which is what the title describes: still not fixed, and NOT fixable in `income_service.ts`. The halves never arrive there. `primaryStream` bands each credit against its group's own running median at 30% (`lib/income/candidates.ts:168`) and then keeps the largest group (`:184`), so two half-sized credits form a second group of two and lose the sort; probe confirms `stream=[6 x 1850000]` with the halves absent and `averageAmount` unmoved. Verified independently by the orchestrator by reading `candidates.ts:152-184`. **FILED AS GAP-117** rather than smuggled in here, because banding on per-date totals moves `averageAmount`, which feeds `monthlyEquivalent`, `baseFor` and every percent-of-income Limit, so existing users' headroom would change -- an owner decision, not an agent's, and not the complexity-S change this entry was filed as. SHIPPED: `maybeEmitPayday` screens collapsed paydays, not credits; the band accepts the day's combined total OR any single credit on the day, the second clause being what keeps a habitual splitter's pay announced at all given their halved average; the collapse moved to its own module `lib/income/paydays.ts` and is now shared with `safe_to_spend_service.ts`, which had kept a private copy -- the two must agree or Safe-to-Spend reserves for a transfer the prompt never asks for, the failure GAP-106 fixed on the forecast side; `income:payday` now carries `transactionIds: string[]` and the combined `amount` (a single id can only name one half of the sum the payload reports, and every covered id must be remembered or the sibling credit re-announces the payday after a restart -- no production consumer read the old field, only tests); `walletId` names the wallet holding the largest share, since its only consumer treats it as the account a transfer leaves from. The irregular path collapses too but keeps `IRREGULAR_PAYDAY_FLOOR` per credit: with no average to compare against the floor is the only thing separating pay from noise, so summing sub-floor credits until they clear it would manufacture paydays from exactly the credits it excludes. `PAYDAY_AMOUNT_TOLERANCE` untouched at `income_service.ts:105`, as the entry's "Do not" requires. **ALSO RECORDED, NOT FIXED:** the pre-existing test `maybeEmitPayday ignores a credit far from the average amount` passes VACUOUSLY -- the stream band and rule 11's tolerance are both 30% of essentially the same median, so its P500.00 credit is banded out upstream and the test never exercises the tolerance it is named for. Left alone rather than changed as a drive-by; carried into GAP-117's checklist.
...
> **REMEDIATION: DONE** (2026-09-09) - commit 55a282b, branch worktree-gap-wave-16. Verification: jest lib/ingest + lib/db/repos/__tests__/transactions_repo.test.ts 15 suites 502 PASS, re-run independently by the orchestrator (scoped away from `lib/db/repos` as a whole because a concurrent agent was mid-edit on `app_settings_repo.ts`); the agent's own `jest lib/ingest lib/db/repos` 33 suites 933 PASS; `npx tsc --noEmit` exit 0. **REVERT WITNESS, NOT A CLAIM:** restoring `pipeline.ts` to `listTransactions({})` fails the new pipeline test with `Expected: "cat_groceries_palengke" / Received: "cat_uncategorized"`, so on Free a merchant filed the same way three times really was forgotten the moment those rows crossed the floor. THE ENTRY'S OPEN QUESTION -- "full ledger, or an explicit window?" -- IS ANSWERED FULL LEDGER, on two independent grounds verified first-hand: the ingest plan's §8 rule 3 (`docs/superpowers/plans/2026-08-02-mobile-ingest-m1b-pipeline.md:384`) sets no window on the learned count ("three or more times in `history`", unqualified), and `sumSpend`'s GAP-105 doc comment (`transactions_repo.ts:710-730`) already settles the principle, quoting limits rule 8: "Limit totals are always computed from the full ledger, regardless of the free tier's 90-day history view gate". Shipped as a SEPARATE FUNCTION, `listFullLedger` (`transactions_repo.ts:741`), not a `TxFilter` flag: `hooks/queries/use_transactions.ts:24` passes a caller-supplied filter straight through, so a flag would put a browsing-gate switch one prop away from the ledger screen. `listTransactions` keeps its floor untouched, as the entry's "Do not" requires. Orchestrator checked the one thing the report did not address: `listTransactions` has no default LIMIT, so `listFullLedger()` and `listTransactions({})` issue the same query on plus and the change is a genuine no-op there -- and the repo test asserts that equivalence rather than leaving it as prose. The categorizer's transfer-link and uncategorized exclusions were deliberately NOT pushed into SQL, since that would duplicate `resolveLearned`'s rules with nothing keeping the two in step. **AUDIT FINDING SPUN OUT AS GAP-118:** the agent swept every other `listTransactions` caller and found two computations still inheriting the floor -- income cadence detection asks 130 days (`income_service.ts:98`) and recurring detection asks 800 (`recurring_service.ts:47`), both cut to 90 on Free. Verified independently by the orchestrator, including that `hasRecurringDetection()` gates only the UI at `subscriptions.tsx:118` while `runRecurringPass` runs unconditionally. Correctly left unfixed here: both call sites comment that they pass `now` through BECAUSE the floor applies, so they read as decisions to review rather than oversights. One cite in the orchestrator's brief was wrong and is corrected: `resolveLearned` is DECLARED at `categorizer.ts:282` but CALLED at `:337`, not `:283`.
...
> **REMEDIATION: DONE** (2026-09-09) - commit 7b5cc28, branch worktree-gap-wave-16. Verification: jest lib/bills components/bills bills_screen 9 suites 181 PASS (was 171; +10 tests), re-run independently by the orchestrator. FULL LOCAL SWEEP: 271 suites / 4,746 tests PASS across all three chunks; `npx tsc --noEmit` exit 0. Reproduced red first: before the fix, `A WEEKLY BILL'S WINDOW IS CLAMPED TO HALF ITS PERIOD` received a candidate `{merchant: MERALCO TOO EARLY, score: 0.95}` where `[]` was expected. **THE ENTRY'S PRESCRIPTION WAS WRONG AND WAS NOT FOLLOWED.** Its checklist said "clamp both edges by half the period"; rule 26 (`docs/04-features/07-bills.md:126`) keeps an overdue cycle open 30 days "superseding rule 15's close", so clamping the close would have repealed rule 26 for every bill with a period under 60 days -- monthly included (half of 30.33 floors to 15, so the close would never reach 30), which is the late-Meralco case rule 26 exists for. Shipped instead: the OPEN edge is clamped in both states, the close is clamped only while the cycle is not yet overdue. Two further readings had to be settled and are documented in code at `rule_summary.ts:169-197`: (1) PER EDGE, NOT PER TOTAL WIDTH -- as a total-width cap the rule contradicts its own first clause, since 7+15=22 already exceeds half a monthly period and no bill could ever receive the figures the same sentence promises; per-edge makes the monthly 7/15 exactly satisfiable; (2) HALF IS FLOORED, because rule 15 says never *wider* than half, and two adjacent 4-day edges would overlap on two days and re-offer one payment to both cycles. `DueRule` has no period field, so `periodDays` (`due_rules.ts:264`) derives it from the rule's own occurrences across 52 weeks -- AVERAGED, not the gap to the next cycle, because the forward gap is 28 days from a February due date and would narrow every monthly window each February and change the detail screen's sentence month to month; a constant table per `kind` was rejected as a second model of the calendar free to drift from `due_rules.ts`. **DISCLOSED, NOT PAPERED OVER: the close-side clamp is behaviourally inert.** `bills_service.ts:359` caps the query at `Math.min(to, now + DAY_MS)` and a live cycle has `dueDate >= today`, so the close is clipped by the clock regardless; the OPEN clamp is the entire behavioural fix, and the close-side coverage therefore lives in `rule_summary.test.ts` rather than in a matcher test. The scoring band at `bills_service.ts:315` is deliberately left unclamped (it ranks what the window already admitted; following rule 26's 30 days would give a payment 29 days late the same +0.1 as one 4 days late). `autoMatchRule.dateWindowDays` (`:570`) left alone: written at two sites, read by none, and freezing a cycle-dependent clamped number into stored rule data would go stale the moment the due rule is edited. **RESIDUAL FOR THE OWNER, BY DESIGN NOT BY OVERSIGHT:** an overdue weekly cycle's window is 30 days by rule 26 and so still spans later cycles; rule 17 plus `claimedTransactionIds()` prevent double settlement and the gap band favours the nearer cycle, but whether rule 26 should itself be clamped for short-period bills is a spec question, not a code one.
...
> **REMEDIATION: DONE** (2026-09-09) - commit 7fc30b0, branch worktree-gap-wave-16. Verification: npx jest use_set_provider_pause privacy_screen modules bootstrap provider_picker providers_step --maxWorkers=1: 9 suites, 176/176 PASS, re-run by the orchestrator. npx tsc --noEmit exits 0 across the whole project. KOTLIN RUN, NOT LEFT UNRUN, AND INDEPENDENTLY RECOUNTED: expo prebuild --no-install then ./gradlew :notification_listener:testDebugUnitTest, BUILD SUCCESSFUL; the orchestrator summed the worktree's JUnit XML itself -- 11 classes, tests=170 skipped=0 failures=0 errors=0 -- and confirmed the new cases present by name in the XML ('an allowlist that could not be sealed reports failure and leaves the stored one alone', 'a resume that could not be sealed reports failure and leaves the block standing', 'a deny-all that landed without its allowlist still reports success'). That is 163 + 7. REVERT WITNESS: flipping the seal-failure branch's 'return false' to 'return true' produced BUILD FAILED with exactly the three tests that assert it failing, then restored byte-identical. THE BRIEF'S PROPOSED CONTRACT WAS WRONG AND THE AGENT REFUSED IT. A boolean return is droppable, and two of the three callers already had a catch or try that would have swallowed it silently -- the orchestrator had flagged that trap for providers.tsx specifically; the agent generalized it and chose a REJECTION instead, which is why the onboarding caller needed no behavioural change at all: its existing .catch now fires and does what its comment already claimed. ACCEPTANCE CRITERION DELIBERATELY NARROWED, AND THE NARROWING IS CORRECT: a deny-all that lands without its allowlist still reports success, because the flag is plaintext, shouldCapture consults it before ever opening the filter (verified by the orchestrator at CapturePrefs.kt:260), and setProviderFilter is the flag's only writer whose only path back to false is the full write that replaces the filter in the same commit -- so the stale allowlist is unreachable and reporting failure would instead make the caller drop paused_provider_packages for a block the device really is applying, undoing half of GAP-103. Failure copy is screen-specific and replaces the app-wide card under the same dedupeKey rather than using meta.errorToast:false, so the worst case is the generic card, not silence. NOT VERIFIED: no on-device run -- the seal failure is simulated with a FakeKeyVault holding no prefs KEK, and a real Keystore refusing the key was not reproduced; the AsyncFunction DSL body is still covered only by module_wiring's text assertion, so 'the rejection crosses the real bridge into JS' is proven in two halves (Kotlin throws the code, JS maps it) but never end to end -- the same gap GAP-103's stage-3 device check closed for two-argument marshalling. FLAGGED FOR FILING: an onboarding provider selection that fails to seal is still lost silently, because resyncProviderFilter only knows paused_provider_packages and providers.tsx never writes it -- pre-existing, GAP-092 shaped, not filed yet
...
> **REMEDIATION: DONE** (2026-09-09) - commit 829963a, branch worktree-gap-wave-16. Verification: npx jest loan_card: 13/13 PASS. The card had NO progress test before this, so both new ones were proved against the code they guard rather than assumed: the adjustment case fails on the old principal-minus-outstanding formula with 'Expected: 33, Received: 17', and the amortized case fails on the paidTotal substitution the entry warns about with 'Expected: 50, Received: 55'. npx tsc --noEmit exits 0. The entry's line cite (loan_card.tsx:105) was ACCURATE, unlike its neighbours in this wave. principalApplied(loan.schedule ?? null, status.paidTotal) leaves flat and free-form loans unmoved because for those every peso is principal. The now-unused destructured outstanding was dropped; every remaining read already went through status
...
> **REMEDIATION: DONE** (2026-09-09) - commit 6a6dc08, branch worktree-gap-wave-16. Verification: jest bootstrap provider_picker providers_step privacy_screen onboarding 25 suites 291 PASS (`EXIT=0`), re-run by the orchestrator on a clean machine with nothing competing; the agent's own scoped run 4 suites 109 PASS and `jest onboarding` 23 suites 233 PASS; `npx tsc --noEmit` exit 0. **CI PASS: run `34418411897` on `6a6dc08`, `completed / success`** -- the authoritative full-suite gate, because a full LOCAL sweep was attempted and KILLED by the OS for low memory (the machine had 1.01 GB free of 23.71 GB, `r5apex_dx12` holding 7.4 GB), and a killed sweep is not a PASS. **THE ENTRY'S CHECKLIST ITEM 1 WAS UNIMPLEMENTABLE AS WRITTEN and was not followed.** It said to write `paused_provider_packages` from `providers.tsx`; a `setSetting` there could only ever throw. That screen is one of the three pre-flow onboarding screens rendered above the unlock gate: `app/(onboarding)/index.tsx:58-59` says "the three pre-flow screens, none of which touches the database", and `lib/db/database.ts:365-368` returns `Promise.reject(new DatabaseLockedError())` while `dbPromise === null`; the `app_settings` table does not exist yet either, since `runMigrations` lives inside the `bootstrapApp()` that unlock fires. All verified first-hand by the orchestrator. Shipped instead: an in-memory hand-off, `mobile/lib/onboarding/pending_provider_pause.ts`, persisted by `persistOnboardingProviderPause` (`lib/bootstrap.ts:182-192`) called at `:66` BEFORE `resyncProviderFilter()` at `:73`, so the launch that first stores the row is the one that pushes it back across the bridge. **THE ORCHESTRATOR'S OWN BRIEF WAS WRONG AND THE AGENT CAUGHT IT.** The brief judged observed-but-unknown packages harmless; they are catastrophic when the user ticks ONLY packages the ruleset has never heard of (a rural bank the seed does not carry, offered from `listObservedPackages`), because `paused = universe - {unknown}` is then the ENTIRE universe, non-empty, and `resyncProviderFilter` answers `setProviderFilter([], true)` -- a deny-all blocking the one bank they asked for. Three states are therefore never recorded, each pinned by a test through a real seeded DB: an empty allowlist (Skip means ALLOW-ALL not allow-nothing, per `providers.tsx:13-19` quoting rule 2 and `CapturePrefs.shouldCapture`), everything-known-allowed (nothing to narrow), and only-unknown-allowed. **ALSO FOUND BY THE AGENT, NOT IN THE ENTRY:** `bootstrapApp()` runs on EVERY unlock, not once per process, so an uncleared record would re-write a stale onboarding choice over a pause the user had since changed in the Privacy centre; the record is therefore cleared ONLY on a successful `setSetting`, and a swallowed write leaves it pending for the next launch. AsyncStorage was rejected as the holding place despite being the only durable pre-unlock store: it is plaintext, and which banks a user has is exactly what this app keeps inside SQLCipher, so a copy there would add a `lib/security/wipe.ts` obligation. Known and accepted cost: a process death between the provider step and the first unlock loses the record, where the selection is already unrecoverable because the step never re-runs for a keyed install. The row write is deliberately the INVERSE of the Privacy centre's native-first ordering (`hooks/mutations/use_set_provider_pause.ts:72-88`): that rule stops a switch list showing a row nobody applied, but on the launch path there is nobody on screen to mislead and a record outliving a failed push is the entire point. GAP-092's narrowing invariant re-proved by four tests. **NO USER-FACING MESSAGE, left as the owner's call as the entry asks** (`PROVIDER_PAUSE_NOT_STORED_TOAST` exists to borrow copy from); the free consequence is that the Privacy centre's switch rows now show the onboarding selection for the first time, so the record is visible wherever the user looks.
...
> **REMEDIATION: DONE** (2026-09-17) - commit 5ccfe66, branch worktree-gap-wave-17. Verification: **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand; verification of record, because wave 17's full local sweep never completed (two attempts killed by the OS for low memory). **Orchestrator-run scoped suites, 2026-09-17:** `lib/income lib/recurring lib/db/repos` 30 suites / 698 tests PASS; `npx tsc --noEmit` exit 0. **THIS WAS A SPLIT DECISION AND ONE HALF WAS REVERSED INSIDE THE SAME WAVE, BY GAP-122 (`7dc0c57`), ABOUT AN HOUR LATER. THE ORCHESTRATOR'S OWN FRAMING CAUSED THE REVERSAL, and that is the durable lesson here.** The recurring half ("do not run the pass at all on Free") was put to the owner as "the results are invisible on Free anyway", which is true of the shipped build and FALSE of the spec: reports rule 19 (`docs/04-features/10-reports.md:98`) promises Free "the count of detected patterns only", and `LockedPreview({ count })` was already written and already wired at its call site. Asked again once the conflict surfaced, the owner chose to honour rule 19, so `5ccfe66`'s Free skip in `recurring_ledger_subscriber` no longer exists. A question framed from the code rather than the doc produced a decision the doc had already settled. **THE INCOME HALF STANDS AND IS THE DURABLE PART OF THIS COMMIT:** cadence detection reads its full 130 days regardless of tier, through `listFullLedgerBetween` (`lib/income/income_service.ts:200`, window built at `:171`, `LEDGER_WINDOW_DAYS = 130` at `:111`). Not sent to the owner, because GAP-105 (`sumSpend`) and GAP-111 (`listFullLedger`) already settled the same computation-versus-browsing question, and unlike recurring detection income cadence is tier-gated NOWHERE, so its sample must not move with the tier. **THE ENTRY OVERSTATED THE INCOME DAMAGE, AND THE REAL DAMAGE LANDS IN THE SAME PLACE ANYWAY.** The entry predicted a monthly-paid Free user might never reach a confirmed cadence at all; measured, cadence still confirms on Free. What actually moved is `averageAmount`, which medians the last four paydays and on Free was handed three: the figure shifted 5% in the fixture, and it feeds `monthlyEquivalent`, `baseFor` and therefore every percent-of-income Limit, which is the blast radius GAP-117 describes. A narrower defect with the same reach. **`listFullLedgerBetween` IS A SIBLING OF `sumSpend` AND `listFullLedger`, NEVER A FLAG ON `TxFilter`** (`lib/db/repos/transactions_repo.ts:798`). A flag would hand every future caller a switch that turns the browsing gate off, and `hooks/queries/use_transactions.ts` passes a caller-supplied filter straight through to the repository. `listFullLedger` itself is deliberately UNBOUNDED, so reaching for it here would have widened detection from 130 days to the entire ledger, which nobody decided. The 90-day floor stays on `listTransactions` for every browsing caller, per the entry's Do-not. **LINE CITES IN THE ENTRY HAVE DRIFTED, corrected by reading the files on 2026-09-17:** `LEDGER_WINDOW_DAYS` is at `income_service.ts:111` and `recurring_service.ts:63`, not `:98` and `:47`, and `listFullLedgerBetween` is defined at `transactions_repo.ts:798`, not `:788`.

**Location**
- `mobile/lib/income/income_service.ts:98` (`const LEDGER_WINDOW_DAYS = 130`), the window built at `:156` and read at `:172` (`listTransactions({ from, to: now + 1, now })`)
- `mobile/lib/recurring/recurring_service.ts:47` (`const LEDGER_WINDOW_DAYS = 800`), the window built at `:132` and read at `:136` (the same call shape)
- `mobile/lib/db/repos/transactions_repo.ts:666` (`listTransactions` raises `from` to `historyFloor`), `:741` (`listFullLedger`, the exemption GAP-111 added)
- `docs/04-features/03-limits.md:79` (limits rule 8), `docs/05-monetization.md:82` (§3.3)

**Evidence**
Found by the GAP-111 agent while auditing every `listTransactions` caller for the same defect, and verified independently by the orchestrator on 2026-09-10 by reading all four call sites. Both callers pass `now` through to the repository, and both carry a comment saying they do so precisely because "the tier's history floor is part of this window" -- so the floor's application is known at each site, which is why this is filed as a decision to review rather than as an oversight.

**What is wrong**
GAP-111 established the principle for the categorizer and `listFullLedger` now expresses it: the 90-day floor is a BROWSING gate, and a computation must read the window it asks for. Two computations still inherit the gate:

- Income cadence detection asks for 130 days and receives 90 on Free. Rule 6's "confirmed" threshold wants six paydays. Ninety days holds about six kinsenas paydays and only three MONTHLY ones, so a monthly-paid Free user may never reach a confirmed cadence at all -- no payday prompt, no auto-allocation, and `averageAmount` computed from a third of the intended sample.
- Recurring detection asks for 800 days and receives 90 on Free. An annual subscription cannot be detected inside 90 days by construction.

**Why it matters**
`averageAmount` feeds `monthlyEquivalent`, which feeds `baseFor` and therefore every percent-of-income Limit, so a truncated income sample does not stay inside the income feature -- it moves limit headroom, the same blast radius GAP-117 describes. Both are latent today because `MVP_TIER` is hardcoded to `plus` (`mobile/lib/entitlements.ts:13`), which is exactly the condition under which this kind of defect ships unnoticed.

The recurring case needs a decision before it needs a fix. `hasRecurringDetection()` (`mobile/lib/entitlements.ts:58`) is called in exactly ONE place, the UI branch at `mobile/app/(tabs)/more/subscriptions.tsx:118`; `runRecurringPass` (`mobile/lib/recurring/recurring_ledger_subscriber.ts:37`) runs unconditionally. Verified by grepping every non-test caller. So on Free the detector runs, spends the work, and sees 90 days of the 800 it asked for, while its results are hidden anyway. Whether the right answer is "exempt the window", "do not run the pass on Free", or "leave it, the results are gated" is a product question, not a code one.

**Intended behavior**
A computation reads the window it asks for, or it is not run at all. Whichever is chosen, the call site says which and why, as `listFullLedger` and `sumSpend` now do.

**Proposed fix**
For income detection, read the asked-for 130 days regardless of tier. `listFullLedger` is deliberately unfiltered and so is the wrong tool for a bounded window; this needs either a floor-exempt bounded read or an explicit decision that 90 days is enough for cadence, recorded at `income_service.ts:98`.

For recurring detection, take the product decision first, then make the call site state it.

**Implementation checklist**
- [ ] Decide, with the owner, whether recurring detection should run at all on Free given its results are UI-gated.
- [ ] Give income cadence detection the 130 days it asks for, or record why 90 is enough.
- [ ] Whichever is chosen for recurring, state it at `recurring_service.ts:47` in the voice `listFullLedger` uses.
- [ ] Tests with a Free entitlement (`__setTierForTests`) and rows older than 90 days, asserting the chosen behaviour for each.
- [ ] Re-check that no NEW floor-exempt read is reachable from a browsing surface.

**Acceptance criteria**
- [ ] Income cadence detection's sample does not change with the tier, or the code says why it may.
- [ ] Recurring detection either sees its full window or does not run, with the choice stated at the call site.
- [ ] `listTransactions` still applies the floor for every browsing caller.

**Verification commands**
```bash
cd mobile && npx jest lib/income lib/recurring lib/db/repos --maxWorkers=4
cd mobile && npx tsc --noEmit
```

**Do not**
Do not remove `historyFloor` from `listTransactions`; the browsing gate is the feature (the same "do not" GAP-105 and GAP-111 carry). Do not reach for `listFullLedger` for a bounded window -- it is unfiltered by design, and using it here would widen two windows to "everything" without anyone deciding that.
...
> **REMEDIATION: DONE** (2026-09-17) - commit e4c1bfe, branch worktree-gap-wave-19. Verification: **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** `jest plan_segments limit_routes components/income` 3 suites / 53 tests PASS, `npx tsc --noEmit` exit 0. **BOTH NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `limits_panel.tsx` alone and re-running them. **THE ENTRY'S PRESCRIPTION WAS RIGHT AND WAS FOLLOWED EXACTLY, WHICH IS WORTH RECORDING BECAUSE IT IS THE EXCEPTION IN THIS CAMPAIGN.** Its three cites all held on reading: the notice block at `mobile/components/income/income_summary_card.tsx:145`, the unconditional row at `mobile/components/plan/limits_panel.tsx:154`, and `IncomeSummary.hasSplitPaydayNotice` at `mobile/lib/income/income_service.ts:107`. **NO NEW QUERY, which the checklist asked for and which the row already made free:** `limits_panel.tsx:88` already holds `const { data: income } = useIncomeSummary()` and reads `income?.monthlyEquivalent` at `:149`, so the caption reads `income?.hasSplitPaydayNotice` off the object that was already there. A second query would have let the notice and the number it explains render a frame apart, which is the reason the flag is a field on the summary rather than its own hook (`income_service.ts:102-107`). **BOUND TO THE SAME FLAG THE CARD SETTLES, never a second one-time flag**, per the entry's Do-not: `hasSplitPaydayNotice` is what `dismissSplitPaydayNotice` clears by writing `income_split_payday_notice = "done"` (`income_service.ts:514-516`), so the caption cannot outlive the explanation it points at and the notice still settles exactly once per device. **NO SECOND COPY OF THE EXPLANATION.** The caption is a signpost, "Your income figure changed. Tap to see why.", echoing only the card's own headline; the two paragraphs that do the explaining stay on the card, so there is nothing to drift. No new row and no new navigation target: the existing `plan-income-row` already pushes `/plan/income`. **THE TESTS DRIVE THE REAL SETTING RATHER THAN MOCKING THE HOOK**, which is what let the second one assert the acceptance criterion literally: it writes `"done"`, the value the card's "Got it" button produces, so the test is "the pointer is gone once the notice has been acknowledged" rather than the weaker "never owed". **ONE TEST-HARNESS TRAP, worth the line because it looked like a product failure:** `toHaveTextContent` matches the whole string here rather than a substring, so the first assertion failed against the correct rendered caption with `Expected: Your income figure changed / Received: Your income figure changed. Tap to see why.`. Switched to regexes. The caption was right the whole time.
...
> **REMEDIATION: DONE** (2026-09-17) - commit 2ac2f3a, branch worktree-gap-wave-19. Verification: **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** the change is three lines in one file and touches no code, confirmed by `git diff --stat`, which reports `docs/04-features/04-income.md` alone at 3 insertions and 3 deletions. `jest lib/income` 8 suites / 133 tests PASS and `npx tsc --noEmit` exit 0 were both run on a tree carrying this commit. **THE ENTRY SCOPED RULES 4 AND 9; RULE 11 WENT WITH THEM, AND LEAVING IT WOULD HAVE BEEN THE SAME DEFECT ONE RULE OVER.** Rule 11 said a candidate event is a matched pay event when "its amount is within ±30% of `averageAmount`", which reads as the credit's amount, while `maybeEmitPayday` calls `collapsePaydays(inScope)` and then applies the band to the day's combined total (`mobile/lib/income/income_service.ts:695`, read to confirm). The entry's own Evidence names rule 11 as changed by GAP-110 without any doc edit, and then its checklist omits it. **RULE 11'S IRREGULAR FLOOR IS DOCUMENTED AS THE EXCEPTION IT GENUINELY IS, which a doc-only rewrite would have got wrong by assuming the collapse applies everywhere:** `income_service.ts:702-711` keeps the ₱1,000.00 floor PER CREDIT, because there is no average to test against on that path and summing sub-floor credits until they clear the floor would manufacture paydays out of exactly the small credits the floor exists to exclude; the payday it produces still carries the day's total. **RULE 4 GAINED THE THREE LOAD-BEARING PARTS THAT WERE NOWHERE IN THE SPEC AND SURVIVED ONLY AS COMMENTS:** the collapse of one local date into a single banded candidate, a key's first payday seeding its group whole before any fallback can apply (seeding from a half would set the running median to half a payday and put every later whole payday out of band, which is the original bug reintroduced through the escape hatch), and the out-of-band-day fallback that re-tests that day credit by credit so a ₱50,000 13th month cannot evict the ₱18,500 salary it landed beside, which is rule 8's promise. "Largest recurring group" is now also stated as counting PAYDAYS rather than credits, or a stream paid in halves outranks a genuinely more frequent one on deposit habit alone. **RULE 6'S KINSENAS ROW WAS CONSIDERED AND DELIBERATELY LEFT ALONE.** Its wording is "credits landing alternately in the two pay windows", but its evidence thresholds already count matched WINDOWS ("3 consecutive matched windows", "4 of the last 5 expected windows"), and `tryKinsenas` scores windows too; once rule 11 says the unit is the payday, rule 6 inherits it. Changing it would have been churn, not correction. **THE ARCHIVED PLAN AT `docs/superpowers/plans/2026-08-02-mobile-control-m2.md` REPEATS THE OLD WORDING AND IS DELIBERATELY UNTOUCHED:** it records a plan that was executed, not the current spec, and editing it would falsify the history rather than fix a document anyone implements from. `grep -rn "running median" docs/04-features/` now returns rule 4 alone. **LINE CITES IN THE ENTRY HAD DRIFTED, corrected by reading:** `averageAmountFor` is at `income_math.ts:97` not `:131`, and `tryKinsenas` at `cadence_detector.ts:145` not `:159`. Rule 11 itself is at `04-income.md:101`. No code was changed, which is the entry's own second acceptance criterion.
...
> **REMEDIATION: DONE** (2026-09-17) - commit 7dc0c57, branch worktree-gap-wave-17. Verification: **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), re-verified first-hand; verification of record, because wave 17's full local sweep never completed (two attempts killed by the OS for low memory). **Orchestrator-run scoped suites, 2026-09-17:** `components/gates more_hub subscriptions_screen` 3 suites / 46 tests PASS and `lib/income lib/recurring lib/db/repos` 30 suites / 698 tests PASS; `npx tsc --noEmit` exit 0. **THREE THINGS HAD TO MOVE, NOT ONE, AND EACH ALONE LEAVES RULE 19 STILL BROKEN.** The Free skip goes (reversing GAP-118's recurring half within the same wave, an hour after it landed); the 800-day window is exempted through `listFullLedgerBetween` (`lib/recurring/recurring_service.ts:164`, window at `:148`, `LEDGER_WINDOW_DAYS = 800` at `:63`); and the More-tab row navigates on Free. **THE WINDOW EXEMPTION IS NOT OPTIONAL, and it is the piece most easily dropped as a nice-to-have.** A count computed from a floor-clamped 90 days understates the answer, and three instances of an annual charge span about two years, so the subscription a user most wants flagged could never be counted at all and the teaser would lie in the one direction that costs a conversion, "we found nothing". **`PlusGate` GAINED AN OPT-IN `lockedPress` PROP DEFAULTING TO `"intercept"`** (`components/gates/plus_gate.tsx:91`, passthrough branch at `:131`), so every existing gated call site behaves exactly as before and exactly ONE row opts into `"passthrough"` (`app/(tabs)/more/index.tsx:309`). Verified safe by reading the screen, not by trusting the prop: the data hiding was never the row's job, because `app/(tabs)/more/subscriptions.tsx:124-125` early-returns `<LockedPreview count={patterns?.length ?? 0} />` before any pattern reaches `PatternCard`, `LockedInHeader` or `monthlyLockedIn`. Rule 19 puts the gate at the entitlements call site, and that is now where it is. **`decayStalePatterns` NOW RUNS ON FREE, AND THE WINDOW EXEMPTION IS WHAT MAKES THAT CORRECT, NOT THE GUARD'S REMOVAL.** GAP-118 deliberately skipped decay so a Free period could not forget what a Plus period found, which would be the tier deleting data. With both tiers now detecting from the same 800 days, decay judges the same evidence either way, so a pattern can only be forgotten for having genuinely stopped. This was re-derived rather than assumed, as the entry's checklist demanded. Merchants, amounts and the total stay Plus-only, asserted by test on the screen itself. **ONE PROMISE IN RULE 19 IS NOT CLOSED BY THIS COMMIT AND CANNOT BE TODAY:** "on upgrade, patterns computed from the full retained history appear immediately". `MVP_TIER` is a `const` (`lib/entitlements.ts:13`) whose only mutator is `__setTierForTests`, so there is no runtime tier switch to hang "immediately" on; the promise is unimplementable and unfalsifiable until one exists. Pre-existing, out of scope here, and owed its own entry when tier switching becomes real.
...
> **REMEDIATION: DONE** (2026-09-17) - commit c506133, branch worktree-gap-wave-19. Verification: **CI RUN 35214316033 ON e4c1bfe: 9 OF 9 JOBS SUCCESS** (`typecheck`, six `jest` shards, `tests`, `secret-scan`), read from the completed run. **Orchestrator-run, 2026-09-17:** `jest lib/income` 8 suites / 133 tests PASS, `jest safe_to_spend` 4 suites / 95 tests PASS, `npx tsc --noEmit` exit 0. **BOTH NEW TESTS WERE RUN AGAINST THE UNFIXED SOURCE FIRST AND OBSERVED TO FAIL**, by reverting `income_service.ts` alone and re-running them, so they pin the behaviour rather than merely accompanying it. **READ-THE-WINDOW WAS CHOSEN OVER DOCUMENT-THE-CLAMP**, matching GAP-105 for `sumSpend`, GAP-111 for the categorizer and GAP-118 for cadence detection one function up: `listPayEventsBetween` answers "what pay actually arrived between these two instants", which is a fact about the ledger and not about what the user may browse. It now reads `listFullLedgerBetween`, bounded, never `listFullLedger`, which is unbounded by design and would have widened the caller's window to the whole ledger. The refund lookback still widens `from` by seven days, unchanged, because rule 2 needs the outflows before the range to tell a refund from pay. **THE ENTRY UNDERSTATES THE MECHANISM, AND THE HALF IT MISSES IS THE WORSE ONE.** The entry reads the defect as "reads through `listTransactions` without passing `now`, so it inherits the browsing floor". Omitting `now` does not opt out of the floor at all: `listTransactions` computes it from `filter.now ?? Date.now()` (`mobile/lib/db/repos/transactions_repo.ts:666`, read to confirm), so the omission only pointed the gate at a LIVE WALL CLOCK. The cutoff therefore moved with the real date instead of with the window, and the same call over the same range returned different pay depending on when it ran, which is precisely what `lib/clock.ts` forbids for everything under `lib/`. That drift is also why the new Free test would have been flaky rather than merely wrong: which paydays got clamped depended on the date the suite executed. **THE ENTRY'S STATED HARM CANNOT OCCUR TODAY, AND NOT FOR THE REASON THE ENTRY GIVES.** It calls the gap latent because "no caller asks for a window older than 90 days today". The real reason is stronger and was verified by reading both gates: the only caller is Safe-to-Spend's contributions term (`mobile/lib/safe_to_spend_service.ts:218`, inside `forecastContributions` at `:184`), which returns early unless `hasPaydayAutoAllocation()` holds, and that is `getTier() === "plus"` (`lib/entitlements.ts:76-78`), while `historyWindowDays()` returns a floor only OFF Plus (`:54-56`). The tier that clamps never reaches the call. So the under-reservation of Safe-to-Spend the entry describes was not reachable. The fix still stands: that is a property of one caller's gate, not of this function, and the next caller inherits the trap rather than the exemption. **`listPayEventsBetween` HAS NO BROWSING CALLER**, the entry's third checklist item, confirmed by grepping every `.ts`/`.tsx`: the export, the import in `safe_to_spend_service.ts:19`, and the single call at `:218`. **LINE CITES CORRECTED by reading:** `listFullLedgerBetween` is at `transactions_repo.ts:798` not `:788`, and `forecastContributions` at `safe_to_spend_service.ts:184` not `:183`.
...
### GAP-124 [TEST] The recovery-phrase affordance test scanned the generated words, so a phrase containing the BIP39 word `share` or `copy` failed it at random

> **REMEDIATION: DONE** (2026-09-17) - commit a811029, branch worktree-gap-wave-17. Verification: **FILED AND CLOSED IN ONE PASS**, because the defect was found and fixed inside wave 17 before it had an entry; this row exists so the fix is not invisible to the campaign. **Orchestrator-run 2026-09-17:** `npx jest components/onboarding/__tests__/recovery_phrase --maxWorkers=1`, 1 suite / 36 tests PASS. **CI RUN 34446827319 ON a811029: 9 OF 9 JOBS SUCCESS**, re-verified first-hand; that is the run carrying this fix. **THE FAILURE IT FIXES IS ON THE RECORD:** run `34443170128` on `59d813a` failed the `jest (3)` shard with `Expected pattern: not /share/`, on a branch that had not touched this screen. **THE RATE IS COMPUTED, NOT ESTIMATED.** `mobile/lib/crypto/recovery_phrase.ts:25` sets `WORD_COUNT = 12` and the bundled BIP39 list (`mobile/lib/crypto/wordlist.ts`) holds 2048 words including both `share` and `copy`, verified by reading the list rather than assuming it, so the false-failure probability is 1 - (2046/2048)^12 = **1.17%, about one run in 86**. **THIS RETRACTS AN EARLIER DIAGNOSIS.** Two local failures of this suite in wave 17 were attributed to memory contention and overlapping jest workers, and that was wrong; the suite was genuinely flaky for a reason having nothing to do with the machine. The contention warning still stands on its own evidence for other suites, but it was not what was happening here. **THE FIX NARROWS THE SCAN WITHOUT WEAKENING THE CLAIM:** the assertion serialized the whole rendered step with `screen.toJSON()`, generated words included, so it was pattern-matching random data against the two words it searched for. The `phrase-word-list` subtree (`mobile/components/onboarding/phrase_display.tsx:165`) is now excluded through a `JSON.stringify` replacer and everything else on the step is still scanned in full, since an affordance cannot live inside the generated words. A second assertion, `expect(tree).not.toMatch(/phrase-word-/)`, pins the exclusion itself, so renaming that `testID` fails loudly instead of quietly turning the replacer into a no-op and restoring the coin flip. `queryByTestId("phrase-share-button")` and the platform-share-sheet check are untouched; they were never the flaky part.

**Location**
- `mobile/components/onboarding/__tests__/recovery_phrase.test.tsx:453` (`offers no share or copy affordance, and never reaches the platform share sheet`), the serialize-and-scan at `:455` as it stood before the fix
- `mobile/lib/crypto/wordlist.ts` (the bundled 2048-word BIP39 English list, which contains both `share` and `copy`)
- `mobile/lib/crypto/recovery_phrase.ts:25` (`const WORD_COUNT = 12`)
- `mobile/components/onboarding/phrase_display.tsx:165` (`testID="phrase-word-list"`, the subtree the fix excludes)

**Evidence**
CI run `34443170128` on `59d813a` failed the `jest (3)` shard with `Expected pattern: not /share/`, on a wave 17 branch that had not touched this screen or anything it renders. The assertion read:
...
`screen.toJSON()` serializes the WHOLE rendered step, recovery words included, so the twelve generated words were inside the string being pattern-matched. Both `share` and `copy` are BIP39 words, verified in the bundled list rather than assumed.

**What is wrong**
The test asserted a claim about affordances by scanning a blob that also contained random data drawn from a word list containing the very words it searched for. A phrase that legitimately contained `share` or `copy` failed a screen that was behaving correctly.

**Why it matters**
It is a false negative with a computable rate, not a mystery flake: with 12 words drawn from 2048 and two of them fatal, the failure probability is 1 - (2046/2048)^12 = **1.17%, about one run in 86**. That is frequent enough to redden CI on unrelated branches and rare enough to look like infrastructure, which is exactly what happened: two earlier local failures on this suite in wave 17 were put down to memory contention and jest worker overlap, and that diagnosis was wrong. A test that fails for reasons unrelated to its claim teaches the team to re-run red CI, which is the expensive part.

**Intended behavior**
The assertion covers everything on the step that could be an affordance, and nothing that is generated data.

**Proposed fix**
Exclude the word-list subtree from the serialization and keep scanning the rest of the step in full, then pin the exclusion itself so it cannot silently become a no-op.

**Implementation checklist**
- [x] Exclude the `phrase-word-list` subtree via a `JSON.stringify` replacer, leaving every other node scanned.
- [x] Add a second assertion (`expect(tree).not.toMatch(/phrase-word-/)`) so a renamed `testID` fails loudly instead of restoring the coin flip.
- [x] Leave `expect(screen.queryByTestId("phrase-share-button")).toBeNull()` and the platform-share-sheet check untouched; they were never the flaky part.

**Acceptance criteria**
- [x] The test's outcome does not depend on which words the generator produced.
- [x] A real share or copy affordance added anywhere on the step still fails the test.
- [x] Renaming or removing the word list's `testID` fails the test rather than widening the scan back to the words.

**Verification commands**
```bash
cd mobile && npx jest components/onboarding/__tests__/recovery_phrase --maxWorkers=1
```

**Do not**
Do not weaken the assertion to `queryByTestId` alone; the serialize-and-scan is what catches an affordance added as plain text or as an untagged node. Do not exclude any subtree other than the generated words.

**Rollback**
Revert. Note that reverting restores a 1.17%-per-run false failure.
